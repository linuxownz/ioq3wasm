...
void G_CheckTeamItems( void ) {
    ...
    if( g_gametype.integer == GT_CTF ) {
        ...
        if ( !item || !itemRegistered[ item - bg_itemlist ] ) {
            Com_Printf( "%s", S_COLOR_YELLOW "WARNING: No team_CTF_redflag in map\n" );
        }
        item = BG_FindItem( "Blue Flag" );
        if ( !item || !itemRegistered[ item - bg_itemlist ] ) {
            Com_Printf( "%s", S_COLOR_YELLOW "WARNING: No team_CTF_blueflag in map\n" );
        }
    }
#ifdef MISSIONPACK
    if( g_gametype.integer == GT_1FCTF ) {
        ...
        if ( !item || !itemRegistered[ item - bg_itemlist ] ) {
            Com_Printf( "%s", S_COLOR_YELLOW "WARNING: No team_CTF_neutralflag in map\n" );
        }
    }
    ...
    if( g_gametype.integer == GT_OBELISK ) {
        ...
        if( !ent ) {
            Com_Printf( "%s", S_COLOR_YELLOW "WARNING: No team_redobelisk in map\n" );
        }

        ent = NULL;
        ent = G_Find( ent, FOFS(classname), "team_blueobelisk" );
        if( !ent ) {
            Com_Printf( "%s", S_COLOR_YELLOW "WARNING: No team_blueobelisk in map\n" );
        }
    }
    ...
    if( g_gametype.integer == GT_HARVESTER ) {
        ...
        if( !ent ) {
            Com_Printf( "%s", S_COLOR_YELLOW "WARNING: No team_neutralobelisk in map\n" );
        }
    }
#endif
}
