...
// NULL for everyone
static __attribute__ ((format (printf, 2, 3))) void PrintMsg( gentity_t *ent, const char *fmt, ... ) {
    char        msg[1024];
    va_list     argptr;
    char        *p;
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker)
{
    ...
    if ( ( ( VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS &&
        SV_inPVSIgnorePortals(flag->r.currentOrigin, targ->r.currentOrigin ) ) ||
        ( VectorLength(v2) < CTF_TARGET_PROTECT_RADIUS &&
        SV_inPVSIgnorePortals(flag->r.currentOrigin, attacker->r.currentOrigin ) ) ) &&
    ...
    if (carrier && carrier != attacker) {
        ...
        if ( ( ( VectorLength(v1) < CTF_ATTACKER_PROTECT_RADIUS &&
            SV_inPVSIgnorePortals(carrier->r.currentOrigin, targ->r.currentOrigin ) ) ||
            ( VectorLength(v2) < CTF_ATTACKER_PROTECT_RADIUS &&
                SV_inPVSIgnorePortals(carrier->r.currentOrigin, attacker->r.currentOrigin ) ) ) &&
            attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
...
gentity_t *Team_GetLocation(gentity_t *ent)
{
    ...
    for (eloc = level.locationHead; eloc; eloc = eloc->nextTrain) {
        ...
        if ( !SV_inPVSIgnorePortals( origin, eloc->r.currentOrigin ) ) {
            continue;
        }
...
void TeamplayInfoMessage( gentity_t *ent ) {
    ...
    for (i = 0, cnt = 0; i < g_maxclients.integer && cnt < TEAM_MAXOVERLAY; i++) {
        ...
        if (player->inuse && (int)player->client->sess.sessionTeam == team ) {
...
void ObeliskInit( gentity_t *ent ) {
    ...
    if ( ent->spawnflags & 1 ) {
        ...
        if ( tr.startsolid ) {
            ent->s.origin[2] -= 1;
            Com_Printf( "SpawnObelisk: %s startsolid at %s\n", ent->classname, vtos(ent->s.origin) );
