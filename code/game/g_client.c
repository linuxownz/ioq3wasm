...
============
*/
static gentity_t *SelectRandomFurthestSpawnPoint ( vec3_t avoidPoint, vec3_t origin, vec3_t angles, qboolean isbot ) {
    gentity_t   *spot;
    vec3_t      delta;
    float       dist;
...
void ClientUserinfoChanged( int clientNum ) {
    ...
    if ( !Info_Validate(userinfo) ) {
        ...
        SV_GameDropClient(clientNum, "Invalid userinfo");
    }
...
*/
    ...
    */
    ...
    {
        s = va("n\\%s\\t\\%i\\model\\%s\\headmodel\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\skill\\%s\\tt\\%d\\tl\\%d",
    ...
    {
        s = va("n\\%s\\t\\%i\\model\\%s\\headmodel\\%s\\g_redteam\\%s\\g_blueteam\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\tt\\%d\\tl\\%d",
