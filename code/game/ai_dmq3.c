...
// for the voice chats
#include "../qcommon/menudef.h" // sos001205 - for q3_ui also
...
#if 0
// not used
static qboolean EntityHasQuad(aas_entityinfo_t *entinfo) {
...
==================
*/
static int Bot1FCTFCarryingFlag(bot_state_t *bs) {
...
==================
*/
static int BotHarvesterCarryingCubes(bot_state_t *bs) {
    ...
    if ( bs->lastgoal_ltgtype ) {
        ...
        if ( gametype == GT_CTF ) {
            if ( bs->ltgtype == LTG_GETFLAG ) {
                ...
                tb = BotTeamFlag(bs);
                eb = BotEnemyFlag(bs);
                tt = AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, tb->areanum, TFL_DEFAULT);
                et = AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, eb->areanum, TFL_DEFAULT);
...
==================
*/
static void BotGoHarvest(bot_state_t *bs) {
    //
    if (BotTeam(bs) == TEAM_RED) memcpy(&bs->teamgoal, &blueobelisk, sizeof(bot_goal_t));
    else memcpy(&bs->teamgoal, &redobelisk, sizeof(bot_goal_t));
    //set the ltg type
...
==================
*/
char *stristr(char *str, char *charset) { // TODO this should be static or moved if used elsewhere
    int i;
...
qboolean BotInLavaOrSlime(bot_state_t *bs) {
    ...
    feet[2] -= 23;
    return (AAS_PointContents(feet) & (CONTENTS_LAVA|CONTENTS_SLIME));
}
...
static int BotFuncButtonActivateGoal(bot_state_t *bs, int bspent, bot_activategoal_t *activategoal) {
    ...
    AAS_ValueForBSPEpairKey(bspent, "model", model, sizeof(model));
    if (!*model)
        return qfalse;
    ...
    AAS_FloatForBSPEpairKey(bspent, "lip", &lip);
    if (!lip) lip = 4;
    //get the move direction from the angle
    AAS_FloatForBSPEpairKey(bspent, "angle", &angle);
    ...
    dist *= 0.5;
    //
    AAS_FloatForBSPEpairKey(bspent, "health", &health);
    ...
    if (health) {
        ...
        numareas = AAS_TraceAreas(start, end, areas, NULL, 10);
        //
...
void BotEnableActivateGoalAreas(bot_activategoal_t *activategoal, int enable) {
    ...
    for (i = 0; i < activategoal->numareas; i++)
        AAS_EnableRoutingArea( activategoal->areas[i], enable );
...
static int BotGetActivateGoal(bot_state_t *bs, int entitynum, bot_activategoal_t *activategoal) {
    ...
    for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
        if (!AAS_ValueForBSPEpairKey(ent, "model", tmpmodel, sizeof(tmpmodel))) continue;
    ...
    if (!strcmp(classname, "func_door")) {
        ...
        if (*model) {
            ...
            if (modelindex) {
                ...
                //
                numareas = AAS_BBoxAreas(absmins, absmaxs, areas, MAX_ACTIVATEAREAS*2);
    ...
    for (i = 0; i >= 0 && i < 10;) {
        for (ent = cur_entities[i]; ent; ent = AAS_NextBSPEntity(ent)) {
            if (!AAS_ValueForBSPEpairKey(ent, "target", target, sizeof(target))) continue;
        ...
        if (!strcmp(classname, "func_button")) {
            ...
            if ( AAS_AreaReachability(bs->areanum) ) {
                // disable all areas the blocking entity is in
                BotEnableActivateGoalAreas( activategoal, qfalse );
                //
                t = AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, activategoal->goal.areanum, bs->tfl);
        ...
        // the actual button or trigger might be linked through a target_relay or target_delay
            ...
            if (AAS_ValueForBSPEpairKey(ent, "targetname", targetname[i+1], sizeof(targetname[0]))) {
                i++;
                cur_entities[i] = AAS_NextBSPEntity(0);
            }
        }
    }
#ifdef OBSTACLEDEBUG
...
static void BotPrintActivateGoalInfo(bot_state_t *bs, bot_activategoal_t *activategoal, int bspent) {
    ...
    AAS_ValueForBSPEpairKey(bspent, "classname", classname, sizeof(classname));
    ...
    if (activate && entinfo.modelindex > 0 && entinfo.modelindex <= max_bspmodelindex) {
        ...
        if (bspent) {
            ...
            if (!(moveresult->flags & MOVERESULT_ONTOPOFOBSTACLE) &&
                AAS_AreaReachability(bs->areanum))
                return;
        }
        else {
            // enable any routing areas that were disabled
...
int BotAIPredictObstacles(bot_state_t *bs, bot_goal_t *goal) {
    ...
    AAS_PredictRoute(&route, bs->areanum, bs->origin,
        ...
        if (!BotMatchMessage(bs, m.message)) {
            ...
            if (m.type == CMS_CHAT && !bot_nochat.integer) {
                ...
                if (bot_testrchat.integer) {
                    //
                    /*botlib_*/Export_BotLibVarSet("bot_testrchat", "1");
...
bot_goal_t *BotAlternateRoute(bot_state_t *bs, bot_goal_t *goal) {
    ...
    if (bs->altroutegoal.areanum) {
        ...
        t = AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, bs->altroutegoal.areanum, bs->tfl);
        if (t && t < 20) {
...
#if 0
// notused
static void BotSetEntityNumForGoalWithModel(bot_goal_t *goal, int eType, char *modelname) {
    gentity_t *ent;
    int i, modelindex;
    vec3_t dir;
...
#if 0
// not used
static int BotGoalForBSPEntity( char *classname, bot_goal_t *goal ) {
    ...
    for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
        if (!AAS_ValueForBSPEpairKey(ent, "classname", value, sizeof(value)))
            continue;
...
void BotSetupDeathmatchAI(void) {
    ...
    Cvar_Register(&bot_grapple, "bot_grapple", "0", 0);
    Cvar_Register(&bot_fastchat, "bot_fastchat", "0", 0);
    Cvar_Register(&bot_nochat, "bot_nochat", "0", 0);
    Cvar_Register(&bot_testrchat, "bot_testrchat", "0", 0);
    Cvar_Register(&bot_challenge, "bot_challenge", "0", 0);
    Cvar_Register(&bot_predictobstacles, "bot_predictobstacles", "1", 0);
    Cvar_Register(&g_spSkill, "g_spSkill", "2", 0);
    //
    if (gametype == GT_CTF) {
