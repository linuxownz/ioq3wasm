...
static cvarTable_t gameCvarTable[] = {
    ...
    { NULL,                 "gamedate",             PRODUCT_DATE,   CVAR_ROM,                                       0, qfalse, qfalse },
    ...
    { &g_maxclients,        "sv_maxclients",        "32",           CVAR_SERVERINFO | CVAR_LATCH | CVAR_ARCHIVE,    0, qfalse, qfalse },
    ...
    { &g_damagePlums,       "g_damagePlums",          "0",          CVAR_ARCHIVE,  0, qfalse, qfalse }, // set back to 1 when finished debugging
    { &g_itemDrop,          "g_itemDrop",             "7",          CVAR_ARCHIVE , 0, qtrue, qfalse },
    ...
    { &g_motd,              "g_motd",               "",             CVAR_SERVERINFO, 0, qfalse, qfalse },
    ...
    { &g_unlagged,          "g_unlagged",           "1", CVAR_SERVERINFO | CVAR_ARCHIVE, 0, qfalse, qfalse },
    { &g_predictPVS,        "g_predictPVS",         "1", CVAR_SERVERINFO | CVAR_ARCHIVE, 0, qfalse, qfalse },
...
#endif
    { &g_smoothClients,       "g_smoothClients",    "0", 0, 0, qfalse, qfalse}, // was 1 set to 0 to debug CM_ClipHandleToModel: bad handle 7 < 7 < 256 ERRORS
...
static void G_RemapTeamShaders( void ) {
#ifdef MISSIONPACK
    char string[1024];
...
============
G_InitGame
#0  G_InitGame (levelTime=0, randomSeed=436, restart=0) at code/game/g_main.c:362
#1  0x0000000000457759 in SV_InitGameVM (restart=qfalse) at code/server/sv_game.c:904
#2  0x00000000004577c5 in SV_InitGameProgs () at code/server/sv_game.c:942
#3  0x0000000000458515 in SV_SpawnServer (server=0x7fffffffce50 "pro-q3dm13", killBots=qfalse) at code/server/sv_init.c:479
#4  0x000000000044fddd in SV_Map_f () at code/server/sv_ccmds.c:211
#5  0x000000000045e531 in Cmd_ExecuteString (text=0x7fffffffcf30 "map pro-q3dm13 ") at code/qcommon/cmd_server.c:795
#6  0x000000000045db45 in Cbuf_Execute () at code/qcommon/cmd_server.c:260
#7  0x00000000004748b2 in Com_Frame () at code/qcommon/common_server.c:2745
#8  0x000000000040240f in main (argc=8, argv=0x7fffffffd8f8) at code/sys/sys_main_server.c:469
============
*/
void G_InitGame( int levelTime, int randomSeed, int restart ) {
    int                 i;
    ...
    SV_LocateGameData( level.gentities, level.num_entities, sizeof( gentity_t ), &level.clients[0].ps, sizeof( level.clients[0] ) );
    ...
    SV_SetConfigstring( CS_INTERMISSION, "" );
}
...
void ExitLevel (void) {
    ...
    Cvar_VariableStringBuffer( "nextmap", nextmap, sizeof(nextmap) );
    Cvar_VariableStringBuffer( "d1", d1, sizeof(d1) );

    if( !Q_stricmp( nextmap, "map_restart 0" ) && Q_stricmp( d1, "" ) ) {
        Cvar_Set( "nextmap", "vstr d2" );
        Cbuf_ExecuteText( EXEC_APPEND, "vstr d1\n" );
    } else {
...
void G_LogPrintf( const char *fmt, ... ) {
    ...
    if ( 1 ) { // g_dedicated.integer ) { // TODO
        Com_Printf( "%s", string + 7 );
    }

    // TODO move up.  why checking now?  do at top
    if ( !level.logFile ) {
        return;
    }

    // FS_Write( string, strlen( string ), level.logFile ); // only Com_Printf -> log file
}
...
static void LogExit( const char *string ) {
    ...
    if (g_singlePlayer.integer) {
        ...
        Cbuf_ExecuteText( EXEC_APPEND, (won) ? "spWin\n" : "spLose\n" );
    }
#endif
}
...
void G_RunThink (gentity_t *ent) {
    ...
    ent->nextthink = 0;
    if (!ent->think) {
        Com_Error (ERR_DROP, "%s", "NULL ent->think");
    }
    ent->think (ent);
}
