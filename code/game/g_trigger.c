...
void SP_trigger_multiple( gentity_t *ent ) {
    ...
    if ( ent->random >= ent->wait && ent->wait >= 0 ) {
        ...
        Com_Printf( "%s", "trigger_multiple has random >= wait\n" );
    }
...
void trigger_teleporter_touch (gentity_t *self, gentity_t *other, trace_t *trace ) {
    ...
    // Spectators only?
    if ( ( self->spawnflags & 1 ) && !is_spectator(other->client ) /*other->client->sess.sessionTeam != TEAM_SPECTATOR*/ ) {
        return;
    }
    ...
    if (!dest) {
        Com_Printf ("%s", "Couldn't find teleporter destination\n");
        return;
    }
...
static void hurt_use( gentity_t *self, gentity_t *other, gentity_t *activator ) {
    ...
    UNUSED(other);
    if ( self->r.linked ) {
        SV_UnlinkEntity( (sharedEntity_t *)self );
    } else {
        SV_LinkEntity( self );
    }
}
