...
#endif
void SP_item_botroam( gentity_t *ent ) { UNUSED(ent); }
...
static qboolean G_CallSpawn( gentity_t *ent ) {
    ...
    if ( !ent->classname ) {
        Com_Printf ("%s", "G_CallSpawn: NULL classname\n");
        return qfalse;
    }
...
static char *G_AddSpawnVarToken( const char *string ) {
    ...
    if ( level.numSpawnVarChars + l + 1 > MAX_SPAWN_VARS_CHARS ) {
        Com_Error( ERR_DROP, "G_AddSpawnVarToken: MAX_SPAWN_VARS_CHARS" );
    }
