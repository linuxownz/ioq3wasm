...
float trap_Cvar_VariableValue( const char *var_name ) {
    ...
    Cvar_VariableStringBuffer(var_name, buf, sizeof(buf));
    return atof(buf);
}
#endif
...
static void G_LoadArenasFromFile( char *filename ) {
    ...
    if ( !f ) {
        Com_Printf( "%s", va( S_COLOR_RED "file not found: %s\n", filename ) );
        return;
    }
    if ( len >= MAX_ARENAS_TEXT ) {
        FS_FCloseFile( f );
        Com_Printf( "%s", va( S_COLOR_RED "file too large: %s is %i, max allowed is %i\n", filename, len, MAX_ARENAS_TEXT ) );
        return;
    }
...
static void G_LoadArenas( void ) {
    ...
    Com_Printf( "%s", va( "%i arenas parsed\n", g_numArenas ) );
...
static void PlayerIntroSound( const char *modelAndSkin ) {
    ...
    Cbuf_ExecuteText( EXEC_APPEND, va( "play sound/player/announce/%s.ogg\n", skin ) );
}
...
static void G_AddRandomBot( int team ) {
    ...
    Cbuf_ExecuteText( EXEC_INSERT, va("addbot random %f %s %i\n", skill, teamstr, 0) );
}
...
static int G_RemoveRandomBot( int team ) {
    ...
    for ( i=0 ; i< g_maxclients.integer ; i++ ) {
        ...
        Cbuf_ExecuteText( EXEC_INSERT, va("clientkick %d\n", i) );
        return qtrue;
    }
    return qfalse;
}
...
static void G_CheckMinimumPlayers( void ) {
    ...
    static int checkminimumplayers_time = 2000;
    ...
    if (checkminimumplayers_time > level.time ) {
        if ( checkminimumplayers_time - level.time > 20000 ) { // something is fucking checkminimumplayers_time
            checkminimumplayers_time = level.time + 10000;
        }
        return;
    }
    checkminimumplayers_time = level.time + 10000;
    //Com_Printf("checking to spawn bots..\n");
...
void G_CheckBotSpawn( void ) {
    ...
    for( n = 0; n < BOT_SPAWN_QUEUE_DEPTH; n++ ) {
        ...
        if( g_gametype.integer == GT_SINGLE_PLAYER ) {
            SV_GetUserinfo( botSpawnQueue[n].clientNum, userinfo, sizeof(userinfo) );
...
static void AddBotToSpawnQueue( int clientNum, int delay ) {
    ...
    Com_Printf( "%s", S_COLOR_YELLOW "Unable to delay spawn\n" );
...
static void G_AddBot( const char *name, float skill, const char *team, int delay, char *altname) {
    ...
    if ( clientNum == -1 ) {
        Com_Printf( "%s", S_COLOR_RED "Unable to add bot. All player slots are in use.\n" );
        Com_Printf( "%s", S_COLOR_RED "Start server with more 'open' slots (or check setting of sv_maxclients cvar).\n" );
        return;
    }
    ...
    if ( Q_stricmp( name, "random" ) == 0 ) {
        ...
        if ( botinfoNum < 0 ) {
            Com_Printf( "%s", S_COLOR_RED "Error: Cannot add random bot, no bot info available.\n" );
    ...
    if (!*s ) {
        Com_Printf( S_COLOR_RED "Error: bot has no aifile specified\n" );
        SV_BotFreeClient( clientNum );
        return;
    }
...
void Svcmd_AddBot_f( void ) {
    ...
    if ( !name[0] ) {
        Com_Printf( "%s", "Usage: Addbot <botname> [skill 1-5] [team] [msec delay] [altname]\n" );
        return;
    }
    ...
    if ( level.time - level.startTime > 1000 &&
        Cvar_VariableIntegerValue( "cl_running" ) ) {
        SV_GameSendServerCommand( -1, "loaddefered\n" );  // FIXME: spelled wrong, but not changing for demo
    }
}
...
void Svcmd_BotList_f( void ) {
    ...
    Com_Printf("%s", "^1name             model            aifile              funname\n");
    for (i = 0; i < g_numBots; i++) {
        ...
        Com_Printf("%s", va("%-16s %-16s %-20s %-20s\n", name, model, aifile, funname));
    }
}
...
char *G_GetBotInfoByNumber( int num ) {
    if( num < 0 || num >= g_numBots ) {
        Com_Printf( "%s", va( S_COLOR_RED "Invalid bot number: %i\n", num ) );
        return NULL;
    }
