...
static void Body_WorldEffects( gentity_t *self ) {
	...
	contents = SV_PointContents( point, -1 );
	...
	num = /*trap_EntitiesInBox*/SV_AreaEntities( mins, maxs, touch, MAX_GENTITIES );
...
void player_freeze( gentity_t *self, gentity_t *attacker, int mod ) {
	...
	if ( self != attacker && g_gametype.integer == GT_CTF ) { //&& redflag && blueflag ) { // TODO this will cause problems...
		vec3_t	dist1, dist2;
...
void team_wins( int team ) {
	...
	SV_GameSendServerCommand( -1, va( "cp \"" S_COLOR_MAGENTA "%s " S_COLOR_WHITE "team scores!\n\"", teamstr ) );
	SV_GameSendServerCommand( -1, va( "print \"%s team scores!\n\"", teamstr ) );
...
void Cmd_Drop_freeze_f( gentity_t *ent ) {
	char	*name;
	gitem_t	*it;
	gentity_t	*drop;
	int	quantity;
...
void Cmd_Ready_f( gentity_t *ent ) {
	...
	SV_GameSendServerCommand( ent - g_entities, "print \"ready\n\"" );
}
...
void voteInvalid( gentity_t *ent ) {
	...
	msg[ strlen( msg ) - 1 ] = '\0';
	strcat( msg, ".\n\"" );
	SV_GameSendServerCommand( ent - g_entities, msg );
}
