...
static int BotGPSToPosition(char *buf, vec3_t position) {
    int i, j = 0;
    int num, sign;
...
static void BotMatch_WhereAreYou(bot_state_t *bs, bot_match_t *match) {
    ...
    if (bestitem != -1) {
        if (gametype == GT_CTF
            ...
            redtt = AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, ctf_redflag.areanum, TFL_DEFAULT);
            bluett = AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, ctf_blueflag.areanum, TFL_DEFAULT);
            ...
            redtt = AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, redobelisk.areanum, TFL_DEFAULT);
            bluett = AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, blueobelisk.areanum, TFL_DEFAULT);
            if (redtt < (redtt + bluett) * 0.4) {
                BotAI_BotInitialChat(bs, "teamlocation", nearbyitems[bestitem], "red", NULL);
            }
            else if (bluett < (redtt + bluett) * 0.4) {
