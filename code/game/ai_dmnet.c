    ...
    if (bs->lastair_time < FloatTime() - 6) {
        ...
        if (BotGetAirGoal(bs, &goal)) {
            ...
            while(BotChooseNBGItem(bs->gs, bs->origin, bs->inventory, tfl, ltg, range)) {
                ...
                if (!(AAS_PointContents(goal.origin) & (CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA))) {
                    return qtrue;
                }
                BotPopGoal(bs->gs);
            }
...
static int BotNearbyGoal(bot_state_t *bs, int tfl, bot_goal_t *ltg, float range) {
    ...
    if (BotCTFCarryingFlag(bs)
        ...
        if (AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
...
static int BotReachedGoal(bot_state_t *bs, bot_goal_t *goal) {
    if (goal->flags & GFL_ITEM) {
        ...
        if (BotItemGoalInVisButNotVisible(bs->entitynum, bs->eye, bs->viewangles, goal)) {
            ...
            if (avoidtime > 0) {
                t = AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, goal->areanum, bs->tfl);
    ...
    if (bs->ltgtype == LTG_TEAMACCOMPANY && !retreat) {
        ...
        if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, bs->teammate)) {
            ...
            if (VectorLengthSquared(dir) < Square(bs->formation_dist)) {
                ...
                if (AAS_Swimming(bs->origin)) bs->attackcrouch_time = FloatTime() - 1;
