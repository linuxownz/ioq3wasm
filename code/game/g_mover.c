...
static gentity_t   *G_TestEntityPosition( gentity_t *ent ) {
    ...
    if ( ent->client ) {
        SV_Trace( &tr, ent->client->ps.origin, ent->r.mins, ent->r.maxs, ent->client->ps.origin, ent->s.number, mask, qfalse );
    } else {
        SV_Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, ent->s.pos.trBase, ent->s.number, mask, qfalse );
    }
...
qboolean G_CheckProxMinePosition( gentity_t *check ) {
    ...
    SV_Trace( &tr, start, NULL, NULL, end, check->s.number, MASK_SOLID, qfalse );
...
static qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
    ...
    *obstacle = NULL;
    ...
    SV_UnlinkEntity( (sharedEntity_t *)pusher );
...
static void Reached_BinaryMover( gentity_t *ent ) {
    ...
    if ( ent->moverState == MOVER_1TO2 ) {
        ...
        if ( ent->teammaster == ent || !ent->teammaster ) {
            SV_AdjustAreaPortalState( (sharedEntity_t *)ent, qfalse );
        }
    } else {
...
//freeze
    if ( other->client && is_spectator(other->client) /*other->client->sess.sessionTeam == TEAM_SPECTATOR*/ ) {
//freeze
...
void SP_func_door (gentity_t *ent) {
    ...
    SV_SetBrushModel( (sharedEntity_t *)ent, ent->model );
...
void SP_func_train (gentity_t *self) {
    ...
    SV_SetBrushModel( (sharedEntity_t *)self, self->model );
    InitMover( self );
