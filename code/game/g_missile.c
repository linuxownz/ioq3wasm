...
void G_RunMissile( gentity_t *ent ) {
    ...
    SV_Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, ent->clipmask, qfalse );

    if ( tr.startsolid || tr.allsolid ) {
        // make sure the tr.entityNum is set to the entity we're stuck in
        SV_Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, passent, ent->clipmask, qfalse );
        tr.fraction = 0;
    }
    else {
    ...
    if (ent->s.weapon == WP_PROX_LAUNCHER && !ent->count) {
        ...
        SV_Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, ENTITYNUM_NONE, ent->clipmask, qfalse );
...
gentity_t *fire_grapple (gentity_t *self, vec3_t start, vec3_t dir) {
    ...
    // unlagged comment out. hook->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;     // move a bit on the very first frame
    // unlagged comment out. hook->s.otherEntityNum = self->s.number; // use to match beam in client
