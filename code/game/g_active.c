...
void    G_TouchTriggers( gentity_t *ent ) {
    ...
    for ( i=0 ; i<num ; i++ ) {
        ...
        if ( hit->s.eType == ET_ITEM ) {
            ...
            if ( !SV_EntityContact( mins, maxs, (sharedEntity_t *)hit, qfalse ) ) { // TODO is this right?
                continue;
            }
        }
...
//      G_Printf("serverTime >>>>>\n" );
    ...
    // unlagged
	client->frameOffset = Sys_Milliseconds() - level.frameStartTime;
...
//freeze
    ...
    if (qfalse && g_smoothClients.integer) { // unlagged
...
void ClientEndFrame( gentity_t *ent ) {
    ...
    if ( g_predictPVS.integer && svf_self_portal2 ) {
        ...
        int lag = level.time - client->ps.commandTime + 50;
        if ( lag > 500 ) {
            lag = 500;
        }
