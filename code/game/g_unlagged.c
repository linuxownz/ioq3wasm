...
static qboolean G_PredictPlayerSlideMove( gentity_t *ent, float frametime ) {
    ...
    for ( bumpcount = 0; bumpcount < numbumps; bumpcount++ ) {
        ...
        CM_BoxTrace( &trace, origin, ent->r.mins, ent->r.maxs, end, ent->s.number, ent->clipmask, qfalse );
...
static void G_PredictPlayerStepSlideMove( gentity_t *ent, float frametime ) {
    ...
    CM_BoxTrace( &trace, start_o, ent->r.mins, ent->r.maxs, up, ent->s.number, ent->clipmask, qfalse );
    if ( trace.allsolid ) {
    ...
    down[2] -= stepSize;
    CM_BoxTrace( &trace, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, down, ent->s.number, ent->clipmask, qfalse );
    if ( !trace.allsolid ) {
        VectorCopy( trace.endpos, ent->s.pos.trBase );
    }
