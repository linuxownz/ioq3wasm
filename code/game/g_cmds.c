...
static qboolean    CheatsOk( gentity_t *ent ) {
    if ( !g_cheats.integer ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"Cheats are not enabled on this server.\n\"");
        return qfalse;
    }
    if ( ent->health <= 0 ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"You must be alive to use this command.\n\"");
        return qfalse;
    }
    return qtrue;
}
...
==================
*/
static qboolean StringIsInteger( const char * s ) {
    int         i;
    int         len;
    qboolean    foundDigit;
...
==================
*/
static int ClientNumberFromString( gentity_t *to, char *s, qboolean checkNums, qboolean checkNames ) {
    gclient_t   *cl;
    int         idnum;
...
static void Cmd_Team_f( gentity_t *ent ) {
    ...
    if ( Cmd_Argc() != 2 ) {
        ...
        switch ( oldTeam ) {
        case TEAM_BLUE:
            SV_GameSendServerCommand( ent-g_entities, "print \"Blue team\n\"" );
            break;
        case TEAM_RED:
            SV_GameSendServerCommand( ent-g_entities, "print \"Red team\n\"" );
            break;
        case TEAM_FREE:
            SV_GameSendServerCommand( ent-g_entities, "print \"Free team\n\"" );
            break;
        case TEAM_SPECTATOR:
            SV_GameSendServerCommand( ent-g_entities, "print \"Spectator team\n\"" );
            break;
        }
        return;
    }
    ...
    if ( ent->client->switchTeamTime > level.time ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"May not switch teams more than once per 5 seconds.\n\"" );
        return;
    }
...
static void Cmd_Tell_f( gentity_t *ent ) {
    ...
    if ( Cmd_Argc () < 3 ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"Usage: tell <player id> <message>\n\"" );
        return;
    }
...
static void G_VoiceTo( gentity_t *ent, gentity_t *other, int mode, const char *id, qboolean voiceonly ) {
    ...
    SV_GameSendServerCommand( other-g_entities, va("%s %d %d %d %s", cmd, voiceonly, ent->s.number, color, id));
}
...
static void Cmd_VoiceTell_f( gentity_t *ent, qboolean voiceonly ) {
    ...
    if ( Cmd_Argc () < 3 ) {
        SV_GameSendServerCommand( ent-g_entities, va( "print \"Usage: %s <player id> <voice id>\n\"", voiceonly ? "votell" : "vtell" ) );
        return;
    }
...
static void Cmd_GameCommand_f( gentity_t *ent ) {
    ...
    if ( Cmd_Argc() != 3 ) {
        SV_GameSendServerCommand( ent-g_entities, va( "print \"Usage: gc <player id> <order 0-%d>\n\"", numgc_orders - 1 ) );
        return;
    }
    ...
    if ( order < 0 || order >= numgc_orders ) {
        SV_GameSendServerCommand( ent-g_entities, va("print \"Bad order: %i\n\"", order));
        return;
    }
...
static void Cmd_CallVote_f( gentity_t *ent ) {
    ...
    if ( !g_allowVote.integer ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"Voting not allowed here.\n\"" );
        return;
    }

    if ( level.voteTime ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"A vote is already in progress.\n\"" );
        return;
    }
    if ( ent->client->pers.voteCount >= MAX_VOTE_COUNT ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"You have called the maximum number of votes.\n\"" );
        return;
    }
    if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"Not allowed to call a vote as spectator.\n\"" );
        return;
    }
    ...
    if ( level.voteExecuteTime ) {
        ...
        if ( !Q_stricmpn( level.voteString, "map", 3 )
            ...
            SV_GameSendServerCommand( ent-g_entities, "print \"Vote after map change.\n\"" );
            return;
        }
    ...
    if ( !Q_stricmp( arg1, "g_gametype" ) ) {
        ...
        if( i == GT_SINGLE_PLAYER || i < GT_FFA || i >= GT_MAX_GAME_TYPE) {
            SV_GameSendServerCommand( ent-g_entities, "print \"Invalid gametype.\n\"" );
            return;
        }
        ...
        Cvar_VariableStringBuffer( "nextmap", s, sizeof(s) );
        if (!*s) {
            SV_GameSendServerCommand( ent-g_entities, "print \"nextmap not set.\n\"" );
            return;
        }
        ...
        if ( level.clients[i].pers.localClient ) {
            SV_GameSendServerCommand( ent - g_entities, "print \"Cannot kick host player.\n\"" );
            return;
        }
        ...
        Cbuf_ExecuteText( EXEC_APPEND, "addbot sarge 1" );
    } else {
        Com_sprintf( level.voteString, sizeof( level.voteString ), "%s \"%s\"", arg1, arg2 );
        Com_sprintf( level.voteDisplayString, sizeof( level.voteDisplayString ), "%s", level.voteString );
    }
...
static void Cmd_Vote_f( gentity_t *ent ) {
    ...
    if ( !level.voteTime ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"No vote in progress.\n\"" );
        return;
    }
    if ( ent->client->ps.eFlags & EF_VOTED ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"Vote already cast.\n\"" );
        return;
    }
    if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"Not allowed to vote as spectator.\n\"" );
        return;
    }

    SV_GameSendServerCommand( ent-g_entities, "print \"Vote cast.\n\"" );
...
static void Cmd_CallTeamVote_f( gentity_t *ent ) {
    ...
    if ( level.teamVoteTime[cs_offset] ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"A team vote is already in progress.\n\"" );
        return;
    }
    if ( ent->client->pers.teamVoteCount >= MAX_VOTE_COUNT ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"You have called the maximum number of team votes.\n\"" );
        return;
    }
    if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"Not allowed to call a vote as spectator.\n\"" );
    ...
    if ( !Q_stricmp( arg1, "leader" ) ) {
        ...
        if ( !arg2[0] ) {
            ...
            if ( i >= 3 || !arg2[i]) {
                ...
                if ( i < 0 || i >= level.maxclients ) {
                    SV_GameSendServerCommand( ent-g_entities, va("print \"Bad client slot: %i\n\"", i) );
                    return;
                }

                if ( !g_entities[i].inuse ) {
                    SV_GameSendServerCommand( ent-g_entities, va("print \"Client %i is not active\n\"", i) );
                    return;
                }
            }
            else {
                ...
                if ( i >= level.maxclients ) {
                    SV_GameSendServerCommand( ent-g_entities, va("print \"%s is not a valid player on your team.\n\"", arg2) );
                    return;
                }
            }
        }
        ...
        SV_GameSendServerCommand( ent-g_entities, "print \"Invalid vote string.\n\"" );
        SV_GameSendServerCommand( ent-g_entities, "print \"Team vote commands are: leader <player>.\n\"" );
        return;
    }
    ...
    SV_SetConfigstring( CS_TEAMVOTE_TIME + cs_offset, va("%i", level.teamVoteTime[cs_offset] ) );
    SV_SetConfigstring( CS_TEAMVOTE_STRING + cs_offset, level.teamVoteString[cs_offset] );
    SV_SetConfigstring( CS_TEAMVOTE_YES + cs_offset, va("%i", level.teamVoteYes[cs_offset] ) );
    SV_SetConfigstring( CS_TEAMVOTE_NO + cs_offset, va("%i", level.teamVoteNo[cs_offset] ) );
}
...
static void Cmd_TeamVote_f( gentity_t *ent ) {
    ...
    if ( !level.teamVoteTime[cs_offset] ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"No team vote in progress.\n\"" );
        return;
    }
    if ( ent->client->ps.eFlags & EF_TEAMVOTED ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"Team vote already cast.\n\"" );
        return;
    }
    if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"Not allowed to vote as spectator.\n\"" );
    ...
    SV_GameSendServerCommand( ent-g_entities, "print \"Team vote cast.\n\"" );
...
static void Cmd_SetViewpos_f( gentity_t *ent ) {
    ...
    if ( Cmd_Argc() != 5 ) {
        SV_GameSendServerCommand( ent-g_entities, "print \"usage: setviewpos x y z yaw\n\"");
        return;
    }
...
=================
*/
static void Cmd_Stats_f( gentity_t *ent ) { // TODO this is NOT player stats
    UNUSED(ent);
/*
    int max, n, i;
    ...
    //SV_GameSendServerCommand( ent-g_entities, va("print \"visited %d of %d areas\n\"", n, max));
    SV_GameSendServerCommand( ent-g_entities, va("print \"%d%% level coverage\n\"", n * 100 / max));
*/
}
...
gentity_t *DropWeapon( gentity_t *ent ) {
    ...
    for ( int i = 15 ; i >= 0; i-- ) {
        if ( bits & ( 1 << i ) && ent->client->ps.ammo[i] ) {
