...
static void Bullet_Fire (gentity_t *ent, float spread, int damage, int mod ) {
    ...
    for (i = 0; i < 10; i++) {
        ...
        SV_Trace (&tr, muzzle, NULL, NULL, end, passent, MASK_SHOT, qfalse);
...
qboolean ShotgunPellet( vec3_t start, vec3_t end, gentity_t *ent ) {
    ...
    for (i = 0; i < 10; i++) {
        SV_Trace (&tr, tr_start, NULL, NULL, tr_end, passent, MASK_SHOT, qfalse);
        traceEnt = &g_entities[ tr.entityNum ];
...
static void weapon_railgun_fire (gentity_t *ent) {
    ...
    passent = ent->s.number;
    do {
        SV_Trace (&trace, muzzle, NULL, NULL, end, passent, MASK_SHOT, qfalse);
        ...
        SV_UnlinkEntity( (sharedEntity_t *)traceEnt );
...
static void CalcMuzzlePointOrigin ( gentity_t *ent, vec3_t origin, vec3_t forward, vec3_t right, vec3_t up, vec3_t muzzlePoint ) {
    UNUSED(origin); UNUSED(right); UNUSED(up);
