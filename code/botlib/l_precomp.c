...
int PC_StringizeTokens(token_t *tokens, token_t *token)
{
    ...
    {
        strncat(token->string, t->string, MAX_TOKEN - strlen(token->string) ); // removed -1 fixes  l_precomp.c:375:9:
        // error: ‘strncat’ output may be truncated copying between 0 and 1023 bytes from a string of length 1023 [-Werror=stringop-truncation]
    }
                        ...
                        case P_SUB:
                            {
                                ...
                                __attribute__ ((fallthrough)); // TODO code/botlib/l_precomp.c:1677:36: error: this statement may fall through [-Werror=implicit-fallthrough=]
                            }
