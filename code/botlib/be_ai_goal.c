...
{
    {"name",        ITEMINFO_OFS(name),        FT_STRING,         0, 0.0, 0.0, NULL},
    {"model",       ITEMINFO_OFS(model),       FT_STRING,         0, 0.0, 0.0, NULL},
    {"modelindex",  ITEMINFO_OFS(modelindex),  FT_INT,            0, 0.0, 0.0, NULL},
    {"type",        ITEMINFO_OFS(type),        FT_INT,            0, 0.0, 0.0, NULL},
    {"index",       ITEMINFO_OFS(index),       FT_INT,            0, 0.0, 0.0, NULL},
    {"respawntime", ITEMINFO_OFS(respawntime), FT_FLOAT,          0, 0.0, 0.0, NULL},
    {"mins",        ITEMINFO_OFS(mins),        FT_FLOAT|FT_ARRAY, 3, 0.0, 0.0, NULL},
    {"maxs",        ITEMINFO_OFS(maxs),        FT_FLOAT|FT_ARRAY, 3, 0.0, 0.0, NULL},
...
{
    UNUSED(goalstate); UNUSED(filename);
    //bot_goalstate_t *gs;
...
static itemconfig_t *LoadItemConfig(char *filename)
{
    ...
    FreeSource(source);
    //
    if (!ic->numiteminfo) BotImport_Print(PRT_WARNING, "no item info loaded\n");
...
// Changes Globals:     -
//===========================================================================
static void BotAddToAvoidGoals(bot_goalstate_t *gs, int number, float avoidtime) // this is static but removefromavoidgoals is used elsewhere???? is this called?
{
    int i;
