...
static int botlibsetup;                        //true when the bot library has been setup
static int maxentities;                        //maximum number of entities
static int maxclients;                         //maximum number of clients
...
int Export_BotLibSetup(void)
{
    ...
    maxclients  = (int) LibVarValue("maxclients", "128");
...
//  AAS_ClearShownDebugLines();
    ...
    BotImport_Print(PRT_MESSAGE, "\rtravel time to goal (%d) = %d  ", goalareanum,
        AAS_AreaTravelTimeToGoalArea(newarea, origin, goalareanum, TFL_DEFAULT));
    ...
    if (newarea != area)
    {
        ...
        BotImport_Print(PRT_MESSAGE, "travel time to goal (%d) = %d\n", goalareanum,
                    AAS_AreaTravelTimeToGoalArea(newarea, origin, goalareanum, TFL_DEFAULT|TFL_ROCKETJUMP));
        /*
