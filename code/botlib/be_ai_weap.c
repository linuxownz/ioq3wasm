...
{
    {"number",          WEAPON_OFS(number),         FT_INT,                 0, 0.0, 0.0, NULL }, //weapon number
    {"name",            WEAPON_OFS(name),           FT_STRING,              0, 0.0, 0.0, NULL }, //name of the weapon
    {"level",           WEAPON_OFS(level),          FT_INT,                 0, 0.0, 0.0, NULL },
    {"model",           WEAPON_OFS(model),          FT_STRING,              0, 0.0, 0.0, NULL }, //model of the weapon
    {"weaponindex",     WEAPON_OFS(weaponindex),    FT_INT,                 0, 0.0, 0.0, NULL }, //index of weapon in inventory
    {"flags",           WEAPON_OFS(flags),          FT_INT,                 0, 0.0, 0.0, NULL }, //special flags
    {"projectile",      WEAPON_OFS(projectile),     FT_STRING,              0, 0.0, 0.0, NULL }, //projectile used by the weapon
    {"numprojectiles",  WEAPON_OFS(numprojectiles), FT_INT,                 0, 0.0, 0.0, NULL }, //number of projectiles
    {"hspread",         WEAPON_OFS(hspread),        FT_FLOAT,               0, 0.0, 0.0, NULL }, //horizontal spread of projectiles (degrees from middle)
    {"vspread",         WEAPON_OFS(vspread),        FT_FLOAT,               0, 0.0, 0.0, NULL }, //vertical spread of projectiles (degrees from middle)
    {"speed",           WEAPON_OFS(speed),          FT_FLOAT,               0, 0.0, 0.0, NULL }, //speed of the projectile (0 = instant hit)
    {"acceleration",    WEAPON_OFS(acceleration),   FT_FLOAT,               0, 0.0, 0.0, NULL }, //"acceleration" * time (in seconds) + "speed" = projectile speed
    {"recoil",          WEAPON_OFS(recoil),         FT_FLOAT|FT_ARRAY,      3, 0.0, 0.0, NULL }, //amount of recoil the player gets from the weapon
    {"offset",          WEAPON_OFS(offset),         FT_FLOAT|FT_ARRAY,      3, 0.0, 0.0, NULL }, //projectile start offset relative to eye and view angles
    {"angleoffset",     WEAPON_OFS(angleoffset),    FT_FLOAT|FT_ARRAY,      3, 0.0, 0.0, NULL }, //offset of the shoot angles relative to the view angles
    {"extrazvelocity",  WEAPON_OFS(extrazvelocity), FT_FLOAT,               0, 0.0, 0.0, NULL }, //extra z velocity the projectile gets
    {"ammoamount",      WEAPON_OFS(ammoamount),     FT_INT,                 0, 0.0, 0.0, NULL }, //ammo amount used per shot
    {"ammoindex",       WEAPON_OFS(ammoindex),      FT_INT,                 0, 0.0, 0.0, NULL }, //index of ammo in inventory
    {"activate",        WEAPON_OFS(activate),       FT_FLOAT,               0, 0.0, 0.0, NULL }, //time it takes to select the weapon
    {"reload",          WEAPON_OFS(reload),         FT_FLOAT,               0, 0.0, 0.0, NULL }, //time it takes to reload the weapon
    {"spinup",          WEAPON_OFS(spinup),         FT_FLOAT,               0, 0.0, 0.0, NULL }, //time it takes before first shot
    {"spindown",        WEAPON_OFS(spindown),       FT_FLOAT,               0, 0.0, 0.0, NULL }, //time it takes before weapon stops firing
...
{
    {"name",        PROJECTILE_OFS(name),       FT_STRING, 0, 0.0, 0.0, NULL }, //name of the projectile
    {"model",       PROJECTILE_OFS(model),      FT_STRING, 0, 0.0, 0.0, NULL }, //model of the projectile
    {"flags",       PROJECTILE_OFS(flags),      FT_INT,    0, 0.0, 0.0, NULL }, //special flags
    {"gravity",     PROJECTILE_OFS(gravity),    FT_FLOAT,  0, 0.0, 0.0, NULL }, //amount of gravity applied to the projectile [0,1]
    {"damage",      PROJECTILE_OFS(damage),     FT_INT,    0, 0.0, 0.0, NULL }, //damage of the projectile
    {"radius",      PROJECTILE_OFS(radius),     FT_FLOAT,  0, 0.0, 0.0, NULL }, //radius of damage
    {"visdamage",   PROJECTILE_OFS(visdamage),  FT_INT,    0, 0.0, 0.0, NULL }, //damage of the projectile to visible entities
    {"damagetype",  PROJECTILE_OFS(damagetype), FT_INT,    0, 0.0, 0.0, NULL }, //type of damage (combination of the DAMAGETYPE_? flags)
    {"healthinc",   PROJECTILE_OFS(healthinc),  FT_INT,    0, 0.0, 0.0, NULL }, //health increase the owner gets
    {"push",        PROJECTILE_OFS(push),       FT_FLOAT,  0, 0.0, 0.0, NULL }, //amount a player is pushed away from the projectile impact
    {"detonation",  PROJECTILE_OFS(detonation), FT_FLOAT,  0, 0.0, 0.0, NULL }, //time before projectile explodes after fire pressed
    {"bounce",      PROJECTILE_OFS(bounce),     FT_FLOAT,  0, 0.0, 0.0, NULL }, //amount the projectile bounces
    {"bouncefric",  PROJECTILE_OFS(bouncefric), FT_FLOAT,  0, 0.0, 0.0, NULL }, //amount the bounce decreases per bounce
    {"bouncestop",  PROJECTILE_OFS(bouncestop), FT_FLOAT,  0, 0.0, 0.0, NULL }, //minimum bounce value before bouncing stops recurive projectile definition??
    {NULL,          0,                          0,         0, 0.0, 0.0, NULL }
};
...
weaponconfig_t *LoadWeaponConfig(char *filename)
{
    ...
    if (!wc->numweapons) BotImport_Print(PRT_WARNING, "no weapon info loaded\n");
