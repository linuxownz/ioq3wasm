...
void toggle_console(short show); // defined in cl_emscripten.c
...
void Con_ToggleConsole_f (void) {
    ...
    //qboolean show = Key_GetCatcher( ) & KEYCATCH_CONSOLE;
    //toggle_console(show);
}
...
void Con_ToggleMenu_f( void ) {
    CL_KeyEvent( K_CONSOLE, qtrue, Sys_Milliseconds() );
    CL_KeyEvent( K_CONSOLE, qfalse, Sys_Milliseconds() );
...
//  qhandle_t       conShader;
    ...
    qtime_t now;
    Com_RealTime( &now );
    char *nowString = va( "%04d%02d%02d %02d:%02d:%02d", 1900 + now.tm_year, 1 + now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec );

    i = strlen( nowString );

    for ( x = 0 ; x < i ; x++ ) {
        SCR_DrawSmallChar( ( x + 1 ) * SMALLCHAR_WIDTH, lines - SMALLCHAR_HEIGHT, nowString[x] );
    }
