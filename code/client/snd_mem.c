...
static int ResampleSfx( sfx_t *sfx, int channels, int inrate, int inwidth, int samples, byte *data, qboolean compressed ) {
    ...
    for (i=0 ; i<outcount ; i++)
    {
        ...
        {
            if( inwidth == 2 ) {
                sample = ( ((short *)data)[srcsample+j] );// TODO possible alignment fault
            } else {
...
static int ResampleSfxRaw( short *sfx, int channels, int inrate, int inwidth, int samples, byte *data ) {
    ...
    for (i=0 ; i<outcount ; i++)
    {
        ...
        {
            if( inwidth == 2 ) {
                sample = LittleShort ( ((short *)data)[srcsample+j] );// TODO possible alignment fault
            } else {
...
assert(0);
    ...
    // load it in
    data = NULL; //S_CodecLoad(sfx->soundName, &info);
    if(!data)
        return qfalse;
