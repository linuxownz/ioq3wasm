...
*/
static
const char* S_AL_FormatToString(int width, int channels)
{
    // Work out format
    if(width == 1) {
        if(channels == 1)
...
*/
static
const char *S_AL_ErrorMsg(ALenum error)
{
    switch(error)
    {
        case AL_NO_ERROR:
...
typedef struct alSfx_s
{
    ...
    sfxHandle_t pos;                   // its own position in the alSfx_t struct array knownSfx[MAX_SFX]
...
{
    assert(sfx >= 0 && sfx < MAX_SFX);
    alSfx_t *curSfx = &knownSfx[sfx];

    // Nothing?
    if(curSfx->filename[0] == '\0') {
        Com_Printf("%s curSfx->filename[0] is null\n", __func__);
        return;
    }

    // Already done?
    if((curSfx->inMemory) || (curSfx->isDefault) || (curSfx->isDefaultChecked)) {
        Com_Printf("%s curSfx '%s' already in memory\n", __func__, curSfx->filename);
        return;
    }

    snd_info_t info;
    void *buffer = S_CodecLoad(curSfx->filename, &info); // client/snd_codec.c : 47
    ...
    curSfx->info.channels = info.channels;
    curSfx->info.dataofs  = info.dataofs;
    curSfx->info.rate     = info.rate;
    curSfx->info.samples  = info.samples;
    curSfx->info.size     = info.size;
    curSfx->info.width    = info.width;

    ALenum error;
    ALuint format;

    //Com_Printf("Codec loaded: info: channels:%d, rate:%d, samples:%d, size:%d\n", info.channels, info.rate, info.samples, info.size );
    ...
    {
        Com_Error(ERR_FATAL, "%s S_CodecLoad no genbuffers\n", __func__); // TODO remove me
        S_AL_BufferUseDefault(sfx);
        return;
    }

    format = S_AL_Format(info.width, info.channels);
    //Com_Printf("S_AL_Format(info.width:%d, info.channels:%d) = %s\n", info.width, info.channels, S_AL_FormatToString(info.width, info.channels));

    // Fill the buffer
    if( info.size == 0 )
    {
        Com_Error(ERR_FATAL, "%s S_CodecLoad size == 0\n", __func__); // TODO remove me
        // We have no data to buffer, so buffer silence
        byte dummyData[ 2 ] = { 0 };
        qalBufferData(curSfx->buffer, AL_FORMAT_MONO16, (void *)dummyData, 2, 22050);
    } else {
        // TODO error here but still maybe ok.  openal example had errors and it ran fine...
        // the size parameter is not valid for the format specified, the buffer is in use, or the data is a NULL pointer
        // void alBufferData( ALuint buffer, ALenum format, const ALvoid *data, ALsizei size, ALsizei freq)
        qalBufferData(curSfx->buffer, format, buffer, info.size, info.rate);
    }
    ...
    if ( AL_NO_ERROR != error ) {
        Com_Printf("OpenAL error: is pointer NULL? %p\n", buffer);
        Com_Printf("is this the right size? %d\n", info.size);
        Com_Printf("is the rate correct? %d\n", info.rate);
        Com_Printf("is the buffer in use?\n");
        //Com_Error(ERR_FATAL, "%s S_CodecLoad OpenAL error %s\n", __func__, S_AL_ErrorMsg(error)); // TODO remove me
    }
#endif
...
// test // havne't proven it works but it seems to successfully initialize .... TODO
// TODO
#if 1
    ...
    curSfx->lastUsedTime = Com_Milliseconds();
...
qboolean S_AL_BufferInit( void )
{
    ...
    for ( int i = 0 ; i < MAX_SFX ; i++ ) {  // TODO what is this?
        knownSfx[i].pos = i;
    }

    // printf("S_AL_BufferInit: Skipping loading sound/feedback/hit.wav\n");
...
=================
*/
sfxHandle_t S_AL_RegisterSound( const char *sample ) {
...
#if USE_OGG
    if ( strstr(sample, ".wav") ) {
        Com_Printf("%s Warning wave file when USE_OGG in effect %s\n", __func__, sample);
    }
#else
    if ( strstr(sample, ".ogg") ) {
        //Com_Printf("%s Warning OGG file when USE_OGG is not in effect %s\n", __func__, sample);
    }
#endif
...
=================
S_AL_ScaleGain
Adapt the gain if necessary to get a
quicker fadeout when the source is too far away.
=================
*/
...
qboolean S_AL_SrcInit( void )
{
    ...
    for(i = 0; i < limit; i++)
    {
        ...
        if( res != AL_NO_ERROR) {
            Com_Error(ERR_FATAL, "%s", S_AL_ErrorMsg(res));
            break;
        }
        srcCount++;
    }
...
static void S_AL_SrcSetup(srcHandle_t src, sfxHandle_t sfx, alSrcPriority_t priority, int entity, int channel, qboolean local)
{
    ...
    if(sfx >= 0)
    {
        ...
        S_AL_BufferUse(sfx);
        qalSourcei(curSource->alSource, AL_BUFFER, S_AL_BufferGet(sfx)); // TODO
    }
...
void S_AL_MusicProcess(ALuint b)
{
    ...
    l = 0; //S_CodecReadStream(curstream, MUSIC_BUFFER_SIZE, decode_buffer);
    ...
    if(l == 0)
    {
        ...
        if(intro_stream) {
            ...
            mus_stream = 0; // S_CodecOpenStream(s_backgroundLoop);
        }
...
void S_AL_StartBackgroundTrack( const char *intro, const char *loop )
{
    ...
    if(!issame)
    {
        ...
        intro_stream = 0; //S_CodecOpenStream(intro);
    }
    else {
        intro_stream = NULL;
    }
...
void S_AL_StopAllSounds( void )
{
    ...
    for (int i = 0; i < MAX_RAW_STREAMS; i++) {
        S_AL_StreamDie(i);
    }
}
...
void S_AL_BeginRegistration( void )
{
    if(!numSfx) {
        ...
        Com_Printf("S_AL_BeginRegistration fail.  numSfx != 0");
        //assert(0);
    }
}
...
void S_AL_Shutdown( void )
{
    ...
    //QAL_Shutdown(); // TODO empty func to be removed
}
...
=================
S_AL_Init
THIS IS THE ONLY NON STATIC
FUNCTION IN THIS FILE
=================
*/
qboolean S_AL_Init( void )
{
    ...
    if(inputdevice && !*inputdevice) {
        ...
        Com_Printf("input device is NOT NULL???\n");
    }
    ...
    if ( enumeration_ext || enumeration_all_ext )
    {
        ...
        Com_Printf("\n\nAll available openAL device names: %s\n", s_alAvailableDevices->string);
    } else {
        Com_Printf("no audio device enumeration made..\n");
    }

    Com_Printf("Opening device %s\n", device);
    ...
    {
        qalcCloseDevice( alDevice );
        Com_Error( ERR_FATAL, "Failed to create OpenAL context.\n" );
        return qfalse;
    }
    ...
    S_AL_BufferInit( ); // skipped loading of hit.wav
    S_AL_SrcInit( );
