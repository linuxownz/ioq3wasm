...
==================
*/
field_t * Field_CharEvent( field_t *edit, int ch ) {
    int     len;
...
void Key_SetBinding( int keynum, const char *binding ) {
    ...
    if ( Key_GetCatcher() == 1 ) {
        char *bind = Key_KeynumToString(keynum);
        Com_Printf("%s %s %s\n", __func__, bind, binding);
        void binding_change(const char *a, const char *b);
...
{
    char buf[ MAX_STRING_CHARS ], *p = buf, *end; // TODO
...
void CL_KeyDownEvent( int key, unsigned time )
{
    ...
    if ( ( key < 128 || key == K_MOUSE1 ) && ( clc.demoplaying ) && Key_GetCatcher( ) == 0 ) { // TODO control playback of demo...
    ...
    if ( key == K_ESCAPE ) { // TODO special handling TODO
        Message_Key( key );
        //return; // say team stuff
        if ( clc.state == CA_ACTIVE && !clc.demoplaying ) {
            void toggle_escape_menu(void);
            toggle_escape_menu();
    ...
    if ( Key_GetCatcher( ) & KEYCATCH_CONSOLE ) {
        ...
        //CG_KeyEvent(key, qtrue); //vmMain( CG_KEY_EVENT, key, qtrue, 0 ); // MISSIONPAK ONLY
...
void CL_KeyUpEvent( int key, unsigned time )
{
    ...
    if ( Key_GetCatcher( ) & KEYCATCH_CONSOLE ) {
        // TODO // VM_Call( uivm, UI_KEY_EVENT, key, qfalse );
    } else if ( Key_GetCatcher( ) & KEYCATCH_CGAME ) {
        CG_KeyEvent(key, qfalse);// vmMain( CG_KEY_EVENT, key, qfalse, 0 ); // TODO VM_Call( cgvm, CG_KEY_EVENT, key, qfalse );
    }
}
...
void CL_CharEvent( int key ) {
    ...
    if ( Key_GetCatcher( ) & KEYCATCH_CONSOLE )
    {
        //Com_Printf("%s KEYCATCH_CONSOLE %c\n", __func__, key);
        //console_key(key); // TODO not sure this is the right place...
        field = Field_CharEvent( &g_consoleField, key );
    }
    else if ( Key_GetCatcher( ) & KEYCATCH_UI )
    {
        // TODO //VM_Call( uivm, UI_KEY_EVENT, key | K_CHAR_FLAG, qtrue );
    }
    else if ( Key_GetCatcher( ) & KEYCATCH_MESSAGE )
    {
    ...
    if ( field ) {
        // put input.value = field->

        void console_set ( char * str, short length, short cursorpos );
        console_set ( field->buffer, sizeof(field->buffer) - 1, field->cursor );
    }
}
...
================
*/
qboolean FS_GetIDBFileAsync ( const char *filename, void(*callback)(const char *filename, const void *data, const int length) ) ;
void CL_LoadConsoleHistoryCallback ( const char *filename, const void *ptr, const int length );
void CL_LoadConsoleHistory( void )
{

    qboolean loading = FS_GetIDBFileAsync( CONSOLE_HISTORY_FILE, CL_LoadConsoleHistoryCallback);
...
void CL_LoadConsoleHistoryCallback ( const char *filename, const void *ptr, const int length )  {
    ...
    consoleSaveBufferSize = length;

    strncpy ( consoleSaveBuffer, ptr, MAX_CONSOLE_SAVE_BUFFER - 1 );
    ...
    if( consoleSaveBufferSize < MAX_CONSOLE_SAVE_BUFFER ) {
        ...
        for( i = COMMAND_HISTORY - 1; i >= 0; i-- )
        {
            ...
            {
                Com_Printf( "WARNING: probable corrupt history\n" );
                break;
            }
...
void CL_SaveConsoleHistory( void )
{
    ...
    FS_SaveFile( CONSOLE_HISTORY_FILE, consoleSaveBuffer, consoleSaveBufferSize );
}
