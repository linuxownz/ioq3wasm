...
void CL_SystemInfoChanged( void ) {
    ...
    while ( s ) {
        ...
        {
            if(0) // TODO FS_InvalidGameDir(value)) redo this
            {
...
static void CL_ParseDownload ( msg_t *msg ) {
    ...
    Com_Printf ( "CL_ParseDownload TODO %s\b", clc.downloadTempName );
    ...
    if (!clc.download)
    {
        clc.download = 1; //FS_SV_FOpenFileWrite( clc.downloadTempName ); // TODO
    ...
    if (!size) { // A zero length block means EOF
        if (clc.download) {
            assert(0);
            //FS_FileClose( clc.download ); // TODO
            clc.download = 0;

            // rename the file
            // FS_SV_Rename ( clc.downloadTempName, clc.downloadName, qfalse ); // TODO
        }
...
void CL_ParseServerMessage( msg_t *msg ) {
    ...
    while ( 1 ) {
        if ( msg->readcount > msg->cursize ) {
            Com_Error (ERR_FATAL, "CL_ParseServerMessage: read past end of server message");
            break;
        }
