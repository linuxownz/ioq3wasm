...
void S_ByteSwapRawSamples( int samples, int width, int s_channels, const byte *data ) {
    ...
    for ( i = 0 ; i < samples ; i++ ) {
        ((short *)data)[i] = LittleShort( ((short *)data)[i] ); // TODO possible alignment fault
    }
}
...
void S_Base_RawSamples( int stream, int samples, int rate, int width, int s_channels, const byte *data, float volume, int entityNum)
{
    ...
    if (s_channels == 2 && width == 2)
    {
        ...
        {   // optimized case
            for (i=0 ; i<samples ; i++)
            {
                ...
                s_rawend[stream]++;
                rawsamples[dst].left  = ((short *)data)[i*2  ] * intVolumeLeft; // TODO possible alignment fault
                rawsamples[dst].right = ((short *)data)[i*2+1] * intVolumeRight;// TODO possible alignment fault
            }
        }
        else
        {
            for (i=0 ; ; i++)
            {
                ...
                dst = s_rawend[stream]&(MAX_RAW_SAMPLES-1);
                s_rawend[stream]++;
                rawsamples[dst].left  = ((short *)data)[src*2  ] * intVolumeLeft;// TODO possible alignment fault
                rawsamples[dst].right = ((short *)data)[src*2+1] * intVolumeRight;// TODO possible alignment fault
            }
        }
    }
    else if (s_channels == 1 && width == 2)
    {
        for (i=0 ; ; i++)
        {
            ...
            dst = s_rawend[stream]&(MAX_RAW_SAMPLES-1);
            s_rawend[stream]++;
            rawsamples[dst].left  = ((short *)data)[src] * intVolumeLeft;// TODO possible alignment fault
            rawsamples[dst].right = ((short *)data)[src] * intVolumeRight;// TODO possible alignment fault
        }
    }
