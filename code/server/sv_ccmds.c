...
static client_t *SV_GetPlayerByHandle( void ) {
    ...
    if ( Cmd_Argc() < 2 ) {
        Com_Printf( "%s", JSON("error", "No player specified") );
        return NULL;
    }
    ...
    Com_Printf( "%s", JSON("notice", va("Player %s is not on the server", s)));
...
static client_t *SV_GetPlayerByNum( void ) {
    ...
    if ( Cmd_Argc() < 2 ) {
        Com_Printf( "%s", JSON("warning", "No player specified" ));
        return NULL;
    }
    ...
    for (i = 0; s[i]; i++) {
        if (s[i] < '0' || s[i] > '9') {
            Com_Printf( "%s", JSON("error", va("Bad slot number: %s", s)));
            return NULL;
        }
    }
    idnum = atoi( s );
    if ( idnum < 0 || idnum >= sv_maxclients->integer ) {
        Com_Printf( "%s", JSON("error", va("Bad client slot: %i", idnum)));
        return NULL;
    }

    cl = &svs.clients[idnum];
    if ( !cl->state ) {
        Com_Printf( "%s", JSON("warning", va("Client %i is not active", idnum )));
        return NULL;
    }
...
static void SV_Map_f( void ) {
    ...
    if ( FS_ReadFile (expanded, NULL) == -1 ) {
        Com_Printf ( "%s", JSON("error", va("Can't find map %s", expanded)));
        return;
    }
...
static void SV_MapRestart_f( void ) {
    ...
    // make sure server is running
    if ( !com_sv_running->integer ) {
        Com_Printf( "%s", JSON("error", "Server is not running") );
        return;
    }
    ...
    if ( sv_maxclients->modified || sv_gametype->modified ) {
        ...
        Com_Printf( "%s", JSON("notice", "variable change -- restarting") );
    ...
    for (i=0 ; i<sv_maxclients->integer ; i++) {
        ...
        if ( denied ) {
            ...
            Com_Printf( "%s", JSON("notice", va("SV_MapRestart_f(%d): dropped client %i - denied!", delay, i)));
            continue;
        }
...
static void SV_Kick_f( void ) {
    ...
    if ( Cmd_Argc() != 2 ) {
        Com_Printf ("%s", JSON("notice", "Usage: kick <player name>, kick all = kick everyone, kick allbots = kick all bots") );
        return;
    }
    ...
    if( cl->netchan.remoteAddress.type == NA_LOOPBACK ) {
        Com_Printf("%s", JSON("notice", "Cannot kick host player"));
        return;
    }
...
static void SV_KickNum_f( void ) {
    ...
    if ( Cmd_Argc() != 2 ) {
        Com_Printf ("%s", JSON("notice", va("Usage: %s <client number>", Cmd_Argv(0))));
        return;
    }
...
static void SV_Ban_f( void ) {
    ...
    if ( Cmd_Argc() != 2 ) {
        Com_Printf ("%s", JSON("notice", "Usage: banUser <player name>"));
        return;
    }
    ...
    if ( !svs.authorizeAddress.ip[0] && svs.authorizeAddress.type != NA_BAD ) {
        Com_Printf( "%s", JSON("notice", va("Resolving %s", AUTHORIZE_SERVER_NAME )));
        if ( !NET_StringToAdr( AUTHORIZE_SERVER_NAME, &svs.authorizeAddress, NA_IP ) ) {
            Com_Printf( "%s", JSON("warning", "Couldn't resolve address"));
            return;
        }
        svs.authorizeAddress.port = BigShort( PORT_AUTHORIZE );
        Com_Printf( "%s", JSON("notice", va("%s resolved to %i.%i.%i.%i:%i", AUTHORIZE_SERVER_NAME,
    ...
    if ( svs.authorizeAddress.type != NA_BAD ) {
        ...
        Com_Printf("%s", JSON("notice", va("%s was banned from coming back", cl->name)));
    }
}
...
static void SV_BanNum_f( void ) {
    ...
    if ( Cmd_Argc() != 2 ) {
        Com_Printf ("%s", JSON("notice", "Usage: banClient <client number>"));
        return;
    }
...
static void SV_AddBanToList(qboolean isexception)
{
    ...
    // make sure server is running
    if ( !com_sv_running->integer ) {
        Com_Printf( "%s", JSON("warning", "Server is not running") );
        return;
    }
    ...
    if(argc < 2 || argc > 3)
    {
        Com_Printf ("%s", JSON("warning", va("Usage: %s (ip[/subnet] | clientnum [subnet])", Cmd_Argv(0))));
        return;
    }

    if(serverBansCount >= (int)ARRAY_LEN(serverBans))
    {
        Com_Printf ("%s", JSON("error", "Maximum number of bans/exceptions exceeded"));
        return;
    }
    ...
    if(strchr(banstring, '.') || strchr(banstring, ':'))
    {
        ...
        {
            Com_Printf("%s", JSON("error", va("Invalid address %s", banstring)));
            return;
        }
    }
    else
    {
        ...
        if(!cl)
        {
            Com_Printf("%s", JSON("Error", va("Playernum %s does not exist", Cmd_Argv(1))));
            return;
        }
    ...
    {
        Com_Printf("%s", JSON("error", "Can ban players connected via the internet only"));
        return;
    }
    ...
    for(index = 0; index < serverBansCount; index++)
    {
        ...
        if(curban->subnet <= mask)
        {
            if((curban->isexception || !isexception) && NET_CompareBaseAdrMask(curban->ip, ip, curban->subnet))
            {
                ...
                Com_Printf("%s", JSON("error", va("%s %s/%d supersedes %s %s/%d", curban->isexception ? "Exception" : "Ban",
        ...
        if(curban->subnet >= mask)
        {
            if(!curban->isexception && isexception && NET_CompareBaseAdrMask(curban->ip, ip, mask))
            {
                ...
                Com_Printf("%s", JSON("error", va("%s %s/%d supersedes already existing %s %s/%d", isexception ? "Exception" : "Ban",
    ...
    Com_Printf("%s", JSON("notice", va("Added %s: %s/%d", isexception ? "ban exception" : "ban", NET_AdrToString(ip), mask)));
}
...
static void SV_DelBanFromList(qboolean isexception)
{
    ...
    if(Cmd_Argc() != 2)
    {
        Com_Printf ("%s", JSON("notice", va("Usage: %s (ip[/subnet] | num)", Cmd_Argv(0))));
        return;
    }
    ...
    if(strchr(banstring, '.') || strchr(banstring, ':'))
    {
        ...
        while(index < serverBansCount)
        {
            ...
            {
                Com_Printf("%s", JSON("notice", va("Deleting %s %s/%d",
    ...
    else
    {
        ...
        {
            Com_Printf("%s", JSON("error", "Invalid ban number given"));
            return;
        }
        ...
        for(index = 0; index < serverBansCount; index++)
        {
            if(serverBans[index].isexception == isexception)
            {
                ...
                if(count == todel)
                {
                    Com_Printf("%s", JSON("notice", va("Deleting %s %s/%d", isexception ? "exception" : "ban",
...
static void SV_ListBans_f(void)
{
    ...
    for(index = count = 0; index < serverBansCount; index++)
    {
        ...
        if(!ban->isexception)
        {
            ...
            Com_Printf("%s", JSON("notice", va("Ban #%d: %s/%d", count, NET_AdrToString(ban->ip), ban->subnet)));
        }
    }
    // List all exceptions
    for(index = count = 0; index < serverBansCount; index++)
    {
        ...
        if(ban->isexception)
        {
            ...
            Com_Printf("%s", JSON("notice", va("Except #%d: %s/%d", count, NET_AdrToString(ban->ip), ban->subnet)));
        }
    }
}
...
static void SV_FlushBans_f(void)
{
    ...
    Com_Printf("%s", JSON("notice", "All bans and exceptions have been deleted"));
}
...
static void SV_Status_f( void ) {
    ...
    Com_Printf ("%s", JSON("map", va("%s", sv_mapname->string)));
    ...
    for (i = 0, cl = svs.clients ; i < sv_maxclients->integer ; i++, cl++ )
    {
        ...
        Com_Printf ("%s", JSON("clientnum", va("%2i", i)));

        ps = SV_GameClientNum( i );
        Com_Printf ( "%s", JSON("score", va("%5i", ps->persistant[PERS_SCORE])));


        if (cl->state == CS_CONNECTED) {
            Com_Printf ("%s", JSON("connection_state", "connected"));
        } else if (cl->state == CS_ZOMBIE) {
            Com_Printf ("%s", JSON("connection_state", "zombie"));
        } else {
            ping = cl->ping < 9999 ? cl->ping : 9999;
            Com_Printf ("%s", JSON("ping", va("%4i", ping)));
        }

        Com_Printf ("%s", JSON("clientname", va("%s", cl->name)));

        // TTimo adding a ^7 to reset the color
        s = NET_AdrToString( cl->netchan.remoteAddress );
        Com_Printf ("%s", JSON("remote_address", va("%s", s)));

        Com_Printf ("%s", JSON("rate", va("%5i", cl->rate)));
        Com_Printf ("\n");
    }
    Com_Printf ("\n");
}
...
static void SV_ConSay_f(void) {
    ...
    if ( Cmd_Argc () < 2 ) {
        Com_Printf( "%s", JSON("warning", "consay requires 2+ args") );
        return;
    }

    strcpy (text, "console: "); // TODO rename, hide?
    p = Cmd_Args();
...
static void SV_Serverinfo_f( void ) {
    // make sure server is running
    if ( !com_sv_running->integer ) {
        Com_Printf( "{\"error\":\"%s\"}", "Server is not running." );
        return;
    }

    Com_Printf ( "{" );
    Com_Printf ( "%s", JSON("message","ServerInfo") );

    const char *G_GetMapFullName(void);
    Com_Printf ( "%s", JSON("mapfullname", G_GetMapFullName() ));
    ...
    for (var = cvar_vars ; var ; var = var->next, i++) {
        ...
        // loop through all cvars that are CVAR_SERVERINFO
        if ( var->flags & CVAR_SERVERINFO ) {
            Com_Printf ( "%s", JSON(var->name, var->string) );
        }
    }
...
void SV_Playerinfo_f ( void ) {
    ...
    Com_Printf ( "{" );
    Com_Printf ( "%s", JSON("message","PlayerInfo") );
    ...
    for ( int i = 0 ; i < sv_maxclients->integer ; i++ ) {
        ...
        Com_Printf("{");
        Com_Printf("%s", JSON("name", cl->name));
        Com_Printf("%s", JSON("ping", va("%d", cl->ping )));
        Com_Printf("%s", JSON("rate",     va("%s", Info_ValueForKey (cl->userinfo, "rate"))));
        Com_Printf("%s", JSON("handicap", va("%s", Info_ValueForKey (cl->userinfo, "handicap"))));
        Com_Printf("%s", JSON("snaps",    va("%s", Info_ValueForKey (cl->userinfo, "snaps"))));
        Com_Printf("},");
    }
    Com_Printf ( " ] " );

    Com_Printf( ", %s", JSON("num_players", va("%d", num_players )));
...
// used to get value of cvar from proxy/database
static void SV_GetCvar_f ( void ) {
    ...
    if ( Cmd_Argc() != 2 ) {
        Com_Printf ("%s", JSON("notice", "Usage: getCvar <cvar name>"));
        return;
    }

    char *cvar_name = Cmd_Argv(1);
    cvar_t *cvar    = Cvar_Get(cvar_name, "", CVAR_SERVERINFO | CVAR_USERINFO | CVAR_LATCH );

    Com_Printf ( "{ \"%s\" : \"%s\" } ", cvar_name, cvar->string );
}
...
void SV_AddOperatorCommands( void ) {
    ...
    Cmd_AddCommand ("playerinfo", SV_Playerinfo_f);
    Cmd_AddCommand ("systeminfo", SV_Systeminfo_f);
    Cmd_AddCommand ("getcvar", SV_GetCvar_f);
