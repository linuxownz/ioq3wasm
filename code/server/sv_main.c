...
{
    UNUSED(message);
    return; //disable sending heartbeats to anything
}
...
// TODO to be removed...  use firewall instead...
// TODO to be removed...  use firewall instead...
// TODO to be removed...  use firewall instead...
...
void SV_Frame( int msec ) {
    ...
    if ( sv_killserver->integer ) {
        SV_Shutdown ((char *)"Server was killed");
    ...
    // modified SV_CheckPaused to always return false
    if ( SV_CheckPaused() ) {
        return;
    }
    ...
    if ( sv_fps->integer < 1 ) {
        Com_Printf ( "sv_fps bad value: %d setting to 10\n", sv_fps->integer );
        Cvar_Set( "sv_fps", "10" );
    }
    ...
    if(frameMsec < 1)
    {
        Cvar_Set("timescale", va((char *)"%f", sv_fps->integer / 1000.0f));
        frameMsec = 1;
    }
...
int SV_SendQueuedPackets()
{
    ...
    //if(SV_SendDownloadMessages()) // TODO remove?
    //    timeVal = 0;
