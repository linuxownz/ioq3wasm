...
void SV_DirectConnect( netadr_t from ) {
    ...
    if (!NET_IsLocalAddress(from))
    {
        ...
        if (i == MAX_CHALLENGES)
        {
            ...
            Com_Printf("ERROR, MAX_CHALLENGES\n");
            //Com_Error(ERR_FATAL, "MAX MAX_CHALLENGES"); //TODO REMOVE ME
            return;
        }
...
gotnewcl:
    ...
    *newcl = temp;
    ...
    if ( denied ) {
        // TODO convert to useable string..
...
static void SV_VerifyPaks_f( client_t *cl ) {
    ...
    // if the client is a websocket client assume FOR now everything is ok
...
static qboolean SV_ClientCommand( client_t *cl, msg_t *msg ) {
    ...
    if ( cl->state >= CS_ACTIVE && sv_floodProtect->integer && svs.time < cl->nextReliableTime ) {
