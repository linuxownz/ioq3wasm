...
#if 0
static struct BufferedFile *ReadBufferedFile(const char *name) // async
{
    assert(0);
    ...
    BF = malloc(sizeof(struct BufferedFile));
    if(!BF)
    {
        return(NULL);
    }
    ...
    BF->Length = 0; //FS_GetFile((char *) name, &buffer.v); //async
    BF->Buffer = buffer.b;
...
static uint32_t DecompressIDATs(struct BufferedFile *BF, uint8_t **Buffer)
{
    ...
    *Buffer = DecompressedData;
    ...
    CompressedData = malloc(CompressedDataLength);
    if(!CompressedData)
    {
        return(-1);
    }
    ...
    DecompressedData = malloc(puffDestLen);
    if(!DecompressedData)
    {
        free(CompressedData);
...
void R_LoadPNG(const char *name, byte **pic, int *width, int *height) {
    ...
    qboolean loading = FS_GetPreLoadedFile ( name, &data, &size );
    if ( ! loading ) {
    ...
    ThePNG = malloc(sizeof( struct BufferedFile));
    ...
    ThePNG->Length    = size;
    ThePNG->Buffer    = data;
    ThePNG->Ptr       = ThePNG->Buffer;
    ThePNG->BytesLeft = ThePNG->Length;
    ...
    if(!Signature)
    {
        printf("failed to read png signature %s\n", name);
    ...
    || IHDR_Width > INT_MAX / Q3IMAGE_BYTESPERPIXEL / IHDR_Height)
    {
        ...
        Com_Printf( "%s: invalid image size\n", name );
        return;
    }
    ...
    *pic = OutBuffer;
    ...
    int dur = end - start;
    if ( dur > 500 ) {
        Com_Printf("loading of %s took %dms\n", name, dur );
    }
