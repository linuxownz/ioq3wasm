...
void R_LoadTGAErrorCallback ( void *ptr );
void R_LoadTGACallback ( void *ptr );

extern int numImageLoadsInFlight;

qboolean R_LoadTGA ( const char *name, void *si, const imgType_t type, const imgFlags_t flags )
{
    if ( ! name || ! name[0] ) {
        assert(0);
        return qfalse;
    ...
    qboolean loading = FS_GetFileAsync ( name, &(emscripten_fetch_callback_t){
        .additionalData = ( type & 0xff ) << 8 | ( flags & 0xff ),
        .error_callback = R_LoadTGAErrorCallback,
        .errFatal       = qfalse,
        .errMessage     = "failed to load tga",
        .success_callback = R_LoadTGACallback,
        .userData       = si,
        .nocache        = qfalse,
    });
...
void R_LoadTGACallback ( void *ptr ) {
    ...
    // printf("%s name:%s len:%d type:%d flags:%d\n", __func__, name, length, type, flags);
    ...
    if(length < 18)
    {
        Com_Error( ERR_FATAL, "LoadTGA: header too short (%s)", name );
    }
    ...
    {
        Com_Error (ERR_FATAL, "LoadTGA: %s has an invalid image size", name);
    }
    ...
    if ( targa_header.image_type==2 || targa_header.image_type == 3 )
    {
        ...
        {
            Com_Error (ERR_FATAL, "LoadTGA: file truncated (%s)", name);
        }
                ...
                switch (targa_header.pixel_size)
                {
                    ...
                    case 32:
                        ...
                        *pixbuf++ = alphabyte;
                        break;
                    default:
                        Com_Error( ERR_FATAL, "LoadTGA: illegal pixel_size '%d' in file '%s'", targa_header.pixel_size, name );
                        break;
                }
            }
        }
...
breakOut:;
    ...
    if (targa_header.attributes & 0x20) {
        Com_Printf( "WARNING: '%s' TGA file header declares top-down image, ignoring\n", name);
    }
    ...
    R_FindImageFileCallback ( ptr, targa_rgba, columns, rows, type, flags );
    // have to call R_FindImageFileCallback ( from all R_LoadXXX )
