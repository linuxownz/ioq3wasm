...
void R_LoadJPGCallback(void *ptr);
void R_LoadJPGErrorCallback(void *ptr);

extern int numImageLoadsInFlight;
qboolean R_LoadJPG(const char *filename, void *si, const imgType_t type, const imgFlags_t flags )
{
    //printf("%s type:%d flags:%d\n", __func__, type, flags);

    qboolean loading = FS_GetFileAsync(filename, &(emscripten_fetch_callback_t) {
        .additionalData = ( type & 0xff ) << 8 | ( flags & 0xff ),
        .errFatal         = qfalse,
        .errMessage       = "failed to load jpg",
        .error_callback   = R_LoadJPGErrorCallback,
        .success_callback = R_LoadJPGCallback,
        .userData         = si,
        .nocache          = qfalse,
    });
...
void R_LoadJPGCallback ( void *ptr ) {
    ...
    if ( fetch->numBytes == 0 ) {
        Com_Error(ERR_FATAL,"FS_GetFileAsync %s failed", fetch->url);
        return;
    }
    ...
    unsigned int pixelcount = 0, memcount = 0;
    unsigned int sindex     = 0, dindex   = 0;

    const char *filename   = fetch->url;
    const unsigned int len = fetch->numBytes;

    imgType_t type   = ( fetch_callback->additionalData >> 8 ) & 0xFF;
    imgFlags_t flags = fetch_callback->additionalData & 0xFF;

    //printf("%s %s len: %d type:%d flags:%d\n", __func__, filename, len, type, flags);
    ...
    fbuffer.v = malloc(len);

    if ( ! fbuffer.v ) {
        Com_Error(ERR_FATAL, "malloc");
    }

    memcpy(fbuffer.v, fetch->data, len);

    //printf("jpeg: name: %s\n", filename);
    //printf("jpeg: size: %d\n", len);

    if ( strstr(filename, "png") ) { printf("why is jpeg loading png? '%s'\n", filename); assert(0);}
    if ( strstr(filename, "tga") ) { printf("why is jpeg loading tga? '%s'\n", filename); assert(0);}
    ...
    cinfo.err = jpeg_std_error(&jerr.pub); assert(cinfo.err);
      ...
      )
    {
        ...
        Com_Error(ERR_DROP, "LoadJPG: %s has an invalid image format: %dx%d*4=%d, components: %d", filename, cinfo.output_width, cinfo.output_height, pixelcount * 4, cinfo.output_components);
    }
    ...
    R_FindImageFileCallback ( ptr, out, cinfo.output_width, cinfo.output_height, type, flags );
...
void RE_SaveJPG(char * filename, int quality, int image_width, int image_height, byte *image_buffer, int padding) // TODO remove me
{
  byte *out;
  size_t bufSize;
  ...
  // ri.FS_WriteFile(filename, out, bufSize); // TODO
