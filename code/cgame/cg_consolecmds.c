...
void CG_TargetCommand_f( void ) {
	...
	CL_AddReliableCommand( va( "gc %i %i", targetNum, atoi( test ) ), qfalse );
}
...
static void CG_SizeUp_f (void) {
	Cvar_SetSafe("cg_viewsize", va("%i",(int)(cg_viewsize.integer+10)));
}
...
static void CG_spWin_f( void) {
	Cvar_SetSafe("cg_cameraOrbit", "2");
	Cvar_SetSafe("cg_cameraOrbitDelay", "35");
	Cvar_SetSafe("cg_thirdPerson", "1");
	Cvar_SetSafe("cg_thirdPersonAngle", "0");
	Cvar_SetSafe("cg_thirdPersonRange", "100");
	CG_AddBufferedSound(cgs.media.winnerSound);
	//S_StartLocalSound(cgs.media.winnerSound, CHAN_ANNOUNCER);
...
static void CG_spLose_f( void) {
	...
	//S_StartLocalSound(cgs.media.loserSound, CHAN_ANNOUNCER);
...
static void CG_ConfirmOrder_f (void ) {
	Cbuf_AddText(va("cmd vtell %d %s\n", cgs.acceptLeader, VOICECHAT_YES));
	Cbuf_AddText("+button5; wait; -button5");
	if (cg.time < cgs.acceptOrderTime) {
		CL_AddReliableCommand(va("teamtask %d\n", cgs.acceptTask), qfalse);
...
static void CG_DenyOrder_f (void ) {
	...
	Cbuf_AddText("+button6; wait; -button6");
	if (cg.time < cgs.acceptOrderTime) {
		cgs.acceptOrderTime = 0;
	}
}
...
static void CG_TaskOffense_f (void ) {
	...
	CL_AddReliableCommand(va("teamtask %d\n", TEAMTASK_OFFENSE), qfalse);
}

static void CG_TaskDefense_f (void ) {
	Cbuf_AddText(va("cmd vsay_team %s\n", VOICECHAT_ONDEFENSE));
	CL_AddReliableCommand(va("teamtask %d\n", TEAMTASK_DEFENSE), qfalse);
}

static void CG_TaskPatrol_f (void ) {
	Cbuf_AddText(va("cmd vsay_team %s\n", VOICECHAT_ONPATROL));
	CL_AddReliableCommand(va("teamtask %d\n", TEAMTASK_PATROL), qfalse);
}

static void CG_TaskCamp_f (void ) {
	Cbuf_AddText(va("cmd vsay_team %s\n", VOICECHAT_ONCAMPING));
	CL_AddReliableCommand(va("teamtask %d\n", TEAMTASK_CAMP), qfalse);
}

static void CG_TaskFollow_f (void ) {
	Cbuf_AddText(va("cmd vsay_team %s\n", VOICECHAT_ONFOLLOW));
	CL_AddReliableCommand(va("teamtask %d\n", TEAMTASK_FOLLOW), qfalse);
}

static void CG_TaskRetrieve_f (void ) {
	Cbuf_AddText(va("cmd vsay_team %s\n", VOICECHAT_ONRETURNFLAG));
	CL_AddReliableCommand(va("teamtask %d\n", TEAMTASK_RETRIEVE), qfalse);
}

static void CG_TaskEscort_f (void ) {
	Cbuf_AddText(va("cmd vsay_team %s\n", VOICECHAT_ONFOLLOWCARRIER));
	CL_AddReliableCommand(va("teamtask %d\n", TEAMTASK_ESCORT), qfalse);
}
...
static void CG_TauntKillInsult_f (void ) {
	Cbuf_AddText("cmd vsay kill_insult\n");
}

static void CG_TauntPraise_f (void ) {
	Cbuf_AddText("cmd vsay praise\n");
}

static void CG_TauntTaunt_f (void ) {
	Cbuf_AddText("cmd vtaunt\n");
}

static void CG_TauntDeathInsult_f (void ) {
	Cbuf_AddText("cmd vsay death_insult\n");
}

static void CG_TauntGauntlet_f (void ) {
	Cbuf_AddText("cmd vsay kill_gauntlet\n");
}
...
static void CG_StartOrbit_f( void ) {
	...
	if (cg_cameraOrbit.value != 0) {
		Cvar_SetSafe("cg_cameraOrbit", "0");
		Cvar_SetSafe("cg_thirdPerson", "0");
	} else {
		Cvar_SetSafe("cg_cameraOrbit", "5");
		Cvar_SetSafe("cg_thirdPerson", "1");
		Cvar_SetSafe("cg_thirdPersonAngle", "0");
		Cvar_SetSafe("cg_thirdPersonRange", "100");
	}
