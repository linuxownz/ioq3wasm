...
//freeze
		if ( cgs.g_freezetag && le->refEntity.customShader == cgs.media.freezeShader )
...
//			S_StartSound (re->origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.kamikazeExplodeSound );
			S_StartLocalSound(cgs.media.kamikazeExplodeSound, CHAN_AUTO);
		...
		RE_AddRefEntityToScene( &shockwave );
	}
...
//				S_StartSound (re->origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.kamikazeImplodeSound );
				S_StartLocalSound(cgs.media.kamikazeImplodeSound, CHAN_AUTO);
		...
		RE_AddRefEntityToScene( re );
		// add the dlight
		re.AddLightToScene( re->origin, c * 1000.0, 1.0, 1.0, c );
	}
