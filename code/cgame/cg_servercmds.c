...
void CG_ParseServerinfo( void ) {
    ...
    Cvar_SetSafe("g_gametype", va("%i", cgs.gametype));
    ...
    Cvar_SetSafe("g_redTeam", cgs.redTeam);
...
static void CG_ParseWarmup( void ) {
    ...
    if ( warmup == 0 && cg.warmup ) {
        ...
        if (cgs.gametype >= GT_CTF && cgs.gametype <= GT_HARVESTER) {
            S_StartLocalSound( cgs.media.countPrepareTeamSound, CHAN_ANNOUNCER );
        } else
#endif
        {
...
#ifdef MISSIONPACK
        S_StartLocalSound( cgs.media.voteNow, CHAN_ANNOUNCER );
#endif //MISSIONPACK
...
static void CG_MapRestart( void ) {
    ...
    if (cg_singlePlayerActive.integer) {
        Cvar_SetSafe("ui_matchStartTime", va("%i", cg.time));
        if (cg_recordSPDemo.integer && *cg_recordSPDemoName.string) {
            SendConsoleCommand(va("set g_synchronousclients 1 ; record %s \n", cg_recordSPDemoName.string));
        }
    }
#endif
    Cvar_SetSafe("cg_thirdPerson", "0");
}
...
int CG_ParseVoiceChats( const char *filename, voiceChatList_t *voiceChatList, int maxVoiceChats ) {
    ...
    qboolean exists = FS_FileExists( filename ); // missionpack
    if ( !exists ) {
        Com_Printf( va( S_COLOR_RED "voice chat file not found: %s\n", filename ) );
        return qfalse;
    }

    size_t len = FS_GetFile(filename, &buf[0]); // async change to FS_GetFile  MISSIONPACK

    if ( len >= MAX_VOICEFILESIZE ) {
        Com_Error(ERR_FATAL,"voice chat file too large: %s is %i, max allowed is %i\n", filename, len, MAX_VOICEFILESIZE );
        Com_Printf( va( S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i\n", filename, len, MAX_VOICEFILESIZE ) );
        //FS_FCloseFile( f ); // TODO // missionpack
        return qfalse;
    }
    ...
    if (!Q_stricmp(token, "female")) {
        ...
        Com_Printf( va( S_COLOR_RED "expected gender not found in voice chat file: %s\n", filename ) );
        return qfalse;
    }
    ...
    while ( 1 ) {
        ...
        token = COM_ParseExt(p, qtrue);
        if (Q_stricmp(token, "{")) {
            Com_Printf( va( S_COLOR_RED "expected { found %s in voice chat file: %s\n", token, filename ) );
            return qfalse;
        }
        ...
        while(1) {
            ...
            if (!Q_stricmp(token, "}"))
                break;
            sound = S_RegisterSound( token, compress ); // MISSIONPACK TODO
...
static int CG_HeadModelVoiceChats( char *filename ) {
    ...
    qboolean exists = FS_FileExists( filename ); // missionpack
    if ( !exists ) {
        //Com_Printf( va( "voice chat file not found: %s\n", filename ) );
        return -1;
    }

    size_t len = FS_FileGetSize(filename); // async missionpack
    if ( len >= MAX_VOICEFILESIZE ) {
        Com_Printf( va( S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i\n", filename, len, MAX_VOICEFILESIZE ) );
        // FS_FCloseFile( f ); // TODO missionpack
        return -1;
...
void CG_PlayVoiceChat( bufferedVoiceChat_t *vchat ) {
    ...
    if ( !cg_noVoiceChats.integer ) {
        S_StartLocalSound( vchat->snd, CHAN_VOICE);
...
=================
*/
static void CG_VoiceChatLocal( int mode, qboolean voiceOnly, int clientNum, int color, const char *cmd ) {
    char *chat;
...
static void CG_ServerCommand( void ) {
    ...
    if ( !strcmp( cmd, "print" ) ) {
        ...
        if ( !Q_stricmpn( cmd, "vote failed", 11 ) || !Q_stricmpn( cmd, "team vote failed", 16 )) {
            S_StartLocalSound( cgs.media.voteFailed, CHAN_ANNOUNCER );
        } else if ( !Q_stricmpn( cmd, "vote passed", 11 ) || !Q_stricmpn( cmd, "team vote passed", 16 ) ) {
            S_StartLocalSound( cgs.media.votePassed, CHAN_ANNOUNCER );
        }
#endif
        return;
    }
