...
#define DEFAULT_ENEMYMODEL      "tankjr"
#define DEFAULT_ENEMYHEADMODEL  "tankjr"
...
typedef struct {
    ...
    qhandle_t   player_james_lowerModel;
    qhandle_t   player_james_upperModel;
    qhandle_t   players_james_headModel;

    qhandle_t   player_janet_lowerModel;
    qhandle_t   player_janet_upperModel;
    qhandle_t   player_janet_headModel;
...
extern  vmCvar_t		cg_newScoreboard;
...
void CG_Printf( const char *msg, ... ) __attribute__ ((format (printf, 1, 2)));
void CG_Error( const char *msg, ... ) __attribute__ ((noreturn, format (printf, 1, 2)));
...
//void      Com_Printf( const char *fmt ); // TODO void Com_Printf( const char *msg, ... ) __attribute__ ((format (printf, 1, 2))); in q_shared.h

// abort the game
//void      Com_Error(const char *fmt) __attribute__((noreturn)); // TODO void Com_Error( int level, const char *error, ... ) __attribute__ ((noreturn, format(printf, 2, 3))); in q_shared.h
...
//int         FS_Read( void *buffer, unsigned int cnt, fileHandle_t file );
...
//qboolean    FS_FileExistsAsync(const char* filename, const emscripten_fetch_callback_t * callback);
...
size_t        FS_FileGetSize(const char* filename);
...
void        CM_CapsuleTrace( trace_t *results, const vec3_t start, const vec3_t end,
...
void        CM_TransformedCapsuleTrace( trace_t *results, const vec3_t start, const vec3_t end,
                      const vec3_t mins, const vec3_t maxs,
                      clipHandle_t model, int brushmask,
...
// void        R_LoadWorldMap( const char *mapname );  // remnants of trap_r_load_worldmap ...
