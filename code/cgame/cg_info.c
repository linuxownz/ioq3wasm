...
void CG_LoadingClient( int clientNum ) {
    ...
    if( cgs.gametype == GT_SINGLE_PLAYER ) {
        S_RegisterSound( va( "sound/player/announce/%s.ogg", personality ) );
    }
...
void CG_DrawInformation( void ) {
    ...
    qhandle_t levelshot = RE_RegisterShaderNoMip( va( "levelshots/%s.png", s ) );
    if ( !levelshot ) {
        levelshot = RE_RegisterShaderNoMip( "menu/art/unknownmap" );
    }*/
    RE_SetColor( NULL );
    CG_DrawPic( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, cgs.media.levelShotShader );

    // qhandle_t detail = RE_RegisterShader( "levelShotDetail" );
    // blend a detail texture over it
    RE_StretchPic( 0, 0, cgs.glconfig.vidWidth, cgs.glconfig.vidHeight, 0, 0, 2.5, 2, cgs.media.levelShotDetail );
