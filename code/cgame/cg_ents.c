...
void CG_PositionEntityOnTag( refEntity_t *entity, const refEntity_t *parent,
	...
	// lerp the tag
	R_LerpTag( &lerped, parentModel, parent->oldframe, parent->frame, 1.0 - parent->backlerp, tagName );
...
static void CG_EntityEffects( centity_t *cent ) {
	...
	if ( cent->currentState.loopSound ) {
		if (cent->currentState.eType != ET_SPEAKER) {
			...
			S_AddRealLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin,
				cgs.gameSounds[ cent->currentState.loopSound ] );
		}
	}
	...
	if(cent->currentState.constantLight)
	{
		...
		RE_AddLightToScene(cent->lerpOrigin, i, r, g, b);
	}
...
/*
	...
	if ( weapon->missileDlight ) {
		RE_AddLightToScene(cent->lerpOrigin, weapon->missileDlight, weapon->missileDlightColor[col][0], weapon->missileDlightColor[col][1], weapon->missileDlightColor[col][2] );
	}
*/
	// add dynamic light
	if ( weapon->missileDlight ) {
		RE_AddLightToScene(cent->lerpOrigin, weapon->missileDlight, weapon->missileDlightColor[0], weapon->missileDlightColor[1], weapon->missileDlightColor[2] );
	}
...
static void CG_Grapple( centity_t *cent ) {
	...
	// add missile sound
	if ( weapon->missileSound ) {
		S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, weapon->missileSound );
	}
#endif
		...
		if ( cent->currentState.frame == 2) {
			...
			if (t > h) {
				if ( !cent->muzzleFlashTime ) {
					S_StartSound (cent->lerpOrigin, ENTITYNUM_NONE, CHAN_BODY,  cgs.media.obeliskRespawnSound);
