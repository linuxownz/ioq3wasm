...
sfxHandle_t CG_CustomSound( int clientNum, const char *soundName ) {
    ...
    if ( ! soundName || ! soundName[0] ) {
        assert(0);
        Com_Error ( ERR_FATAL, "%s soundName is null\n", __func__ );
    }
    ...
    for ( int i = 0 ; i < MAX_CUSTOM_SOUNDS && cg_customSoundNames[i] ; i++ ) {
        //Com_Printf("looking for %s in %s\n", soundName, cg_customSoundNames[i]);
        if ( !strcmp( soundName, cg_customSoundNames[i] ) ) {
            return ci->sounds[i];
        }
    }

    Com_Printf("could not find custom sound, using default\n");
    return cgs.gameSounds[0];
...
{
    Com_Printf("%s parsing %s\n", __func__, filename);

    char *data = NULL;
    int length = 0;

    if ( ! FS_GetPreLoadedFile ( filename, (void **)&data, &length ) ) {
    ...
    const char *text = buffer; // TODO free
    ...
    if ( i != MAX_ANIMATIONS ) {
        printf("Error parsing animation file: %s %d %d\n", filename, i, MAX_ANIMATIONS );
        printf("text:'%s'\n", text);
        assert(0);
        Com_Error( ERR_FATAL, "Error parsing animation file: %s\n", filename );
        return qfalse;
    }
    ...
    //
    // new anims changes
    // TODO commented out in original source
...
==========================
*/
static void CG_LoadClientModelFile( qhandle_t *skin,  const team_t _team, const char *teamName, const char *modelName, const char *skinName, const char *base, const char *ext ) {
    char filename[MAX_QPATH];
    int length = MAX_QPATH;
    char *team;
...
/*
==========================
CG_LoadClientHeadFile
2+1 calls
==========================
*/
static void CG_LoadClientHeadFile( qhandle_t *skin, const team_t _team, const char *teamName, const char *headModelName, const char *headSkinName, const char *base, const char *ext ) {
    char filename[MAX_QPATH];
    ...
    if ( cgs.gametype >= GT_TEAM ) {
        if ( _team == TEAM_BLUE ) {
            team = "blue";
        } else if ( _team == TEAM_RED ) {
            team = "red";
        }
    }

    if ( teamName && *teamName ) {
        Com_sprintf( filename, length, "models/players/%s/%s/%s%s_%s.%s", headModelName, headSkinName, teamName, base, team, ext );
    }
    else {
        Com_sprintf( filename, length, "models/players/%s/%s/%s_%s.%s", headModelName, headSkinName, base, team, ext );
    }
    ...
    *skin = RE_RegisterSkin( filename  );
    ...
    if ( cgs.gametype >= GT_TEAM ) {
        if ( teamName && *teamName ) {
            Com_sprintf( filename, length, "models/players/%s/%s%s_%s.%s", headModelName, teamName, base, team, ext );
        }
        else {
        ...
        if ( teamName && *teamName ) {
            Com_sprintf( filename, length, "models/players/%s/%s%s_%s.%s", headModelName, teamName, base, headSkinName, ext );
        }
        else {
...
==========================
*/
static void CG_RegisterClientSkin( clientInfo_t *ci, const char *teamName, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName ) {

    CG_LoadClientModelFile( &ci->legsSkin, ci->team, teamName, modelName, skinName, "lower", "skin" );

    CG_LoadClientModelFile( &ci->torsoSkin, ci->team, teamName, modelName, skinName, "upper", "skin" );

    CG_LoadClientHeadFile( &ci->headSkin, ci->team, teamName, headModelName, headSkinName, "head", "skin" );
}
...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName ) {
    //Com_Printf("%s modelName:%s skinName:%s headModelName:%s headSkinName: %s teamname %s\n", __func__, modelName, skinName, headModelName, headSkinName, teamName);
    ...
    if ( ! ci->legsModel ) { // to simplify things that may never run
    ...
    CG_RegisterClientSkin( ci, teamName, modelName, skinName, headName, headSkinName );

    if ( teamName && *teamName) { // TODO what to do if skin loads fail TODO
        ...
        CG_RegisterClientSkin( ci, newTeamName, modelName, skinName, headName, headSkinName );
    }
    ...
    // CG_LoadClientHeadFile( filename, sizeof(filename), ci, teamName, headName, headSkinName, "icon", "skin" );
...
5 calls
CG_LoadDeferredPlayers   1  call
CG_SetDeferredClientInfo 3 calls
CG_NewClientInfo         1  call
===================
*/
static void CG_LoadClientInfo( int clientNum, clientInfo_t *ci ) {
    ...
    if ( ! CG_RegisterClientModelname( ci, ci->modelName, ci->skinName, ci->headModelName, ci->headSkinName, teamname ) ) {
        if ( cg_buildScript.integer ) { // TODO buildScript is default 0
...
void CG_NewClientInfo( int clientNum ) {
    ...
    qboolean me     = clientNum == cg.clientNum;
    qboolean myteam = cgs.clientinfo[clientNum].team == cgs.clientinfo[cg.clientNum].team;
    const char *enemyModel = cg_enemyModel.string;
    ...
    if ( cg_forceModel.integer ) {
        ...
        if( cgs.gametype >= GT_TEAM ) {
            if ( me ) {
                ...
                cvar_t *team_model = Cvar_Get("team_model", "sarge", CVAR_ARCHIVE | CVAR_USERINFO);
                if (  team_model ) {
                    Q_strncpyz( newInfo.modelName, team_model->string, sizeof( newInfo.modelName ) );
                } else {
                    Com_Printf("FAILED to find team_model cvar\n\n");
                }
            } else {
                Q_strncpyz( newInfo.modelName, enemyModel[0] ? enemyModel : DEFAULT_ENEMYMODEL, sizeof( newInfo.modelName ) );
            }
    ...
    // head model
    v = Info_ValueForKey( configstring, "headmodel" );  // was hmodel
    if ( cg_forceModel.integer ) {
        // forcemodel makes everyone use a single model
        // to prevent load hitches
        char modelStr[MAX_QPATH];
        ...
        if( cgs.gametype >= GT_TEAM ) {
            if ( me ) {
                ...
                cvar_t *team_headmodel = Cvar_Get("team_headmodel", "sarge", CVAR_ARCHIVE | CVAR_USERINFO);
                if ( team_headmodel ) {
                    Q_strncpyz( newInfo.headModelName, team_headmodel->string, sizeof( newInfo.headModelName ) );
                } else {
                    Com_Printf("FAILED to find team_headmodel cvar\n\n");
                }
            } else {
                Q_strncpyz( newInfo.headModelName, enemyModel[0] ? enemyModel : DEFAULT_ENEMYHEADMODEL, sizeof( newInfo.headModelName ) );
            }
            Q_strncpyz( newInfo.headSkinName, "default", sizeof( newInfo.headSkinName ) );
        } else {
            Cvar_VariableStringBuffer( "headmodel", modelStr, sizeof( modelStr ) );
            if ( ( skin = strchr( modelStr, '/' ) ) == NULL) {
                skin = "default";
            } else {
                *skin++ = 0;
    ...
    if ( !CG_ScanForExistingClientInfo( &newInfo ) ) {
        ...
        forceDefer = Hunk_MemoryRemaining() < 4000000; // TODO
    ...
    memcpy(ci, &newInfo, sizeof(newInfo)); // *ci = newInfo; // TODO struct returned by value <<<<<
}
...
void CG_LoadDeferredPlayers( void ) {
    ...
    for ( i = 0, ci = cgs.clientinfo ; i < cgs.maxclients ; i++, ci++ ) {
        if ( ci->infoValid && ci->deferred ) {
            ...
            if ( Hunk_MemoryRemaining() < 4000000 ) {
                CG_Printf( "Memory is low. Using deferred model.\n" );
                ci->deferred = qfalse;
                continue;
            }
            CG_LoadClientInfo( i, ci );
//          break; // TODO commented out in original
        }
    }
}
...
assert(0); // TODO modified but not tested
    ...
    int tokens          = cent->currentState.generic1;
    ...
    for ( int i = 0; i < tokens - trail->numpositions; i++ ) {
        for ( int j = trail->numpositions; j > 0; j-- ) {
    ...
    for ( int i = 0; i < trail->numpositions; i++ ) {
    ...
    for (int i = 0; i < trail->numpositions; i++) {
        ...
        float angle = (((cg.time + 500 * MAX_SKULLTRAIL - 500 * i) / 16) & 255) * (M_PI * 2) / 255;
...
static qboolean CG_PlayerShadow( centity_t *cent, float *shadowPlane ) {
    ...
    *shadowPlane = 0;
    ...
    CM_BoxTrace( &trace, cent->lerpOrigin, end, mins, maxs, 0, MASK_PLAYERSOLID, 0 ); // TODO 0??
...
static void CG_PlayerSplash( centity_t *cent ) {
    ...
    CM_BoxTrace( &trace, start, end, NULL, NULL, 0, ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ), 0 ); // TODO 0??
    ...
    RE_AddPolyToScene( cgs.media.wakeMarkShader, 4, verts, 1 ); // TODO 1?? Poly vs PolyS
}
...
*/
#if 0
static int CG_LightVerts( vec3_t normal, int numVerts, polyVert_t *verts )
{
    int             i, j;
    float           incoming;
    vec3_t          ambientLight;
    ...
    R_LightForPoint( verts[0].xyz, ambientLight, directedLight, lightDir );
...
void CG_Player( centity_t *cent ) {
    ...
    if ( cent->currentState.powerups & ( 1 << PW_GUARD ) ) {
        ...
        re.AddRefEntityToScene( &powerup );
    }
