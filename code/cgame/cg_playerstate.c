...
void CG_CheckLocalSounds( playerState_t *ps, playerState_t *ops ) {
	...
	if ( ps->stats[STAT_HEALTH] < ops->stats[STAT_HEALTH] - 12 ) {
        //Com_Printf("%s health:%d old health:%d\n", __func__, ps->stats[STAT_HEALTH], ops->stats[STAT_HEALTH]);
	...
	if (ps->persistant[PERS_PLAYEREVENTS] != ops->persistant[PERS_PLAYEREVENTS]) {
		if ((ps->persistant[PERS_PLAYEREVENTS] & PLAYEREVENT_DENIEDREWARD) !=
			...
			S_StartLocalSound( cgs.media.holyShitSound, CHAN_ANNOUNCER );
		}
		reward = qtrue;
	}
	...
	if ( cgs.gametype > GT_TEAM ) {
		...
		{
			S_StartLocalSound( cgs.media.youHaveFlagSound, CHAN_ANNOUNCER );
		}
	}
	...
	// fraglimit warnings
	if ( cgs.fraglimit > 0 && ( ( ! cgs.g_freezetag && cgs.gametype < GT_CTF ) || ( cgs.g_freezetag && cgs.gametype < GT_TEAM ) ) ) {
