...
void CG_Draw3DModelWithShaders( float x, float y, float w, float h, qhandle_t model, qhandle_t skin, vec3_t origin, vec3_t angles, qhandle_t shaders[3] ) {
    refdef_t        refdef;
    refEntity_t     ent;
    ...
    qboolean shader = qfalse;
    for ( int i = 0 ; i < 3 ; i++ ) {
        if (shaders[i]) {
            shader = qtrue;
            ent.customShader = shaders[i];
            RE_AddRefEntityToScene( &ent );
        }
    }
...
void CG_DrawHead( float x, float y, float w, float h, int clientNum, vec3_t headAngles ) {
    ...
    if ( cg_draw3dIcons.integer ) {
        ...
        qhandle_t shaders[3] = { 0 };
        int shaderpos = 0;

        if ( cg.snap->ps.clientNum == clientNum ) {
            if ( cg.snap->ps.powerups[PW_QUAD] ) {
                shaders[shaderpos++] = cgs.media.quadWeaponShader;
            }
            if ( cg.snap->ps.powerups[PW_REGEN] ) {
                shaders[shaderpos++] = cgs.media.regenShader;
            }
            if ( cg.snap->ps.powerups[PW_BATTLESUIT] ) {
                shaders[shaderpos++] = cgs.media.battleWeaponShader;
            }
        } else if ( ci->powerups ) {
        ...
        if ( shaderpos ) {
            CG_Draw3DModelWithShaders( x, y, w, h, ci->headModel, ci->headSkin, origin, headAngles, shaders );
        } else {
...
static float CG_DrawSpeedMeter( float y ) {
    ...
    if ( cg_drawSpeed.integer == 1 ) {
        ...
        /* center of screen */
        CG_DrawSmallString( 320, 210, s, 1.0F); // TODO make into cvars x, y
        return y;
    }
}
...
static void CG_DrawLagometer( void ) {
    ...
    for ( a = 0 ; a < aw ; a++ ) {
        ...
        if ( v > 0 ) {
            ...
            RE_StretchPic ( ax + aw - a, mid - v, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
        } else if ( v < 0 ) {
            if ( color != 2 ) {
                color = 2;
            ...
            RE_StretchPic( ax + aw - a, mid, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
        }
    }
    ...
    for ( a = 0 ; a < aw ; a++ ) {
        ...
        if ( v > 0 ) {
            ...
            RE_StretchPic( ax + aw - a, ay + ah - v, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
        } else if ( v < 0 ) {
            if ( color != 4 ) {
                color = 4;      // RED for dropped snapshots
                RE_SetColor( g_color_table[ColorIndex(COLOR_RED)] );
            }
            RE_StretchPic( ax + aw - a, ay + ah - range, 1, range, 0, 0, 0, 0, cgs.media.whiteShader );
        }
    }
    ...
    if ( !cg.demoPlayback && cg_drawPing.integer) {
        CG_DrawSmallString( x+12, y + 28, va( "%ims", (int)meanPing ), 0.5f );
    }
...
static void CG_DrawMovementKeys(void) {
    ...
    if ( Key_IsDown ( Key_GetKey("+forward") ) ) {
        CG_DrawBigString( 315, 214, "^", 1.0F);
    }

    if ( Key_IsDown ( Key_GetKey("+back") ) ) {
        CG_DrawBigString( 315, 244, "v", 1.0F);
    }

    if ( Key_IsDown ( Key_GetKey("+moveleft") ) ) {
        CG_DrawBigString( 300, 232, "<", 1.0F);
    }

    if ( Key_IsDown ( Key_GetKey("+moveright") ) ) {
        CG_DrawBigString( 330, 232, ">", 1.0F);
    }

    if ( Key_IsDown ( Key_GetKey("+moveup") ) ) {
        CG_DrawBigString( 315, 234, "J", 1.0F);
    }
}
...
static void CG_DrawCrosshair(void)
{
    ...
    RE_StretchPic( x + cg.refdef.x + 0.5 * (cg.refdef.width - w), y + cg.refdef.y + 0.5 * (cg.refdef.height - h), w, h, 0, 0, 1, 1, hShader );
...
static qboolean CG_DrawFollow( void ) {
    ...
    CG_DrawSmallString( 320 - 9 * SMALLCHAR_WIDTH - 5, 7, "following", 1.0F );

    name = cgs.clientinfo[ cg.snap->ps.clientNum ].name;

    x = 0.5 * ( 640 - SMALL_WIDTH * CG_DrawStrlen( name ) );

    CG_DrawStringExt( x + 45, 1, name, color, qtrue, qtrue, SMALL_WIDTH, SMALL_HEIGHT, 0 );
