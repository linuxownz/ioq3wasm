...
extern int glesMajorVersion ; // TODO
static  void R_LoadLightmaps( lump_t *l, lump_t *surfs ) {
    ...
    else
    {
        ...
        for( i = 0, surf = (dsurface_t *)(fileBase + surfs->fileofs); // TODO possible alignment fault
    ...
    image = malloc(tr.lightmapSize * tr.lightmapSize * 4 * 2);
    ...
    tr.lightmaps = Hunk_Alloc( tr.numLightmaps * sizeof(image_t *), h_low );

    if (tr.worldDeluxeMapping)
        tr.deluxemaps = Hunk_Alloc( tr.numLightmaps * sizeof(image_t *), h_low );

    textureInternalFormat = GL_RGBA8;
    if (r_hdr->integer && ! glesMajorVersion)
    {
        assert(0);
    ...
    for(i = 0; i < numLightmaps; i++)
    {
        ...
        // if (tr.worldLightmapping)
        {
            ...
            if (textureInternalFormat == GL_RGBA16)
            {
                ...
                assert(0);
                size = 0; //FS_GetFile(filename, (void **)&hdrLightmap);
            }
            ...
            for ( j = 0 ; j < tr.lightmapSize * tr.lightmapSize; j++ )
            {
                if (hdrLightmap)
                {
                    ...
                    ColorToRGB16(color, (uint16_t *)(&image[j * 8])); // TODO possible alignment fault
                    ((uint16_t *)(&image[j * 8]))[3] = 65535; // TODO possible alignment fault
                }
...
static  void R_LoadVisibility( lump_t *l ) {
    ...
    s_worldData.numClusters = LittleLong( ((int *)buf)[0] ); // TODO possible alignment fault
    s_worldData.clusterBytes = LittleLong( ((int *)buf)[1] ); // TODO possible alignment fault
...
ShaderForShaderNum
called by ParseX
which is called by RE_LoadWorldMap2
has 1 R_FindShader call

called from R_LoadSurface
    ParseFace
    ParseMesh
    ParseTrisurf
    ParseFlare
...
static void ParseFace( dsurface_t *ds, drawVert_t *verts, float *hdrVertColors, msurface_t *surf, int *indexes  ) {
    ...
    cv->indexes = Hunk_Alloc(numIndexes * sizeof(cv->indexes[0]), h_low);

    cv->numVerts = numVerts;
    cv->verts = Hunk_Alloc(numVerts * sizeof(cv->verts[0]), h_low);
    ...
    if (badTriangles)
    {
        Com_Printf("Face has bad triangles, originally shader %s %d tris %d verts, now %d tris\n", surf->shader->name, numIndexes / 3, numVerts, numIndexes / 3 - badTriangles);
...
static void ParseTriSurf( dsurface_t *ds, drawVert_t *verts, float *hdrVertColors, msurface_t *surf, int *indexes ) {
    ...
    if (badTriangles)
    {
        Com_Printf("Trisurf has bad triangles, originally shader %s %d tris %d verts, now %d tris\n", surf->shader->name, numIndexes / 3, numVerts, numIndexes / 3 - badTriangles);
        cv->numIndexes -= badTriangles * 3;
    }
...
void R_MovePatchSurfacesToHunk(void) {
    ...
    for ( i = 0; i < s_worldData.numsurfaces; i++ ) {
        ...
        grid->widthLodError = Hunk_Alloc( grid->width * 4, h_low );
        Com_Memcpy(grid->widthLodError, copyFrom, grid->width * 4);
        free(copyFrom);

        copyFrom = grid->heightLodError;
        grid->heightLodError = Hunk_Alloc(grid->height * 4, h_low);
        Com_Memcpy(grid->heightLodError, copyFrom, grid->height * 4);
        free(copyFrom);

        copyFrom = grid->indexes;
        grid->indexes = Hunk_Alloc(grid->numIndexes * sizeof(glIndex_t), h_low);
...
static  void R_LoadSurfaces( lump_t *surfs, lump_t *verts, lump_t *indexLump ) {
    ...
    s_worldData.surfacesViewCount   = Hunk_Alloc ( count * sizeof(*s_worldData.surfacesViewCount), h_low );
    s_worldData.surfacesDlightBits  = Hunk_Alloc ( count * sizeof(*s_worldData.surfacesDlightBits), h_low );
    s_worldData.surfacesPshadowBits = Hunk_Alloc ( count * sizeof(*s_worldData.surfacesPshadowBits), h_low );
    ...
    if (r_hdr->integer)
    {
        ...
        assert(0);
        size = 0; //FS_GetFile(filename, (void **)&hdrVertColors);

        if (hdrVertColors)
        {
            //ri.Printf(PRINT_ALL, "Found!\n");
            if (size != sizeof(float) * 3 * (verts->filelen / sizeof(*dv)))
                Com_Error(ERR_DROP, "Bad size for %s (%i, expected %i)!", filename, size, (int)((sizeof(float)) * 3 * (verts->filelen / sizeof(*dv))));
        }
    }
    ...
    for ( i = 0 ; i < count ; i++, in++, out++ ) {
        switch ( LittleLong( in->surfaceType ) ) {
            case MST_PATCH:
                out->data = Hunk_Alloc( sizeof(srfBspSurface_t), h_low);
                break;
            ...
            case MST_PLANAR:
                ...
                break;
            case MST_FLARE:
                out->data = Hunk_Alloc( sizeof(srfFlare_t), h_low);
                break;
            default:
                break;
        }
...
static  void R_LoadSubmodels( lump_t *l ) {
    ...
    s_worldData.bmodels = out = Hunk_Alloc( count * sizeof(*out), h_low );
    ...
    for ( i=0 ; i<count ; i++, in++, out++ ) {
        ...
        if ( model == NULL ) {
            Com_Error(ERR_FATAL, "%s model == NULL", __func__);
        }
...
static  void R_LoadNodesAndLeafs (lump_t *nodeLump, lump_t *leafLump) {
    ...
    out = Hunk_Alloc ( (numNodes + numLeafs) * sizeof(*out), h_low);
...
static  void R_LoadPlanes( lump_t *l ) {
    ...
    count = l->filelen / sizeof(*in);
    out = Hunk_Alloc ( count*2*sizeof(*out), h_low);
...
R_LoadFogs
1 R_FindShader call
but may be a request to find static *fog shader/image
=================
*/
static  void R_LoadFogs( lump_t *l, lump_t *brushesLump, lump_t *sidesLump ) {
    ...
    s_worldData.fogs = Hunk_Alloc ( s_worldData.numfogs*sizeof(*out), h_low);
...
void R_LoadLightGrid( lump_t *l ) {
    ...
    w->lightGridData = Hunk_Alloc( l->filelen, h_low );
    ...
    if (r_hdr->integer)
    {
        ...
        assert(0);
        size = 0; //FS_GetFile(filename, (void **)&hdrLightGrid);
        ...
        if (hdrLightGrid)
        {
            ...
            if (size != sizeof(float) * 6 * numGridPoints)
                Com_Error(ERR_DROP, "Bad size for %s (%i, expected %i)!", filename, size, (int)(sizeof(float)) * 6 * numGridPoints);

            w->lightGrid16 = Hunk_Alloc(sizeof(w->lightGrid16) * 6 * numGridPoints, h_low);
...
static void R_LoadEntities( lump_t *l ) {
    ...
    w->entityString = Hunk_Alloc( l->filelen + 1, h_low );
...
qboolean R_ParseSpawnVars( char *spawnVarChars, int maxSpawnVarChars, int *numSpawnVars, char *spawnVars[MAX_SPAWN_VARS][2] )
{
    ...
    *numSpawnVars = 0;
    ...
    if ( com_token[0] != '{' ) {
        Com_Printf( "R_ParseSpawnVars: found %s when expecting {\n",com_token );
        return qfalse;
    }
    ...
    while ( 1 ) {
        ...
        if ( !R_GetEntityToken( keyname, sizeof( keyname ) ) ) {
            Com_Printf( "R_ParseSpawnVars: EOF without closing brace\n" );
            return qfalse;
        }
        ...
        if ( com_token[0] == '}' ) {
            Com_Printf( "R_ParseSpawnVars: closing brace without data\n" );
            return qfalse;
        }

        if ( *numSpawnVars == MAX_SPAWN_VARS ) {
            Com_Printf( "R_ParseSpawnVars: MAX_SPAWN_VARS\n" );
            return qfalse;
        }
        ...
        {
            Com_Printf( "R_ParseSpawnVars: MAX_SPAWN_VAR_CHARS\n" );
            return qfalse;
        }
...
void R_LoadEnvironmentJson(const char *baseName)
{
    ...
    assert(0);
    filelen = 0; //FS_GetFile(filename, &buffer.v);
    if (!buffer.c)
        return;
    ...
    {
        Com_Printf("Bad %s: does not start with a object\n", filename);
        free(buffer.v) ;
        buffer.v = NULL;
        return;
    }
    ...
    {
        Com_Printf("Bad %s: Cubemaps not an array\n", filename);
        free ( buffer.v );
        buffer.v = NULL;
        return;
    }
    ...
    tr.cubemaps = Hunk_Alloc(tr.numCubemaps * sizeof(*tr.cubemaps), h_low);
...
void R_LoadCubemaps(void)
{
    ...
    for (i = 0; i < tr.numCubemaps; i++)
    {
        ...
        assert(0); // TODO we are skipping dds stuff for now
        /*cubemap->image = */
        //R_FindImageFile(filename, &cubemap->image, IMGTYPE_COLORALPHA, flags);
    }
}
...
void RE_LoadWorldMap( const char *name ) {
    ...
    if ( tr.worldMapLoaded ) {
        Com_Printf( "\n%s WARNING: attempted to redundantly load SAME world map %s\n", __func__, name );
        //assert(0);
        return;
    }
    ...
    if ( tr.worldMapLoaded ) {
        Com_Printf( "\n ERROR: attempted to redundantly load world map at %s\n\n", __func__ );
        assert(0);
    }
    ...
    byte *startMarker = Hunk_Alloc(0, h_low);
    c_gridVerts = 0;

    buffer.v = (void *)data;
    dheader_t *header = (dheader_t *)buffer.b; // TODO possible alignment fault
    fileBase = (byte *)header;

    int i = LittleLong (header->version); // LittleLong on little endian is a NO-OP
    if ( i != BSP_VERSION ) {
        Com_Error (ERR_DROP, "%s: %s has wrong version number (%i should be %i)", __func__, name, i, BSP_VERSION);
    }
...
#if 0
    if (0) // TODO This code is disabled in the original source
    {
        world_t *w;
        ...
        primaryLightGrid = malloc(lightGridSize * sizeof(*primaryLightGrid));
        ...
        {
            int i;
            byte *buffer = malloc(w->lightGridBounds[0] * w->lightGridBounds[1] * 3 + 18);
            byte *out;
            uint8_t *in;
            char fileName[MAX_QPATH];
            ...
            for (i = 0; i < w->lightGridBounds[2]; i++)
            {
                ...
                //ri.FS_WriteFile(fileName, buffer, w->lightGridBounds[0] * w->lightGridBounds[1] * 3 + 18); // TODO
            }
...
qboolean GetWorldLoaded(void) {
    return tr.worldMapLoaded;
}
