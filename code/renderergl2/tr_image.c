...
void GL_TextureMode( const char *string ) {
    ...
    if ( i == 6 ) {
        Com_Error (ERR_FATAL,"bad filter name\n");
        return;
    }
...
static void DoLinear(byte *in, byte *out, int width, int height)
{
    ...
    for (y = 2; y < height - 2; y += 2)
    {
        ...
        for ( ; x < width - 2; x += 2)
        {
            COPYSAMPLE(outbyte, inbyte); // TODO possible alignment fault
            outbyte += 8;
            inbyte += 8;
        }
    }
    ...
    for (y = 1; y < height - 1; y += 2)
    {
        ...
        COPYSAMPLE(sd, line2); line2 += 8; // TODO possible alignment fault
        COPYSAMPLE(sh, line3); line3 += 8; // TODO possible alignment fault

        outbyte = out + (y * width + x) * 4;

        for ( ; x < width - 1; x += 2)
        {
            COPYSAMPLE(se, line2); line2 += 8; // TODO possible alignment fault
            COPYSAMPLE(si, line3); line3 += 8; // TODO possible alignment fault
            ...
            COPYSAMPLE(sd, se); // TODO possible alignment fault
            COPYSAMPLE(sh, si); // TODO possible alignment fault
        }
    }
    ...
    for (y = 1; y < height - 1; y++)
    {
        ...
        COPYSAMPLE(sf, line3); line3 += 8; // TODO possible alignment fault
        ...
        {
            COPYSAMPLE(sd, line2); line2 += 8; // TODO possible alignment fault
            COPYSAMPLE(sg, line3); line3 += 8; // TODO possible alignment fault
            COPYSAMPLE(si, line4); line4 += 8; // TODO possible alignment fault
            ...
            COPYSAMPLE(sf, sg); // TODO possible alignment fault
        }
    }
}
        ...
        for (x = 0; x < width; x += WORKBLOCK_SIZE)
        {
            ...
            for (y2 = 0; y2 < fullworkheight; y2 += 2)
            {
                ...
                for (x2 = 0; x2 < fullworkwidth; x2 += 2)
                {
                    ...
                    COPYSAMPLE(outbyte, inbyte + srcx * 4); // TODO possible alignment fault
                    outbyte += 8;
                }
            }
...
void R_ConvertTextureFormat( const byte *in, int width, int height, GLenum format, GLenum type, byte *out )
{
    ...
    {
        Com_Error( ERR_FATAL, "Unable to convert RGBA image to OpenGL format 0x%X and type 0x%X", format, type );
    }
}
...
static qboolean RawImage_ScaleToPower2( byte **data, int *inout_width, int *inout_height, imgType_t type, imgFlags_t flags, byte **resampledBuffer)
{
         ...
         scaled_width < r_imageUpsampleMaxSize->integer && scaled_height < r_imageUpsampleMaxSize->integer)
    {
        ...
        *resampledBuffer = Hunk_AllocateTempMemory( finalwidth * finalheight * 4 );
...
LINE;
    ...
    if(normalmap)
    {
        if ((type == IMGTYPE_NORMALHEIGHT) && RawImage_HasAlpha(data, numPixels) && r_parallaxMapping->integer) // r_parallaxMapping is default 0
        {
            if ( qfalse /*!forceNoCompression && glRefConfig.textureCompression & TCR_BPTC*/)
            {
                internalFormat = GL_COMPRESSED_RGBA_BPTC_UNORM_ARB;
            }
            else if ( qfalse /*!forceNoCompression && glConfig.textureCompression == TC_S3TC_ARB*/)
            {
                internalFormat = GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
            }
            else if ( qfalse /*r_texturebits->integer == 16*/ )
            {
                internalFormat = GL_RGBA4;
            }
            else if ( qfalse /*r_texturebits->integer == 32*/ )
            {
        ...
        {
            if ( qfalse /*!forceNoCompression && glRefConfig.textureCompression & TCR_RGTC*/)
            {
                internalFormat = GL_COMPRESSED_RG_RGTC2;
            }
            else if ( qfalse /*!forceNoCompression && glRefConfig.textureCompression & TCR_BPTC*/)
            {
                internalFormat = GL_COMPRESSED_RGBA_BPTC_UNORM_ARB;
            }
            else if ( qfalse /*!forceNoCompression && glConfig.textureCompression == TC_S3TC_ARB*/)
    ...
    else if(lightMap)
    {
        ...
        // everything becomes solid black // TODO commented out in original source
    ...
    else
    {
        ...
        if ( samples == 3 )
        {
            if(0 && r_greyscale->integer) // TODO 0'd out in original source
            {
                if(qfalse /*r_texturebits->integer == 16 || r_texturebits->integer == 32*/)
            ...
            else
            {
                ...
                else if ( qfalse /*!forceNoCompression && glConfig.textureCompression == TC_S3TC */)
                {
                    internalFormat = GL_RGB4_S3TC;
                }
                else if ( qfalse /*r_texturebits->integer == 16*/ ) // r_texturebits is default 0
                {
                    internalFormat = GL_RGB5;
                }
                else if ( qfalse /* r_texturebits->integer == 32 */ ) // TODO test this was false
                {
                    internalFormat = GL_RGB8;
                }
                else
        ...
        {
            if(0 && r_greyscale->integer) // TODO 0'd out in original
            {
                if(qfalse /* r_texturebits->integer == 16 || r_texturebits->integer == 32*/)
            ...
            else
            {
                ...
                else if ( qfalse /*r_texturebits->integer == 16*/ ) // TODO r_texturebits is default 0
                {
                    internalFormat = GL_RGBA4;
                }
                else if ( qfalse /*r_texturebits->integer == 32*/ )// TODO test this was false
...
static void RawImage_UploadToRgtc2Texture(GLuint texture, int miplevel, int x, int y, int width, int height, byte *data)
{
    ...
    p = compressedData = Hunk_AllocateTempMemory(size);
...
static int CalculateMipSize(int width, int height, GLenum picFormat)
{
    ...
    switch (picFormat)
    {
        ...
        default:
            Com_Printf("Unsupported texture format %08x\n", picFormat);
            return 0;
    }
...
static void RawImage_UploadTexture(GLuint texture, byte *data, int x, int y, int width, int height, GLenum target, GLenum picFormat, GLenum dataFormat, GLenum dataType, int numMips, GLenum internalFormat, imgType_t type, imgFlags_t flags, qboolean subtexture )
{
    ...
    if (glesMajorVersion && rgba8 && (dataFormat != GL_RGBA || dataType != GL_UNSIGNED_BYTE))
    {
        formatBuffer = Hunk_AllocateTempMemory(4 * width * height);
    }
    ...
    do
    {
        ...
        else
        {
            ...
            if (rgba8 && rgtc) {
                ...
                GLDSA_TextureSubImage2DEXT(texture, target, miplevel, x, y, width, height, dataFormat, dataType, formatBuffer);
            }
            else {
                // TODO in newer main branch source
                //glTexSubImage2D(texture, target, x, y, width, height, dataFormat, dataType, data);
    ...
    if ( formatBuffer != NULL ) {
        Hunk_FreeTempMemory(formatBuffer);
    }
}
        ...
        if (type == IMGTYPE_COLORALPHA)
        {
            ...
            else if( r_greyscale->value )
            {
                for ( int i = 0; i < c; i++ )
                {
                    ...
                    scan[ i * 4     ] = LERP(scan[ i * 4 + 0 ], luma, r_greyscale->value);
...
image_t *R_CreateImage2( const char *name, byte *pic, int width, int height, GLenum picFormat, int numMips, imgType_t type, imgFlags_t flags, int internalFormat ) {
    ...
    // Com_Printf("%s %s width:%d height:%d numMips:%d\n", __func__, name, width, height, numMips);
    ...
    if ( tr.numImages >= MAX_DRAWIMAGES ) {
        Com_Error( ERR_DROP, "%s: MAX_DRAWIMAGES hit", __func__);
    }

    // TODO AllocImage
    image_t *image = tr.images[tr.numImages] = Hunk_Alloc( sizeof( image_t ), h_low );
    Com_Memset(image, 0, sizeof(image_t));

    image->index  = tr.numImages; // TODO use this to look up image
    ...
    strncpy (image->imgName, name, MAX_QPATH - 1);
    ...
    if (glesMajorVersion)
    {
        ...
        switch (internalFormat)
        {
            ...
            case GL_DEPTH_COMPONENT24:
            case GL_DEPTH_COMPONENT32:
                internalFormat = GL_UNSIGNED_INT;
                dataFormat     = GL_UNSIGNED_INT;
                dataType       = GL_UNSIGNED_INT;

            default:
                Com_Printf( "Missing OpenGL ES support for image '%s' with internal format 0x%X\n", name, internalFormat );
                //Com_Error( ERR_FATAL, "Missing OpenGL ES support for image '%s' with internal format 0x%X\n", name, internalFormat );
        }
    }
    ...
    do {
        ...
        if (cubemap) {
            for (int i = 0; i < 6; i++) {
                GLDSA_TextureImage2DEXT(image->texnum, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, miplevel, internalFormat, mipWidth, mipHeight, 0, dataFormat, dataType, NULL);
            }
        } else {
            GLDSA_TextureImage2DEXT(image->texnum, GL_TEXTURE_2D, miplevel, internalFormat, mipWidth, mipHeight, 0, dataFormat, dataType, NULL);
        }
    ...
    if (textureFilterAnisotropic && !cubemap) {
        GLDSA_TextureParameteriEXT(image->texnum, textureTarget, GL_TEXTURE_MAX_ANISOTROPY_EXT, mipmap ? (GLint)Com_Clamp(1, maxAnisotropy, r_ext_max_anisotropy->integer) : 1);
    }
    ...
    switch(internalFormat)
    {
        ...
        case GL_DEPTH_COMPONENT32_ARB:
            ...
            if ( !GL_VERSION_ATLEAST( 3, 0 ) ) {
                assert(0); // NEVER CALLED so far
                GLDSA_TextureParameterfEXT(image->texnum, textureTarget, GL_DEPTH_TEXTURE_MODE, GL_LUMINANCE);
            }
...
=================
*/
//static qboolean R_LoadImage( const char *name, shader_info_t *_si, imgType_t type, imgFlags_t flags )
...
void R_LoadImage( const char *name, byte **pic, int *width, int *height, GLenum *picFormat, int *numMips )
{
    ...
    *numMips = 0;
    ...
    if( *ext )
    {
        // assert ext == png
        if ( strcmp(ext, "png") ) {
            Com_Printf("%s extension error. want 'png' got '%s' for '%s'\n", __func__, ext, name);
            assert(0);
        }
    } else {
        // we got here because either no extension or no extension match or the other attempts failed because of ignore.files
        Com_sprintf( altName, MAX_QPATH, "%s.png", localName);
        loadName = altName;
    }

    R_LoadPNG( loadName, pic, width, height );
}
/*
===============
R_FindImageFile
...
all R_FindImageFile calls come from shader.c
except one call for loading .dds for cubemaps
==============
*/
//qboolean R_FindImageFile( const char *name, void *si, imgType_t type, imgFlags_t flags )

image_t *R_FindImageFile( const char *name, imgType_t type, imgFlags_t flags )
{
    if ( ! ( name && name[0] ) ) {
        Com_Error(ERR_FATAL,"%s no name", __func__);
        return NULL;
    }
        ...
        if ( ! strcmp ( name, image->imgName ) ) {
            ...
            if ( strcmp( name, "*white" ) ) {
                if ( image->flags != flags ) {
                    Com_Printf( "WARNING: reused image %s with mixed flags (%i vs %i)\n", name, image->flags, flags );
                }
            }
...
LINE;
...
#if 0
    imgFlags_t  checkFlagsTrue = IMGFLAG_PICMIP | IMGFLAG_MIPMAP | IMGFLAG_GENNORMALMAP;
    imgFlags_t checkFlagsFalse = IMGFLAG_CUBEMAP;

    // TODO turned off in q3config.cfg for now
    if ( qfalse && qfalse && r_normalMapping->integer && (picFormat == GL_RGBA8) && (type == IMGTYPE_COLORALPHA) && ((flags & checkFlagsTrue) == checkFlagsTrue) && !(flags & checkFlagsFalse))
    {
        assert(0);
        ...
        assert(NULL == "recursion"); // havent tested recursion yet TODO

        // TODO this recursion in oroginal code
        ...
        if (normalImage == NULL)
        {
            ...
            normalWidth = width;
            normalHeight = height;
            normalPic = malloc(width * height * 4);
    ...
    image_t *image = R_CreateImage2( name, pic, width, height, picFormat, picNumMips, type, flags, 0 );
    free(pic);
    return image;
}
...
LINE;
    ...
    if (glRefConfig.framebufferObject)
    {
        ...
        for( int x = 0; x < MAX_DRAWN_PSHADOWS; x++ ) {
            ...
            // TODO commented out in original
            // GLDSA_TextureParameterfEXT(tr.pshadowMaps[x]->texnum, GL_TEXTURE_2D, GL_TEXTURE_COMPARE_MODE, GL_COMPARE_R_TO_TEXTURE);
            // GLDSA_TextureParameterfEXT(tr.pshadowMaps[x]->texnum, GL_TEXTURE_2D, GL_TEXTURE_COMPARE_FUNC, GL_LEQUAL);
        }
        ...
        if (r_sunlightMode->integer)
        {
            ...
            for ( int x = 0; x < 4; x++ ) {
                ...
                GLDSA_TextureParameterfEXT(tr.sunShadowDepthImage[x]->texnum, GL_TEXTURE_2D, GL_TEXTURE_COMPARE_MODE, GL_COMPARE_R_TO_TEXTURE);
...
{
    if ( !name || !name[0] ) {
        Com_Printf( "Empty name passed to %s\n", __func__ );
        return 0;
    }
    ...
    if ( tr.numSkins + 1 >= MAX_SKINS ) {
        Com_Error( ERR_FATAL, "error: %s ( '%s' ) MAX_SKINS hit\n", __func__, name );
        return 0;
    }
    ...
    R_IssuePendingRenderCommands(); // TODO

    // If not a .skin file, load as a single shader
    if ( strcmp( name + strlen( name ) - 5, ".skin" ) ) {
        assert(0); // TODO
        skin->numSurfaces = 1;
        skin->surfaces = Hunk_Alloc( sizeof( skinSurface_t ), h_low );
        skin->surfaces[0].shader = R_FindShader( name, LIGHTMAP_NONE, qtrue );
        return hSkin;
    }

    int length;
    if ( ! FS_GetPreLoadedFile ( name, &text.v, &length ) ) {
        Com_Printf(">>>>>>>>> %s failed to load skin %s\n", __func__, name );
        //assert(0);
        return 0;
    }

    text.v = realloc(text.v, length + 1);
    text.c[length] = '\0';
    ...
    skin->surfaces = Hunk_Alloc( skin->numSurfaces * sizeof( skinSurface_t ), h_low );
