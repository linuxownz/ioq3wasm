...
extern unsigned int fallbackShader_bokeh_vp_len;
extern unsigned int fallbackShader_bokeh_fp_len;

extern const char fallbackShader_calclevels4x_vp[];
extern const char fallbackShader_calclevels4x_fp[];
extern unsigned int fallbackShader_calclevels4x_vp_len;
extern unsigned int fallbackShader_calclevels4x_fp_len;

extern const char fallbackShader_depthblur_vp[];
extern const char fallbackShader_depthblur_fp[];
extern unsigned int fallbackShader_depthblur_vp_len;
extern unsigned int fallbackShader_depthblur_fp_len;

extern const char fallbackShader_dlight_vp[];
extern const char fallbackShader_dlight_fp[];
extern unsigned int fallbackShader_dlight_vp_len;
extern unsigned int fallbackShader_dlight_fp_len;

extern const char fallbackShader_down4x_vp[];
extern const char fallbackShader_down4x_fp[];
extern unsigned int fallbackShader_down4x_vp_len;
extern unsigned int fallbackShader_down4x_fp_len;

extern const char fallbackShader_fogpass_vp[];
extern const char fallbackShader_fogpass_fp[];
extern unsigned int fallbackShader_fogpass_vp_len;
extern unsigned int fallbackShader_fogpass_fp_len;

extern const char fallbackShader_generic_vp[];
extern const char fallbackShader_generic_fp[];
extern unsigned int fallbackShader_generic_vp_len;
extern unsigned int fallbackShader_generic_fp_len;

extern const char fallbackShader_lightall_vp[];
extern const char fallbackShader_lightall_fp[];
extern unsigned int fallbackShader_lightall_vp_len;
extern unsigned int fallbackShader_lightall_fp_len;

extern const char fallbackShader_pshadow_vp[];
extern const char fallbackShader_pshadow_fp[];
extern unsigned int fallbackShader_pshadow_vp_len;
extern unsigned int fallbackShader_pshadow_fp_len;

extern const char fallbackShader_shadowfill_vp[];
extern const char fallbackShader_shadowfill_fp[];
extern unsigned int fallbackShader_shadowfill_vp_len;
extern unsigned int fallbackShader_shadowfill_fp_len;

extern const char fallbackShader_shadowmask_vp[];
extern const char fallbackShader_shadowmask_fp[];
extern unsigned int fallbackShader_shadowmask_vp_len;
extern unsigned int fallbackShader_shadowmask_fp_len;

extern const char fallbackShader_ssao_vp[];
extern const char fallbackShader_ssao_fp[];
extern unsigned int fallbackShader_ssao_vp_len;
extern unsigned int fallbackShader_ssao_fp_len;

extern const char fallbackShader_texturecolor_vp[];
extern const char fallbackShader_texturecolor_fp[];
extern unsigned int fallbackShader_texturecolor_vp_len;
extern unsigned int fallbackShader_texturecolor_fp_len;

extern const char fallbackShader_tonemap_vp[];
extern const char fallbackShader_tonemap_fp[];
extern unsigned int fallbackShader_tonemap_vp_len;
extern unsigned int fallbackShader_tonemap_fp_len;
...
extern int glesMajorVersion, glesMinorVersion;// TODO
static void GLSL_GetShaderHeader( GLenum shaderType, const GLchar *extra, char *dest, int size )
{
    ...
    {
        if ( glesMajorVersion >= 3 && glRefConfig.glslMajorVersion >= 3 ) {
            ...
            Com_Error(ERR_FATAL, "glslMajor version is too low");
...
{
    //printf("%s compiling %s shader\n", __func__, shaderType == GL_VERTEX_SHADER ? "vertex" : "fragment" );
    ...
    if( ! compiled ) {
        ...
        Com_Error(ERR_FATAL, "Couldn't compile shader");
        return 0;
    }
...
static int GLSL_LoadGPUShaderText(const char *name, const char *fallback, const unsigned int fallback_len, GLenum shaderType, char *dest, int destSize)
{
    ...
    if(shaderType == GL_VERTEX_SHADER) {
        Com_sprintf(filename, sizeof(filename), "%s_vp", name);
    } else {
        Com_sprintf(filename, sizeof(filename), "%s_fp", name);
    }

    if ( r_externalGLSL->integer ) { // TODO
        Com_Error ( ERR_FATAL, "r_externalGLSL set but disabled in code. unset r_externalGLSL or change code %s %d\n", __func__, __LINE__ );
assert(0);
        size = 0; //FS_GetFile(filename, (void **)&buffer); // async
    } else {
        size   = 0;
        buffer = NULL;
    }
    ...
    {
        if ( fallback ) {
            //Com_Printf("...loading built-in '%s'\n", filename); // TODO
            shaderText = fallback;
            size = fallback_len;
        } else {
            Com_Error(ERR_FATAL, "couldn't load '%s' no fallback\n", filename);
            return 0;
        }
    } else {
    ...
    if ( size > destSize ) {
        Com_Error(ERR_FATAL, "size %d > dest %d !!\n", size, destSize);
        result = 0;
    } else {
...
static void GLSL_LinkProgram(GLuint program)
{
    ...
    if ( ! glIsProgram(program) ) {
        Com_Error( ERR_FATAL, "%s %d not program %d\n", __func__, __LINE__, program);
    }
    ...
    if ( ! linked) {
        ...
        Com_Error(ERR_FATAL, "shaders failed to link");
    }
}
...
static int GLSL_InitGPUShader2(shaderProgram_t * program, const char *name, int attribs, const char *vpCode, const char *fpCode)
{
    ...
    if( strlen(name) >= MAX_QPATH ) {
        Com_Error(ERR_FATAL, "GLSL_InitGPUShader2: \"%s\" is too long", name);
    }
    ...
    {
        Com_Printf("GLSL_InitGPUShader2: Unable to load \"%s\" as GL_VERTEX_SHADER\n", name);
    ...
    if(fpCode)
    {
        ...
        {
            Com_Printf("GLSL_InitGPUShader2: Unable to load \"%s\" as GL_FRAGMENT_SHADER\n", name);
            glDeleteProgram(program->program);
            return 0;
        }
    }
...
static int GLSL_InitGPUShader(shaderProgram_t * program, const char *name, int attribs, qboolean fragmentShader, const GLchar *extra, qboolean addHeader, const char *fallback_vp, const unsigned int fallback_vp_len, const char *fallback_fp, const unsigned int fallback_fp_len)
{
    //printf("--------------- %s %s ---------\n", __func__, name );

#define MAX_VPCODE 32000
#define MAX_FPCODE 32000

    char *postHeader;
    int size = MAX_VPCODE;
    char *vpCode = malloc ( MAX_VPCODE ); // TODO this was a fix to remove char vpCode[32000]
    char *fpCode = NULL;
    int result;

    if ( NULL == vpCode ) {
        Com_Error ( ERR_FATAL, "malloc %s %d\n", __func__, __LINE__ );
    }

    memset ( vpCode, 0, MAX_VPCODE );
    ...
    if (!GLSL_LoadGPUShaderText(name, fallback_vp, fallback_vp_len, GL_VERTEX_SHADER, postHeader, size)) {
        Com_Error(ERR_FATAL,"failed to load GLSL_LoadGPUShaderText(%s)\n", name);
        return 0;
    }
    ...
    {
        size = MAX_FPCODE;
        fpCode = malloc(MAX_FPCODE); // TODO this was a fix to remove char fpCode[32000]

        if ( NULL == fpCode ) {
            Com_Error ( ERR_FATAL, "malloc %s %d\n", __func__, __LINE__ );
        }

        memset ( fpCode, 0, MAX_FPCODE );
        ...
        if (!GLSL_LoadGPUShaderText(name, fallback_fp, fallback_fp_len, GL_FRAGMENT_SHADER, postHeader, size)) {
            return 0;
        }
    }
...
static void GLSL_InitUniforms(shaderProgram_t *program)
{
    ...
    program->uniformBuffer = malloc(size);
}
...
{
    GLint *uniforms = program->uniforms;
    GLint *compare = (GLint *)(program->uniformBuffer + program->uniformBufferOffsets[uniformNum]); // TODO possible alignment fault
    ...
    {
        Com_Printf( "GLSL_SetUniformInt: wrong type for uniform %i in program %s\n", uniformNum, program->name);
        return;
    }
...
{
    GLint *uniforms = program->uniforms;
    GLfloat *compare = (GLfloat *)(program->uniformBuffer + program->uniformBufferOffsets[uniformNum]); // TODO possible alignment fault
    ...
    {
        Com_Printf("GLSL_SetUniformFloat: wrong type for uniform %i in program %s\n", uniformNum, program->name);
        return;
    }
...
{
    GLint *uniforms = program->uniforms;
    vec_t *compare = (float *)(program->uniformBuffer + program->uniformBufferOffsets[uniformNum]); // TODO possible alignment fault
    ...
    {
        Com_Printf( "GLSL_SetUniformVec2: wrong type for uniform %i in program %s\n", uniformNum, program->name);
        return;
    }
...
void GLSL_SetUniformVec3(shaderProgram_t *program, int uniformNum, const vec3_t v)
{
    ...
    {
        Com_Printf( "GLSL_SetUniformVec3: wrong type for uniform %i in program %s\n", uniformNum, program->name);
        return;
    }
...
void GLSL_SetUniformVec4(shaderProgram_t *program, int uniformNum, const vec4_t v)
{
    ...
    {
        Com_Printf( "GLSL_SetUniformVec4: wrong type for uniform %i in program %s\n", uniformNum, program->name);
        return;
    }
...
void GLSL_SetUniformFloat5(shaderProgram_t *program, int uniformNum, const vec5_t v)
{
    ...
    {
        Com_Printf( "GLSL_SetUniformFloat5: wrong type for uniform %i in program %s\n", uniformNum, program->name);
        return;
    }
...
void GLSL_SetUniformMat4(shaderProgram_t *program, int uniformNum, const mat4_t matrix)
{
    ...
    {
        Com_Printf( "GLSL_SetUniformMat4: wrong type for uniform %i in program %s\n", uniformNum, program->name);
        return;
    }
...
void GLSL_SetUniformMat4BoneMatrix(shaderProgram_t *program, int uniformNum, /*const*/ mat4_t *matrix, int numMatricies)
{
    ...
    {
        Com_Printf( "GLSL_SetUniformMat4BoneMatrix: wrong type for uniform %i in program %s\n", uniformNum, program->name);
        return;
    }

    if (numMatricies > glRefConfig.glslMaxAnimatedBones)
    {
        Com_Printf( "GLSL_SetUniformMat4BoneMatrix: too many matricies (%d/%d) for uniform %i in program %s\n", numMatricies, glRefConfig.glslMaxAnimatedBones, uniformNum, program->name);
        return;
    }
...
static void GLSL_DeleteGPUShader(shaderProgram_t *program)
{
    if(program->program)
    {
        ...
        {
            glDetachShader(program->program, program->vertexShader); // TODO GLctx.detachShader is not a function
            glDeleteShader(program->vertexShader);
        }

        if (program->fragmentShader)
        {
            glDetachShader(program->program, program->fragmentShader); // TODO GLctx.detachShader is not a function
...
void GLSL_InitGPUShaders(void)
{
    ...
    Com_Printf("------- GLSL_InitGPUShaders -------\n");
	...
	if ( glRefConfig.maxVertexAttribs > ATTR_INDEX_NORMAL2 ) {
		Com_Printf("Using GPU vertex animation\n");
		glRefConfig.gpuVertexAnimation = qtrue;
	} else {
		Com_Printf("Using CPU vertex animation\n");
    ...
    for (i = 0; i < GENERICDEF_COUNT; i++)
    {
        ...
        if (!GLSL_InitGPUShader(&tr.genericShader[i], "generic", attribs, qtrue, extradefines, qtrue, fallbackShader_generic_vp, fallbackShader_generic_vp_len, fallbackShader_generic_fp, fallbackShader_generic_fp_len))
        {
    ...
    if (!GLSL_InitGPUShader(&tr.textureColorShader, "texturecolor", attribs, qtrue, extradefines, qtrue, fallbackShader_texturecolor_vp, fallbackShader_texturecolor_vp_len, fallbackShader_texturecolor_fp, fallbackShader_texturecolor_fp_len))
    {
    ...
    for (i = 0; i < FOGDEF_COUNT; i++)
    {
        ...
        if (!GLSL_InitGPUShader(&tr.fogShader[i], "fogpass", attribs, qtrue, extradefines, qtrue, fallbackShader_fogpass_vp, fallbackShader_fogpass_vp_len, fallbackShader_fogpass_fp, fallbackShader_fogpass_fp_len))
        {
    ...
    for (i = 0; i < DLIGHTDEF_COUNT; i++)
    {
        ...
        if (!GLSL_InitGPUShader(&tr.dlightShader[i], "dlight", attribs, qtrue, extradefines, qtrue, fallbackShader_dlight_vp, fallbackShader_dlight_vp_len, fallbackShader_dlight_fp, fallbackShader_dlight_fp_len))
        {
    ...
    for (i = 0; i < LIGHTDEF_COUNT; i++)
    {
        ...
        if (!GLSL_InitGPUShader(&tr.lightallShader[i], "lightall", attribs, qtrue, extradefines, qtrue, fallbackShader_lightall_vp, fallbackShader_lightall_vp_len, fallbackShader_lightall_fp, fallbackShader_lightall_fp_len))
        {
    ...
    for (i = 0; i < SHADOWMAPDEF_COUNT; i++)
    {
        ...
        if (!GLSL_InitGPUShader(&tr.shadowmapShader[i], "shadowfill", attribs, qtrue, extradefines, qtrue, fallbackShader_shadowfill_vp, fallbackShader_shadowfill_vp_len, fallbackShader_shadowfill_fp, fallbackShader_shadowfill_fp_len))
        {
    ...
    if (!GLSL_InitGPUShader(&tr.pshadowShader, "pshadow", attribs, qtrue, extradefines, qtrue, fallbackShader_pshadow_vp, fallbackShader_pshadow_vp_len, fallbackShader_pshadow_fp, fallbackShader_pshadow_fp_len))
    {
    ...
    if ( ! GLSL_InitGPUShader(&tr.down4xShader, "down4x", attribs, qtrue, extradefines, qtrue, fallbackShader_down4x_vp, fallbackShader_down4x_vp_len, fallbackShader_down4x_fp, fallbackShader_down4x_fp_len ) )
    {
    ...
    if ( ! GLSL_InitGPUShader(&tr.bokehShader, "bokeh", attribs, qtrue, extradefines, qtrue, fallbackShader_bokeh_vp, fallbackShader_bokeh_vp_len, fallbackShader_bokeh_fp, fallbackShader_bokeh_fp_len ) )
    {
    ...
    if ( ! GLSL_InitGPUShader(&tr.tonemapShader, "tonemap", attribs, qtrue, extradefines, qtrue, fallbackShader_tonemap_vp, fallbackShader_tonemap_vp_len, fallbackShader_tonemap_fp, fallbackShader_tonemap_fp_len ) )
    {
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if ( ! GLSL_InitGPUShader(&tr.calclevels4xShader[i], "calclevels4x", attribs, qtrue, extradefines, qtrue, fallbackShader_calclevels4x_vp, fallbackShader_calclevels4x_vp_len, fallbackShader_calclevels4x_fp, fallbackShader_calclevels4x_fp_len ) )
        {
	    ...
	    || glRefConfig.shadowSamplers)
	{
	    ...
	    if (glesMajorVersion < 3 && glRefConfig.shadowSamplers)
	    {
        ...
        if (!GLSL_InitGPUShader(&tr.shadowmaskShader, "shadowmask", attribs, qtrue, extradefines, qtrue, fallbackShader_shadowmask_vp, fallbackShader_shadowmask_vp_len, fallbackShader_shadowmask_fp, fallbackShader_shadowmask_fp_len))
        {
	    ...
	    || glRefConfig.standardDerivatives)
	{
	    ...
	    if (glesMajorVersion < 3 && glRefConfig.standardDerivatives)
	    {
		    Q_strcat(extradefines, 1024, "#extension GL_OES_standard_derivatives : enable\n");
	    }

        if (!GLSL_InitGPUShader(&tr.ssaoShader, "ssao", attribs, qtrue, extradefines, qtrue, fallbackShader_ssao_vp, fallbackShader_ssao_vp_len, fallbackShader_ssao_fp, fallbackShader_ssao_fp_len))
        {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            if (!GLSL_InitGPUShader(&tr.depthBlurShader[i], "depthBlur", attribs, qtrue, extradefines, qtrue, fallbackShader_depthblur_vp, fallbackShader_depthblur_vp_len, fallbackShader_depthblur_fp, fallbackShader_depthblur_fp_len))
            {
    ...
    Com_Printf("loaded %i GLSL shaders (%i gen %i light %i etc) in %5.2f seconds\n",
...
void GLSL_ShutdownGPUShaders(void)
{
    ...
    Com_Printf("------- GLSL_ShutdownGPUShaders -------\n");
...
shaderProgram_t *GLSL_GetGenericShaderProgram(int stage)
{
    ...
    // TODO check for more
    // TODO check for more
    shaderStage_t *pStage = tess.xstages[stage]; // TODO check that this is correct.  see tr_shade.c:1047
    int shaderAttribs = 0;
