...
void R_PerformanceCounters( void ) {
    ...
    if (r_speeds->integer == 1) {
        Com_Printf ("%i/%i/%i shaders/batches/surfs %i leafs %i verts %i/%i tris %.2f mtex %.2f dc\n",
    ...
    {
        Com_Printf( "VAO draws: static %i dynamic %i\n",
            backEnd.pc.c_staticVaoDraws, backEnd.pc.c_dynamicVaoDraws);
        Com_Printf( "GLSL binds: %i  draws: gen %i light %i fog %i dlight %i\n",
...
// original has no check
    if ( ! backEndData ) {
        Com_Error(ERR_FATAL,"backEndData is NULL");
    }

    renderCommandList_t *cmdList = &backEndData->commands;
    if ( NULL == cmdList ) {
        Com_Error(ERR_FATAL,"%s cmdList is NULL", __func__);
        assert(cmdList); // failing here? TODO
    }

    // add an end-of-list command
    *(int *)(cmdList->cmds + cmdList->used) = RC_END_OF_LIST; // TODO possible alignment fault
...
void RE_BeginFrame( stereoFrame_t stereoFrame ) {
    ...
    {
        if ( glesMajorVersion >= 1 && !glRefConfig.readStencil )
        {
            Com_Printf( "OpenGL ES needs GL_NV_read_stencil to read stencil bits to measure overdraw\n" );
    ...
    if ( r_textureMode->modified ) {
        R_IssuePendingRenderCommands();
        GL_TextureMode( r_textureMode->string ); // TODO is this set and is it correct? TODO
    ...
    if ( ! r_ignoreGLErrors->integer )
    {
        ...
        if ((err = glGetError()) != GL_NO_ERROR) {
            Com_Error(ERR_FATAL, "RE_BeginFrame() - glGetError() failed (0x%x)!", err);
        }
    }
    ...
    {
        if(qfalse && r_anaglyphMode->integer) // 3D classes red/blue  qfalse
        {
            if(r_anaglyphMode->modified)
            {
                ...
                GLenum frontbuf[] = { GL_FRONT };
                glDrawBuffers (1, frontbuf);
                glClear(GL_COLOR_BUFFER_BIT);
            ...
            else if(stereoFrame == STEREO_RIGHT)
            {
                ...
                Com_Error( ERR_FATAL, "%s: Stereo is enabled, but stereoFrame was %i", __func__, stereoFrame );
            }
            ...
            if(stereoFrame != STEREO_CENTER) {
                Com_Error( ERR_FATAL, "%s: Stereo is disabled, but stereoFrame was %i", __func__, stereoFrame );
            }
        ...
        if(cmd)
        {
            ...
            if(qfalse && r_anaglyphMode->modified)
            {
                glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
