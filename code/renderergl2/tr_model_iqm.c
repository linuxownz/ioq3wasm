...
qboolean R_LoadIQM( model_t *mod, void *buffer, int filesize, const char *mod_name ) {
    ...
    if ( header->num_meshes )
    {
        ...
        vertexarray = (iqmVertexArray_t *)((byte *)header + header->ofs_vertexarrays); // TODO possible alignment fault
        for( i = 0; i < header->num_vertexarrays; i++, vertexarray++ ) {
            ...
            case IQM_FLOAT:
                ...
                intPtr = (int *)((byte *)header + vertexarray->offset); // TODO possible alignment fault
            ...
            case IQM_BLENDWEIGHTS:
                ...
                if( vertexarray->format == IQM_FLOAT ) {
                    blendWeights.f = (float*)( (byte*)header + vertexarray->offset ); // TODO possible alignment fault
                } else {
        ...
        if( vertexArrayFormat[IQM_POSITION] == -1 || vertexArrayFormat[IQM_NORMAL] == -1 || vertexArrayFormat[IQM_TEXCOORD] == -1 ) {
            Com_Printf( "R_LoadIQM: %s is missing IQM_POSITION, IQM_NORMAL, and/or IQM_TEXCOORD array.\n", mod_name );
            return qfalse;
        }

        if( header->num_joints ) {
            if( vertexArrayFormat[IQM_BLENDINDEXES] == -1 || vertexArrayFormat[IQM_BLENDWEIGHTS] == -1 ) {
                Com_Printf( "R_LoadIQM: %s is missing IQM_BLENDINDEXES and/or IQM_BLENDWEIGHTS array.\n", mod_name );
                return qfalse;
            }
        } else {
        ...
        if( vertexArrayFormat[IQM_TANGENT] == -1 ) {
            Com_Printf( "R_LoadIQM: %s is missing IQM_TANGENT array.\n", mod_name );
            return qfalse;
        }
        ...
        triangle = (iqmTriangle_t *)((byte *)header + header->ofs_triangles); // TODO possible alignment fault
        ...
        mesh = (iqmMesh_t *)((byte *)header + header->ofs_meshes); // TODO possible alignment fault
        for( i = 0; i < header->num_meshes; i++, mesh++) {
            ...
            if ( mesh->num_vertexes >= SHADER_MAX_VERTEXES ) {
                Com_Printf( "R_LoadIQM: %s has more than %i verts on %s (%i).\n",
                      mod_name, SHADER_MAX_VERTEXES - 1, meshName[0] ? meshName : "a surface",
                      mesh->num_vertexes );
                return qfalse;
            }
            if ( mesh->num_triangles*3 >= SHADER_MAX_INDEXES ) {
                Com_Printf( "R_LoadIQM: %s has more than %i triangles on %s (%i).\n",
            ...
            if( header->num_joints ) {
                for( j = 0; j < mesh->num_vertexes; j++ ) {
                    ...
                    for( k = 0; k < j; k++ ) {
                        ...
                        if( *(int*)&blendIndexes[4*influence] != *(int*)&blendIndexes[4*vtx] ) { // TODO possible alignment fault
                            continue;
                        }
                        ...
                        if( vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT ) {
                            ...
                            if ( *(int*)&blendWeights.b[4*influence] == *(int*)&blendWeights.b[4*vtx] ) { // TODO possible alignment fault
                                break;
                            }
                        }
                    }
    ...
    if( header->num_poses != header->num_joints && header->num_poses != 0 ) {
        Com_Printf("R_LoadIQM: %s has %d poses and %d joints, must have the same number or 0 poses\n", mod_name, header->num_poses, header->num_joints );
        return qfalse;
    }
    ...
    if ( header->num_joints )
    {
        ...
        joint = (iqmJoint_t *)((byte *)header + header->ofs_joints); // TODO possible alignment fault
    ...
    if ( header->num_poses )
    {
        ...
        pose = (iqmPose_t *)((byte *)header + header->ofs_poses); // TODO possible alignment fault
    ...
    if (header->ofs_bounds)
    {
        ...
        bounds = (iqmBounds_t *) ((byte *) header + header->ofs_bounds); // TODO possible alignment fault
    ...
    if( header->num_meshes ) {
        iqmData->surfaces = (struct srfIQModel_s*)dataPtr; // TODO possible alignment fault
        dataPtr += header->num_meshes * sizeof( srfIQModel_t );

        iqmData->triangles = (int*)dataPtr; // TODO possible alignment fault
        dataPtr += header->num_triangles * 3 * sizeof(int);     // triangles

        iqmData->positions = (float*)dataPtr; // TODO possible alignment fault
        dataPtr += header->num_vertexes * 3 * sizeof(float);    // positions

        iqmData->texcoords = (float*)dataPtr; // TODO possible alignment fault
        dataPtr += header->num_vertexes * 2 * sizeof(float);    // texcoords

        iqmData->normals = (float*)dataPtr; // TODO possible alignment fault
        dataPtr += header->num_vertexes * 3 * sizeof(float);    // normals

        if ( vertexArrayFormat[IQM_TANGENT] != -1 ) {
            iqmData->tangents = (float*)dataPtr; // TODO possible alignment fault
        ...
        if ( allocateInfluences ) {
            iqmData->influences = (int*)dataPtr; // TODO possible alignment fault
            ...
            if( vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_UBYTE ) {
                ...
                iqmData->influenceBlendWeights.f = (float*)dataPtr; // TODO possible alignment fault
    ...
    if( header->num_joints ) {
        ...
        iqmData->jointParents = (int*)dataPtr; // TODO possible alignment fault
        dataPtr += header->num_joints * sizeof(int);        // joint parents

        iqmData->bindJoints = (float*)dataPtr; // TODO possible alignment fault
        dataPtr += header->num_joints * 12 * sizeof(float);     // bind joint matricies

        iqmData->invBindJoints = (float*)dataPtr; // TODO possible alignment fault
        dataPtr += header->num_joints * 12 * sizeof(float);     // inverse bind joint matricies
    }
    if( header->num_poses ) {
        iqmData->poses = (iqmTransform_t*)dataPtr; // TODO possible alignment fault
        dataPtr += header->num_poses * header->num_frames * sizeof(iqmTransform_t); // pose transforms
    }
    if( header->ofs_bounds ) {
        iqmData->bounds = (float*)dataPtr; // TODO possible alignment fault
            ...
            for( i = 0; i < header->num_meshes; i++, surface++ ) {
                ...
                for( j = 0; j < surface->num_vertexes; j++ ) {
                    ...
                    for( k = 0; k < surface->num_influences; k++ ) {
                        ...
                        if( *(int*)&iqmData->influenceBlendIndexes[4*influence] != *(int*)&blendIndexes[4*vtx] ) { // TODO possible alignment fault
                            continue;
                        }
                        ...
                        if( vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT ) {
                            ...
                            if ( *(int*)&iqmData->influenceBlendWeights.b[4*influence] == *(int*)&blendWeights.b[4*vtx] ) { // TODO possible alignment fault
                                break;
                            }
                        }
                    }
    ...
    if( header->num_poses )
    {
        ...
        framedata = (unsigned short *)((byte *)header + header->ofs_frames); // TODO possible alignment fault
        for( i = 0; i < header->num_frames; i++ ) {
            pose = (iqmPose_t *)((byte *)header + header->ofs_poses); // TODO possible alignment fault
    ...
    if ( iqmData->num_surfaces && iqmData->num_joints <= glRefConfig.glslMaxAnimatedBones )
    {
        ...
        iqmData->vaoSurfaces = Hunk_Alloc(sizeof(*iqmData->vaoSurfaces) * iqmData->numVaoSurfaces, h_low);
        ...
        for (i = 0; i < iqmData->num_surfaces; i++, vaoSurf++, surf++)
        {
            ...
            for ( j = 0; j < surf->num_vertexes; j++ )
            {
                ...
                // normal
                R_VaoPackNormal((int16_t*)(data + dataOfs), &iqmData->normals[vtx*3]); // TODO possible alignment fault
                dataOfs += sizeof(int16_t) * 4;

                // tangent
                R_VaoPackTangent((int16_t*)(data + dataOfs), &iqmData->tangents[vtx*4]); // TODO possible alignment fault
                dataOfs += sizeof(int16_t) * 4;
...
void R_AddIQMSurfaces( trRefEntity_t *ent ) {
    ...
    if ( (ent->e.frame >= data->num_frames)
         ...
         || (ent->e.oldframe >= data->num_frames)
         || (ent->e.oldframe < 0) ) {
        Com_Printf( "R_AddIQMSurfaces: no such frame %d to %d for '%s'\n",
