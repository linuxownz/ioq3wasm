...
void GL_State( unsigned long stateBits )
{
    ...
    if ( diff & ( GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS ) )
    {
        ...
        if (newState != 0 && storedState != newState)
        {
            ...
            case GLS_DSTBLEND_ONE_MINUS_DST_ALPHA:
                ...
                break;
            default:
                Com_Printf("GL_State: invalid dst blend state bits" ); assert(0);
    ...
    //
#if 0
    // glPolygonMode not supported in openGL ES2.0
...
#if 0
        // if'd out in original source code ....
        float   plane[4];
        GLdouble    plane2[4];
...
void RE_UploadCinematic (int w, int h, int cols, int rows, const byte *data, int client, qboolean dirty) {
    ...
    if ( cols != tr.scratchImage[client]->width || rows != tr.scratchImage[client]->height ) {
        ...
        // TODO qglTextureImage2DEXT(texture, GL_TEXTURE_2D, 0, GL_RGB8, cols, rows, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
        glTextureImage2D(texture, GL_TEXTURE_2D, 0, GL_RGB8, cols, rows, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);

        // TODO changed qgl to gl
        glTexParameterf(texture, GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameterf(texture, GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
        glTexParameterf(texture, GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameterf(texture, GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);

    } else {
        if (dirty) {
            // otherwise, just subimage upload it so that drivers can tell we are going to be changing
            // it and don't try and do a texture compression
            // TODO qglTextureSubImage2DEXT(texture, GL_TEXTURE_2D, 0, 0, 0, cols, rows, GL_RGBA, GL_UNSIGNED_BYTE, data);
...
const void  *RB_DrawSurfs( const void *data ) {
    ...
    if (glRefConfig.framebufferObject && !(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) && (r_depthPrepass->integer || isShadowView))
    {
        ...
        if (!isShadowView)
        {
            ...
            else if (tr.renderFbo == NULL && tr.renderDepthImage)
            {
                ...
                // tr.renderDepthImage is no good
    ...
    if (!isShadowView)
    {
        ...
        if (glRefConfig.framebufferObject && r_drawSunRays->integer)
        {
            ...
            if (glRefConfig.occlusionQuery)
            {
                ...
                glBeginQuery(glRefConfig.occlusionQueryTarget, tr.sunFlareQuery[tr.sunFlareQueryIndex]);
            }
            ...
            if (glRefConfig.occlusionQuery)
            {
                glEndQuery(glRefConfig.occlusionQueryTarget);
            }
    ...
    if (glRefConfig.framebufferObject && tr.renderCubeFbo && backEnd.viewParms.targetFbo == tr.renderCubeFbo)
    {
        ...
        if (cubemap && cubemap->image) {
            assert(0);
            Com_Printf("%s qglGenerateTextureMipmapEXT skipping..\n", __func__);
...
*/
#if 0
static const char *RenderCmdToString( int cmd ) {

    switch ( cmd ) {
        case RC_CAPSHADOWMAP:    return "RC_CAPSHADOWMAP:";
        case RC_CLEARDEPTH:      return "RC_CLEARDEPTH:";
        case RC_COLORMASK:       return "RC_COLORMASK:";
        case RC_DRAW_BUFFER:     return "RC_DRAW_BUFFER:";
        case RC_DRAW_SURFS:      return "RC_DRAW_SURFS:";
        case RC_END_OF_LIST:     return "RC_END_OF_LIST:";
        case RC_EXPORT_CUBEMAPS: return "RC_EXPORT_CUBEMAPS:";
        case RC_POSTPROCESS:     return "RC_POSTPROCESS:";
        case RC_SCREENSHOT:      return "RC_SCREENSHOT:";
        case RC_SET_COLOR:       return "RC_SET_COLOR:";
        case RC_STRETCH_PIC:     return "RC_STRETCH_PIC:";
        case RC_SWAP_BUFFERS:    return "RC_SWAP_BUFFERS:";
        case RC_VIDEOFRAME:      return "RC_VIDEOFRAME:";
    }
    return "??";
}

static const char *GL_bufferEnumToString(int e) {
    switch ( e ) {
...
static const void *RB_DrawBuffer( const void *data ) {
    ...
    //Com_Printf("%s commandId: %s\n", __func__, RenderCmdToString(cmd->commandId));
    //Com_Printf("%s buffer   : %s\n", __func__, GL_bufferEnumToString(cmd->buffer)); // GL_FRONT, etc

    if ( (int)cmd->buffer != GL_BACK ) {
        ///Com_Printf("ERROR: cmd buffer is NOT GL_FRONT\n");
    }

    GLenum glcmd[] = { cmd->buffer };
    int numcmds = sizeof(glcmd)/sizeof(glcmd[0]);

    //emscripten_glDrawBuffersWEBGL( 1, (const GLenum *)cmd->buffer );
    emscripten_glDrawBuffersWEBGL( numcmds, glcmd );
...
===============
*/
void RB_ShowAllShaderImages( void ) {
    int     i;
    image_t *image;
    float   x, y, w, h;
    int     start, end;
    ...
    for ( i=0 ; i<tr.numShaders ; i++ ) {
        ...
        for ( int j = 0 ; j < shader->numUnfoggedPasses ; j++ ) {
            shaderStage_t *stage = shader->stages[j];

            if ( stage->active ) {

                for ( int k = 0 ; k < NUM_TEXTURE_BUNDLES ; k++ ) {
                    textureBundle_t *bundle = &stage->bundle[k];

                    for ( int m = 0 ; m < MAX_IMAGE_ANIMATIONS ; m++ ) {
                        image_t * image = bundle->image[m];
...
Draw the default image to the screen, on top of whatever
was there.  This is used to test for texture thrashing.

===============
*/
void RB_ShowDefaultImage( void ) {
    int     i;
    image_t *image;
    float   x, y, w, h;
    int     start, end;
    ...
    int defaultImageNumber = Cvar_Get("defaultImageNumber", "33", 0)->integer;

    if ( defaultImageNumber > 0 ) {
        image = tr.images[defaultImageNumber];
    } else {
    ...
    w = glConfig.vidWidth / 2;
    h = glConfig.vidHeight / 1.5;
...
const void  *RB_SwapBuffers( const void *data ) {
    ...
    extern cvar_t* r_showDefaultImage;
    extern cvar_t* r_showAllShaderImages;

    // texture swapping test
    if ( r_showImages->integer ) {
        RB_ShowImages();
    } else if ( r_showDefaultImage->integer ) {
        RB_ShowDefaultImage();
    } else if ( r_showAllShaderImages->integer ) {
        RB_ShowAllShaderImages();
    }
...
const void *RB_CapShadowMap(const void *data)
{
    ...
    if (cmd->map != -1)
    {
        if (cmd->cubeSide != -1)
        {
            ...
            {
                // TODO qglCopyTextureSubImage2DEXT(tr.shadowCubemaps[cmd->map]->texnum, GL_TEXTURE_CUBE_MAP_POSITIVE_X + cmd->cubeSide, 0, 0, 0, backEnd.refdef.x, glConfig.vidHeight - ( backEnd.refdef.y + PSHADOW_MAP_SIZE ), PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE);
        ...
        else
        {
            ...
            {
                // TODO qglCopyTextureSubImage2DEXT(tr.pshadowMaps[cmd->map]->texnum, GL_TEXTURE_2D, 0, 0, 0, backEnd.refdef.x, glConfig.vidHeight - (backEnd.refdef.y + PSHADOW_MAP_SIZE), PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE);
...
const void *RB_PostProcess(const void *data)
{
	...
	if (1) { // TODO set to 1 in original source
    ...
    // ALL OF THEM are disabled in the original
    if (0)
    {
        vec4_t quadVerts[4];
        vec2_t texCoords[4];
...
const void *RB_ExportCubemaps(const void *data)
{
    ...
    if (cmd)
    {
        ...
        byte *cubemapPixels = malloc(sideSize * 6);
        int i, j;
        ...
        for (i = 0; i < tr.numCubemaps; i++)
        {
            ...
            Com_Printf("Saved cubemap %d as %s\n", i, filename);
        }
...
void RB_ExecuteRenderCommands( const void *data ) {
    ...
    while ( 1 ) {
        ...
        switch ( *(const int *)data ) {
            ...
            case RC_SCREENSHOT:
                Com_Error(ERR_FATAL, "RC_SCREENSHOT");
                data = RB_TakeScreenshotCmd( data );
                break;

            case RC_VIDEOFRAME:
                Com_Error(ERR_FATAL, "RC_VIDEOFRAME");
            ...
            case RC_EXPORT_CUBEMAPS:
                Com_Error(ERR_FATAL, "RC_EXPORT_CUBEMAPS"); // TODO have to advance data pointer if this happens
