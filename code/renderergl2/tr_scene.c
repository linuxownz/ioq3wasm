...
void RE_AddPolyToScene( qhandle_t hShader, int numVerts, const polyVert_t *verts, int numPolys ) {
    ...
    if ( !hShader ) {
        ...
        // the default shader.
        //Com_Printf( "WARNING: RE_AddPolyToScene: NULL poly shader\n");
        //return;
    }
    ...
    for ( j = 0; j < numPolys; j++ ) {
      ...
      */
            Com_Printf( "WARNING: RE_AddPolyToScene: r_max_polys or r_max_polyverts reached\n");
            return;
        }
...
void RE_AddRefEntityToScene( const refEntity_t *ent ) {
    ...
    if ( r_numentities >= MAX_REFENTITIES ) {
        Com_Printf("RE_AddRefEntityToScene: Dropping refEntity, reached MAX_REFENTITIES\n");
        return;
    }
    if ( Q_isnan(ent->origin[0]) || Q_isnan(ent->origin[1]) || Q_isnan(ent->origin[2]) ) {
        ...
        if (firstTime) {
            firstTime = qfalse;
            Com_Printf( "RE_AddRefEntityToScene passed a refEntity which has an origin with a NaN component\n");
        }
        return;
    }
    if ( (int)ent->reType < 0 || ent->reType >= RT_MAX_REF_ENTITY_TYPE ) {
        Com_Error( ERR_FATAL, "RE_AddRefEntityToScene: bad reType %i", ent->reType );
    }
...
void RE_BeginScene(const refdef_t *fd)
{
    ...
    if ( ! ( tr.refdef.rdflags & RDF_NOWORLDMODEL ) ) {
        ...
        for (i = 0 ; i < MAX_MAP_AREA_BYTES/4 ; i++) {
            areaDiff |= ((int *)tr.refdef.areamask)[i] ^ ((int *)fd->areamask)[i]; // TODO possible alignment fault
            ((int *)tr.refdef.areamask)[i] = ((int *)fd->areamask)[i]; // TODO possible alignment fault
        }
...
void RE_RenderScene( const refdef_t *fd ) {
    ...
    if ( ! tr.world && ! ( fd->rdflags & RDF_NOWORLDMODEL ) ) {
        Com_Error (ERR_FATAL, "R_RenderScene: NULL worldmodel");
    }
