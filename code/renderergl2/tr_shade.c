...
static void R_BindAnimatedImageToTMU( textureBundle_t *bundle, int tmu ) {
    ...
    if ( bundle->isVideoMap ) {
        ...
        // ri.CIN_RunCinematic(bundle->videoMapHandle); // TODO
...
void RB_BeginSurface( shader_t *shader, int fogNum, int cubemapIndex ) {
    ...
    if ( ! shader ) {
        // TODO bot holding flag ....
        // TODO bot holding flag ....
        // TODO bot holding flag ....
        //Com_Printf("%s no shader\n", __func__);
        return;
        assert(shader);
    }
    shader_t *state = (shader->remappedShader) ? shader->remappedShader : shader;

    assert(state->optimalStageIteratorFunc);
...
static void ForwardDlight( void ) {
    ...
    for ( int l = 0 ; l < backEnd.refdef.num_dlights ; l++ ) {
        ...
        if ( !( tess.dlightBits & ( 1 << l ) ) ) {
            continue;   // this surface definitely doesn't have any of this light comment from original source
        }

        dl = &backEnd.refdef.dlights[l];
        //VectorCopy( dl->transformed, origin );comment from original source
        radius = dl->radius;
        //scale = 1.0f / radius;comment from original source

        //if (pStage->glslShaderGroup == tr.lightallShader)comment from original source
        {
...
static void ProjectPshadowVBOGLSL( void ) {
    ...
    for ( int l = 0 ; l < backEnd.refdef.num_pshadows ; l++ ) {
        ...
        backEnd.pc.c_totalIndexes += tess.numIndexes;
        //backEnd.pc.c_dlightIndexes += tess.numIndexes; // commented out in original
    }
}
...
static void RB_IterateStagesGeneric( shaderCommands_t *input )
{
    ...
    for ( int stage = 0; stage < MAX_SHADER_STAGES; stage++ )
    {
        ...
        if ( !pStage || !pStage->active) { // TODO I think only active stages are copied from the global into the shader
            break;
        }
        ...
        //GLSL_SetUniformFloat(sp, UNIFORM_MAPLIGHTSCALE, backEnd.refdef.mapLightScale); // commented out in original
        ...
        else if ( pStage->bundle[0].image[0] != 0 )
        {
            //
            // set state
            //
...
static void RB_RenderShadowmap( shaderCommands_t *input )
{
...

    {
        ...
        // do multitexture
        //
        //if ( pStage->glslShaderGroup ) // commented out in original
        {
            //
            // draw
            //
...
void RB_StageIteratorGeneric( void )
{
    ...
    input = &tess; // TODO problem starts here ... tess->shader stages are no good
...
void RB_EndSurface( void ) {
    ...
    //
    assert(tess.currentStageIteratorFunc);
