...
static qboolean R_CheckFBO(const FBO_t * fbo)
{
    ...
    switch (code)
    {
        case GL_FRAMEBUFFER_UNSUPPORTED:
            Com_Printf("%s: (%s) Unsupported framebuffer format\n", __func__, fbo->name);
            break;

        case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
            Com_Printf("%s: (%s) Framebuffer incomplete attachment\n", __func__, fbo->name);
            break;

        case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
            Com_Printf("%s: (%s) Framebuffer incomplete, missing attachment\n", __func__, fbo->name);
            break;

        case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
            Com_Printf("%s: (%s) Framebuffer incomplete, missing draw buffer\n", __func__, fbo->name);
            break;

        case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
            Com_Printf("%s: (%s) Framebuffer incomplete, missing read buffer\n", __func__, fbo->name);
            break;

        case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
            Com_Printf("%s: (%s) Framebuffer incomplete multisample\n", __func__, fbo->name);
            break;

        default:
            Com_Printf("%s: (%s) unknown error 0x%X\n", __func__, fbo->name, code);
            break;
    }
...
static FBO_t *FBO_Create(const char *name, int width, int height)
{
    ...
    if(width <= 0 || width > glRefConfig.maxRenderbufferSize) {
        Com_Error(ERR_DROP, "FBO_Create(%s): bad width %i", name, width);
    }

    if(height <= 0 || height > glRefConfig.maxRenderbufferSize) {
        Com_Error(ERR_DROP, "FBO_Create(%s): bad height %i", name, height);
    }
    ...
    fbo = tr.fbos[tr.numFBOs] = Hunk_Alloc(sizeof(*fbo), h_low);
...
static void FBO_CreateBuffer(FBO_t *fbo, int format, int index, int multisample)
{
    ...
    switch(format)
    {
        ...
        default:
            Com_Printf("FBO_CreateBuffer: invalid format %d\n", format);
            return;
    }
    ...
    if (multisample && glRefConfig.framebufferMultisample) {
        ...
        //glNamedRenderbufferStorage(*pRenderBuffer, format, fbo->width, fbo->height); // depends on direct state access
    }
...
void FBO_Bind(FBO_t * fbo)
{
    ...
    {
        Com_Printf("FBO_Bind() called without framebuffers enabled!\n");
        return;
    }
...
void R_FBOList_f(void)
{
    ...
    if( ! glRefConfig.framebufferObject ) { // TODO why????
        Com_Error(ERR_FATAL, "GL_EXT_framebuffer_object is not available. why?????\n");
        return;
    }
...
void FBO_BlitFromTexture(struct image_s *src, vec4_t inSrcTexCorners, vec2_t inSrcTexScale, FBO_t *dst, ivec4_t inDstBox, struct shaderProgram_s *shaderProgram, vec4_t inColor, int blend)
{
    ...
    if (!src)
    {
        Com_Printf("Tried to blit from a NULL texture!\n");
        return;
    }
...
void FBO_Blit(FBO_t *src, ivec4_t inSrcBox, vec2_t srcTexScale, FBO_t *dst, ivec4_t dstBox, struct shaderProgram_s *shaderProgram, vec4_t color, int blend)
{
    ...
    if (!src)
    {
        Com_Printf("Tried to blit from a NULL FBO!\n");
        return;
    }
