...
long    FS_GetFile(const char *filename, void **buffer);
...
void R_LoadDDS ( const char *filename, byte **pic, int *width, int *height, GLenum *picFormat, int *numMips )
{
    ...
    if (!picFormat)
    {
        Com_Printf("R_LoadDDS() called without picFormat parameter!");
        return;
    }
    ...
    *pic = NULL;
    ...
    // load the file
    //
    len = FS_GetFile( ( char * ) filename, &buffer.v);
    if (!buffer.b || len < 0) {
        return;
    }
    ...
    {
        Com_Printf("File %s is too small to be a DDS file.\n", filename);
        free ( buffer.v );
        buffer.v = NULL;
        return;
    }
    ...
    //
    if (*((ui32_t *)(buffer.b)) != EncodeFourCC("DDS ")) // TODO possible alignment fault
    {
        Com_Printf("File %s is not a DDS file.\n", filename);
        free ( buffer.v );
        buffer.v = NULL;
        return;
    }
    ...
    //
    ddsHeader = (ddsHeader_t *)(buffer.b + 4); // TODO possible alignment fault
    if ((ddsHeader->pixelFormatFlags & DDSPF_FOURCC) && ddsHeader->fourCC == EncodeFourCC("DX10"))
    {
        ...
        {
            Com_Printf("File %s indicates a DX10 header it is too small to contain.\n", filename);
            free ( buffer.v );
            buffer.v = NULL;
            return;
        }

        ddsHeaderDxt10 = (ddsHeaderDxt10_t *)(buffer.b + 4 + sizeof(ddsHeader_t)); // TODO possible alignment fault
    ...
    if (ddsHeaderDxt10)
    {
        switch (ddsHeaderDxt10->dxgiFormat)
        {
            ...
            default:
                Com_Printf("DDS File %s has unsupported DXGI format %d.", filename, ddsHeaderDxt10->dxgiFormat);
                free ( buffer.v );
                buffer.v = NULL;
                return;
                break;
    ...
    else
    {
        if (ddsHeader->pixelFormatFlags & DDSPF_FOURCC)
        {
            ...
            {
                Com_Printf("DDS File %s has unsupported FourCC.", filename);
                free ( buffer.v );
                buffer.v = NULL;
                return;
            }
        ...
        {
            Com_Printf("DDS File %s has unsupported RGBA format.", filename);
            free ( buffer.v );
            buffer.v = NULL;
            return;
        }
...
void R_SaveDDS(const char *filename, byte *pic, int width, int height, int depth)
{
    ...
    ddsHeader = (ddsHeader_t *)(data + 4); // TODO possible alignment fault
    ...
    //ri.FS_WriteFile(filename, data, size); // TODO
