...
static qhandle_t R_RegisterMD3(const char *filename, model_t *mod)
{
    assert(mod); assert(filename);
    ...
    qboolean loading = qtrue;
    int lod;
    int numLoaded = 0;

    for (lod = MD3_MAX_LODS - 1 ; lod >= 0 ; lod--)
    {
        char namebuf[MAX_QPATH] = { 0 };

        if ( lod ) {
            Com_sprintf(namebuf, sizeof(namebuf), "%s_%d.md3", buffer, lod);
        } else {
            Com_sprintf(namebuf, sizeof(namebuf), "%s.md3", buffer);
        }
        ...
        // need to pass lod mod and handle
        if ( ! FS_GetPreLoadedFile( namebuf, &buf.v, &size ) ) { // binary file
            //Com_Printf("%s Failed to load %s\n", __func__, namebuf);
            continue;
            //assert(0);
        }
        ...
        if (ident == MD3_IDENT) {
            loaded = R_LoadMD3(mod, lod, buf.u, size, namebuf);
        } else {
            continue;
            // TODO set modelinfo data
            Com_Printf ("ERROR loading model\n");
            HexDumpData(buf.v, size);
            Com_Error(ERR_FATAL, "%s: unknown model ident %d %s\n", __func__, ident, namebuf);
        }
...
static qboolean R_LoadMD3(model_t * mod, const int lod, const void *buffer, const int bufferSize, const char *modName)
{
    ...
    mod->dataSize += size;
    mdvModel = mod->mdv[lod] = Hunk_Alloc(sizeof(mdvModel_t), h_low);
...
//  Com_Memcpy(mod->md3[lod], buffer, LittleLong(md3Model->ofsEnd));
    ...
    mdvModel->frames = frame = Hunk_Alloc(sizeof(*frame) * md3Model->numFrames, h_low);
    ...
    mdvModel->tags = tag = Hunk_Alloc(sizeof(*tag) * (md3Model->numTags * md3Model->numFrames), h_low);
    ...
    mdvModel->tagNames = tagName = Hunk_Alloc(sizeof(*tagName) * (md3Model->numTags), h_low);
    ...
    mdvModel->surfaces = surf = Hunk_Alloc(sizeof(*surf) * md3Model->numSurfaces, h_low);
    ...
    for(i = 0; i < md3Model->numSurfaces; i++)
    {
        ...
        {
            Com_Printf("%s: %s has more than %i verts on %s (%i).\n",
                __func__, modName, SHADER_MAX_VERTEXES - 1, md3Surf->name[0] ? md3Surf->name : "a surface",
                md3Surf->numVerts );
            assert(0);
            return qfalse;
        }
        ...
        {
            Com_Printf("%s: %s has more than %i triangles on %s (%i).\n",
                __func__, modName, ( SHADER_MAX_INDEXES / 3 ) - 1, md3Surf->name[0] ? md3Surf->name : "a surface",
        ...
        surf->shaderIndexes = shaderIndex = Hunk_Alloc(sizeof(*shaderIndex) * md3Surf->numShaders, h_low);
        ...
        for(j = 0; j < md3Surf->numShaders; j++, shaderIndex++, md3Shader++)
        {
            ...
            if ( ! sh->finished ) {
                Com_Printf("%s shader not finished '%s'\n", __func__, md3Shader->name);
                //assert(sh->finished);
            }
            ...
            if(sh->defaultShader) {
                *shaderIndex = 0;
                ...
                //Com_Printf("found shader for model %s\n", sh->name);
        ...
        surf->indexes = tri = Hunk_Alloc(sizeof(*tri) * 3 * md3Surf->numTriangles, h_low);

        md3Tri = (md3Triangle_t *) ((byte *) md3Surf + md3Surf->ofsTriangles); // TODO possible alignment fault
        ...
        surf->verts = v = Hunk_Alloc(sizeof(*v) * (md3Surf->numVerts * md3Surf->numFrames), h_low);

        md3xyz = (md3XyzNormal_t *) ((byte *) md3Surf + md3Surf->ofsXyzNormals); // TODO possible alignment fault
        ...
        // swap all the ST
        surf->st = st = Hunk_Alloc(sizeof(*st) * md3Surf->numVerts, h_low);

        md3st = (md3St_t *) ((byte *) md3Surf + md3Surf->ofsSt); // TODO possible alignment fault
        ...
        // calc tangent spaces
        {
            vec3_t *sdirs = malloc(sizeof(*sdirs) * surf->numVerts * mdvModel->numFrames);
            vec3_t *tdirs = malloc(sizeof(*tdirs) * surf->numVerts * mdvModel->numFrames);
    ...
    else
    {
        ...
        mdvModel->vaoSurfaces = Hunk_Alloc(sizeof(*mdvModel->vaoSurfaces) * mdvModel->numSurfaces, h_low);
...
*/
#if 0
static qboolean R_RegisterMDR(const char *name, qhandle_t *handle)
{
assert(0);//untested

// TODO NOT finished , maybe not needed
    union {
        unsigned *u;
        void *v;
    } buf;
    ...
    filesize = 0; //FS_GetFile(name, (void **) &buf.v);
    if( filesize <= 0 || !buf.u ) // TODO possible fix of misuse of FS_GetFile
    {
        mod->type = MOD_BAD;
        return 0;
    }
    ...
    if(!loaded)
    {
        Com_Printf("%s: couldn't load mdr file %s\n", __func__, name);
        mod->type = MOD_BAD;
        return 0;
    }
...
assert(0);//untested
    ...
    if(size > filesize)
    {
        Com_Printf("%s: Header of %s is broken. Wrong filesize declared!\n", __func__, mod_name);
        return qfalse;
    }
    ...
    {
        Com_Printf("%s: %s has broken structure.\n", __func__, mod_name);
        return qfalse;
    }
    ...
    if (pinmodel->ofsFrames < 0)
    {
        ...
        // compressed model...
        cframe = (mdrCompFrame_t *)((byte *) pinmodel - pinmodel->ofsFrames); // TODO possible alignment fault
        ...
        for(i = 0; i < mdr->numFrames; i++)
        {
            ...
            for(j = 0; j < mdr->numBones; j++)
            {
                for(k = 0; k < (sizeof(cframe->bones[j].Comp) / 2); k++)
                {
                    ...
                    ((unsigned short *)(cframe->bones[j].Comp))[k] = LittleShort( ((unsigned short *)(cframe->bones[j].Comp))[k] ); // TODO possible alignment fault
                }
            ...
            // Next Frame...
            cframe = (mdrCompFrame_t *) &cframe->bones[j]; // TODO possible alignment fault
            frame = (mdrFrame_t *) &frame->bones[j]; // TODO possible alignment fault
        }
    }
    else
    {
        ...
        curframe = (mdrFrame_t *)((byte *) pinmodel + pinmodel->ofsFrames); // TODO possible alignment fault
    ...
    curlod = (mdrLOD_t *)((byte *) pinmodel + LittleLong(pinmodel->ofsLODs)); // TODO possible alignment fault
    ...
    for ( l = 0 ; l < mdr->numLODs ; l++)
    {
        ...
        cursurf = (mdrSurface_t *) ((byte *)curlod + LittleLong(curlod->ofsSurfaces)); // TODO possible alignment fault
        ...
        for ( i = 0 ; i < lod->numSurfaces ; i++)
        {
            ...
            {
                Com_Printf("%s: %s has more than %i verts on %s (%i).\n",
                      __func__, mod_name, SHADER_MAX_VERTEXES - 1, surf->name[0] ? surf->name : "a surface",
                      surf->numVerts );
                return qfalse;
            }
            ...
            {
                Com_Printf("%s: %s has more than %i triangles on %s (%i).\n",
                      __func__, mod_name, ( SHADER_MAX_INDEXES / 3 ) - 1, surf->name[0] ? surf->name : "a surface",
                      surf->numTriangles );
                return qfalse;
            }
            // lowercase the surface name so skin compares are faster
            ...
            curv = (mdrVertex_t *) ((byte *)cursurf + LittleLong(cursurf->ofsVerts)); // TODO possible alignment fault
            ...
            curtri = (mdrTriangle_t *)((byte *) cursurf + LittleLong(cursurf->ofsTriangles)); // TODO possible alignment fault
            ...
            // find the next surface.
            cursurf = (mdrSurface_t *) ((byte *) cursurf + LittleLong(cursurf->ofsEnd)); // TODO possible alignment fault
        }
        ...
        // find the next LOD.
        curlod = (mdrLOD_t *)((byte *) curlod + LittleLong(curlod->ofsEnd)); // TODO possible alignment fault
    }

    // lod points to the first tag now, so update the offset too.
    tag = (mdrTag_t *) lod;
    mdr->ofsTags = (int)((byte *) tag - (byte *) mdr);
    curtag = (mdrTag_t *) ((byte *)pinmodel + LittleLong(pinmodel->ofsTags)); // TODO possible alignment fault
...
#if 0
static void R_RegisterIQMCallback( void *ptr ) {
assert(0);
    ...
    const char * name = fetch->url;
    qhandle_t *handle = fetch_callback->userData;
    //int           lod = fetch_callback->additionalData;
    model_t *     mod = NULL;
    ...
    if ( ( mod = R_AllocModel() ) == NULL ) {
        Com_Printf( "%s: R_AllocModel() failed for '%s'\n", __func__, name);
        return;
    }
    ...
    buf.v = (void *)fetch->data;
    qboolean loaded = qfalse;
    int filesize = fetch->numBytes;

    if(filesize <= 0 || !buf.u) // TODO possible fix filesize <= 0 maybe lots of wrong uses of FS_GetFile
    {
        mod->type = MOD_BAD;
        return;
    }
    ...
    if(!loaded)
    {
        Com_Printf("%s: couldn't load iqm file %s\n", __func__, name);
        mod->type = MOD_BAD;
        return;
    }
...
#if 0
static qboolean R_RegisterIQM(const char *name, qhandle_t *handle)
{
assert(0);
#if 0
    if ( !name || !name[0] ) {
    ...
    // need to pass lod mod and handle
    FS_GetFileAsync( name, &(emscripten_fetch_callback_t) {
        .additionalData = lod,
        .errFatal       = qfalse,
        .errMessage     = "failed to load iqm",
        .error_callback = NULL,
        .success_callback = R_RegisterIQMCallback,
        .userData       = handle,
    });
#endif
}
...
qhandle_t RE_RegisterModel( const char *name ) {
    ...
    for ( qhandle_t hModel = 1 ; hModel < tr.numModels; hModel++ ) {
        ...
        if ( !strcmp( mod->name, name ) ) {
            if( mod->type == MOD_BAD ) {
                assert(0); // TODO can this happen normally?
                return 0;
            }
            return hModel;
        }
    ...
    if( *ext ) {
        ...
        Com_sprintf( altName, sizeof (altName), "%s.%s", localName, "md3" );
        loadName = altName;
    }
    ...
    qhandle_t model_handle = R_RegisterMD3( localName, mod );
...
*/
int updateWeaponModelBounds; // TODO move me somewhere else
void RE_BeginRegistration( glconfig_t *glconfigOut ) {
    updateWeaponModelBounds = 0;
    R_Init();
    R_IssuePendingRenderCommands();
}
...
static mdvTag_t *R_GetAnimTag( mdrHeader_t *mod, int framenum, const char *tagName, mdvTag_t * dest)
{
    ...
    tag = (mdrTag_t *)((byte *)mod + mod->ofsTags); // TODO possible alignment fault
    for ( i = 0 ; i < mod->numTags ; i++, tag++ )
    {
        if ( !strcmp( tag->name, tagName ) )
        {
            ...
            frame = (mdrFrame_t *)((byte *)mod + mod->ofsFrames + framenum * frameSize ); // TODO possible alignment fault
...
void R_ModelBounds( qhandle_t handle, vec3_t mins, vec3_t maxs ) {
    ...
    if(model->type == MOD_BRUSH) {
        ...
        frame = (mdrFrame_t *) ((byte *)header + header->ofsFrames); // TODO possible alignment fault
