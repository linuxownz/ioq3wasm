...
// #if 0 from original source IGNORE
#if 0
    int     i, j;
...
static qboolean SurfIsOffscreen( const drawSurf_t *drawSurf, vec4_t clipDest[128] ) {
    ...
    if ( tess.numVertexes >= 128 ) {
        Com_Error(ERR_FATAL, "%s tess.numVertexes >= 128 ", __func__);
    }
...
void R_SortDrawSurfs( drawSurf_t *drawSurfs, int numDrawSurfs ) {
    ...
    for ( i = 0 ; i < numDrawSurfs ; i++ ) {
        ...
        if ( shader->sort == SS_BAD ) {
            Com_Printf ("%s Shader '%s' with sort == SS_BAD finished:%d\n", __func__, shader->name, shader->finished );
            assert(0);
        }
...
void R_DebugPolygon( int color, int numPoints, float *points ) {
    // all this from original source IGNORE
    ...
    qglBegin( GL_POLYGON ); // commented out
        ...
        if (ent->e.reType == RT_MODEL)
        {
            ...
            switch (model->type)
            {
                ...
                case MOD_MDR:
                {
                    ...
                    mdrFrame_t *frame = ( mdrFrame_t * ) ( ( byte * ) header + header->ofsFrames + frameSize * ent->e.frame); // TODO possible alignment fault
