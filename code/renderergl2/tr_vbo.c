...
// For best performance, use VBOs instead and build WITHOUT the -sFULL_ES2 linker flag.

// Even if your application does not need any WebGL 2/OpenGL ES 3 features,
// consider porting the application to run on WebGL 2,
// because JavaScript side performance in WebGL 2 has been optimized to generate no temporary garbage,
// which has been observed to give a solid 3-7% speed improvement,
// as well as reducing potential stuttering at render time.
//
// To enable these optimizations, build with the linker flag -sMAX_WEBGL_VERSION=2 and make sure to create a WebGL 2 context at GL startup time (OpenGL ES 3 context if using EGL).
...
vao_t *R_CreateVao(const char *name, byte *vertexes, int vertexesSize, byte *indexes, int indexesSize, vaoUsage_t usage)
{
    ...
    vao = tr.vaos[tr.numVaos] = Hunk_Alloc(sizeof(*vao), h_low);
    tr.numVaos++;
    ...
    {
        glGenVertexArrays(1, &vao->vao); //glGenVertexArrays(1, &vao->vao);
        glBindVertexArray(vao->vao);  // glBindVertexArray(vao->vao);
    }
...
void R_BindVao(vao_t * vao)
{
    if ( ! vao ) {
        //R_BindNullVao();
        Com_Error(ERR_DROP, "%s: NULL vao", __func__);
        return;
    }

    if ( r_logFile->integer ) {
        // don't just call LogComment, or we will get a call to va() every frame!
        GLimp_LogComment(va("--- %s( %s ) ---\n", __func__, vao->name));
    }
    ...
    if ( glState.currentVao != vao )
    {
        ...
        if (glRefConfig.vertexArrayObject)
        {
            glBindVertexArray(vao->vao); // qglBindVertexArray(vao->vao);
...
void R_BindNullVao(void)
{
    ...
    if(glState.currentVao)
    {
        if (glRefConfig.vertexArrayObject)
        {
            glBindVertexArray(0); // qglBindVertexArray(0);
...
void R_InitVaos(void)
{
    ...
    Com_Printf("------- R_InitVaos complete -------\n");
}
...
void R_VaoList_f(void)
{
    ...
    Com_Printf(" %i total VAOs\n", tr.numVaos);
    Com_Printf(" %d.%02d MB total vertices memory\n", vertexesSize / (1024 * 1024),
              (vertexesSize % (1024 * 1024)) * 100 / (1024 * 1024));
    Com_Printf(" %d.%02d MB total triangle indices memory\n", indexesSize / (1024 * 1024),
...
void VaoCache_DrawElements(int numIndexes, int firstIndex)
{
	...
	glDrawElements(GL_TRIANGLES, numIndexes, glRefConfig.vaoCacheGlIndexType, BUFFER_OFFSET(firstIndex * glRefConfig.vaoCacheGlIndexSize));
}
