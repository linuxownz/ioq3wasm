...
int R_ComputeLOD( trRefEntity_t *ent ) {
    ...
    else
    {
        ...
        if(tr.currentModel->type == MOD_MDR)
        {
            ...
            mdrframe = (mdrFrame_t *) ((byte *) mdr + mdr->ofsFrames + frameSize * ent->e.frame); // TODO possible alignment fault
...
void R_AddMD3Surfaces( trRefEntity_t *ent ) {
    ...
    if ( (ent->e.frame >= tr.currentModel->mdv[0]->numFrames) || (ent->e.frame < 0) || (ent->e.oldframe >= tr.currentModel->mdv[0]->numFrames) || (ent->e.oldframe < 0) ) {
            //Com_Printf( "R_AddMD3Surfaces: no such frame %d to %d for '%s'\n", ent->e.oldframe, ent->e.frame, tr.currentModel->name );
            ent->e.frame = 0;
            ent->e.oldframe = 0;
    }
