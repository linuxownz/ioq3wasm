...
#define GLLINE do{void GL_CheckErrs(char *, int); GL_CheckErrs(__FILE__, __LINE__);}while(0)
...
typedef struct {
    ...
    unsigned int    numImageAnimations;
    float           imageAnimationSpeed;

    texCoordGen_t   tcGen;
    vec3_t          tcGenVectors[2];

    unsigned int    numTexMods;
    texModInfo_t    *texMods;        // texModInfo_t texMods[MAX_SHADER_STAGES][TR_MAX_TEXMODS]
...
typedef struct shader_s {
    ...
    int         shader_realLightmapIndex; // instead of a global var
    ...
    int         numUnfoggedPasses;      // number of shaderStage_t in stages[] ???
    shaderStage_t   *stages[MAX_SHADER_STAGES]; // going to convert back to *stages TODO

    // int         image_load_count; // kept but may not need if preloading all images works
    qboolean    stages_swapped; // TODO
    ...
    qboolean            finished;                     // has FinishShader been called on this shader?
...
typedef struct {
    ...
    qboolean dummy; //directStateAccess;

    int maxVertexAttribs;
    qboolean gpuVertexAnimation;

    GLenum vaoCacheGlIndexType;
...
void RE_RegisterSkinAsync( const char *name, qhandle_t *handle);
...
// NOT FOUND shader_t    *R_GetShaderByState( int index, long *cycleTime );
...
typedef struct shaderCommands_s
{
    ...
    shaderStage_t   *xstages[MAX_SHADER_STAGES];
} shaderCommands_t;
