...
// static FILE   *debuglogfile; // unsed
static fileHandle_t pipefile;
fileHandle_t logfile;
fileHandle_t com_journalFile;            // events are written here
fileHandle_t com_journalDataFile;        // config files are written here  // what is written here? TODO
...
cvar_t *com_pipefile;
cvar_t *com_instance_id;      // remote local bi-directional communication over AF_UNIX socket
...
extern qboolean com_logfileOpened;
...
void Com_Printf( const char *fmt, ... ) {
    ...
    // print to console if run from command line for debugging
    // proxy.pl closes stdout/stderr
    Sys_Print( msg );

    if ( g_output_to_clientsock ) {
        FS_WriteToSock ( msg, strlen(msg) ); // the only place this is called
    } else if ( com_logfileOpened ) {
        FS_Write(msg, strlen(msg), logfile); // logfile
    } else {
        printf("com_logfileOpened is false\n");
    }
}
...
void Com_Error( int code, const char *fmt, ... ) {
    ...
    // TODO some or all of these can be removed??
...
static void Com_ParseCommandLine( char *commandLine ) {
    ...
    while ( *commandLine ) {
        ...
        if ( (*commandLine == '+' && !inq && prev == ' ') || *commandLine == '\n'  || *commandLine == '\r' ) {
...
void Com_StartupVariable( const char *match ) {
    ...
    for (i=0 ; i < com_numConsoleLines ; i++) {
        ...
        {
            if((unsigned int)Cvar_Flags(s) == CVAR_NONEXISTENT)
...
{
    Cbuf_ExecuteText(EXEC_NOW, "exec default.cfg\n"); // default.cfg looks client only ...
    Cbuf_Execute();                                   // Always execute after exec to prevent text buffer overflowing

    // skip the q3config.cfg and autoexec.cfg
    // if "safe" is on the command line
    if(!Com_SafeMode())
    {
        ...
        if ( com_instance_id ) {
            Com_sprintf ( buffer, MAX_OSPATH - 1, "exec %s.cfg\n", com_instance_id->string );
        } else {
            Com_Error ( ERR_FATAL, "com_instance_id not defined Com_ExecuteCfg" );
        }
        ...
        //Cbuf_ExecuteText(EXEC_NOW, "exec autoexec.cfg\n"); // remove this and have all config in Q3CONFIG_CFG TODO
        //Cbuf_Execute(); // TODO
    }
}
...
void Com_Init( char *commandLine ) {
    ...
    //Com_Printf( "%s %s %s\n", Q3_VERSION, PLATFORM_STRING, PRODUCT_DATE ); // TODO
...
//  Swap_Init ();
    ...
    com_developer  = Cvar_Get("developer",      "0",      CVAR_TEMP); // setting to 1 breaks exectosock as Com_DPrintf doesn't know about socket
    com_standalone = Cvar_Get("com_standalone", "0",      CVAR_ROM);
    com_basegame   = Cvar_Get("com_basegame",   BASEGAME, CVAR_INIT);
    com_homepath   = Cvar_Get("com_homepath",   "",       CVAR_INIT|CVAR_PROTECTED);

    FS_InitFilesystem (); // log file is available
    ...
    com_timescale   = Cvar_Get("timescale", COM_TIMESCALE_DEFAULT, CVAR_CHEAT | CVAR_SYSTEMINFO );
    ...
    if ( !Com_AddStartupCommands() ) {
        ...
        Com_Logf ( "qcommon/common.c failed to add startup commands ...\n" );
        Com_Logf ( "qcommon/common.c failed to add startup commands ...\n" );
        Com_Logf ( "qcommon/common.c failed to add startup commands ...\n" );
        Com_Logf ( "qcommon/common.c failed to add startup commands ...\n" );
        Com_Logf ( "qcommon/common.c failed to add startup commands ...\n" );
    ...
    // com_sockfile must be on command line or quake will use what is in config...
    com_instance_id = Cvar_Get ( "com_instance_id", "", CVAR_ARCHIVE|CVAR_LATCH );
    if ( com_instance_id->string[0] ) {
        FS_FCreateOpenSockFile ( com_instance_id->string );
    } else {
        Sys_Error ( "+set com_instance_id <uuid> not on command line.  must be set. socket lives in ~/.q3a/$basegame/sockets/<uuid>.socket log in ~/.q3a/$basegame/log/<uuid>.log\n" );
    }

    com_fullyInitialized = qtrue;

    Com_Logf ("--- Common Initialization Complete ---\n");
}
...
Check if there is a connection and request
waiting on the unix socket connected to the proxy
Read whatever, if anything, and execute it
===============
*/

static int Com_CheckRequest( int msec )
{
    int res;
    uint64_t t1, t2;

    t1  = get_clock_time();
    res = FS_ReadFromSock( msec );
    t2  = get_clock_time();

    int dur = (t2 - t1)/1000;
    if ( dur > msec ) {
        //Com_Logf ("FS_ReadFromSock and execute took %d ms > msec:%d\n", dur, msec );
    }
...
static int Com_ModifyMsec( int msec ) {
    ...
    if (com_sv_running->integer && msec > 500)
        Com_Printf( "Hitch warning: %i msec frame time\n", msec ); // TODO this happends when commands are issued
...
void Com_Frame( void ) {
    ...
    if ( waitfor == 0 || com_frameNumber > waitfor ) {
        //Com_Printf("checking for request\n");
        if ( Com_CheckRequest ( msec / 2 ) ) { // if true(error) back off a bit
            waitfor = com_frameNumber + 20;
        } else if ( com_frameNumber > waitfor ) {
            waitfor = 0;
        }
    }
