...
#ifndef _QCOMMON_SERVER_H_
#define _QCOMMON_SERVER_H_
...
#define PORT_WS_SERVER   27960 // TODO what port should it run on?  trying the same port as UDP
...
void Cbuf_ExecuteTextToSock (int exec_when, const char *text);
// sets global var to set output to clientsock

extern qboolean g_output_to_clientsock;
...
#define Q3CONFIG_CFG "q3config_server.cfg" // config file name will be passed on commandline and will be a mandaotry item
...
void            FS_FCreateOpenSockFile( const char *filename );
...
int     FS_ListenFromSock(void);
void FS_CloseClientSock(void);
...
void        Com_Logf( const char *fmt, ... ) __attribute__ ((format (printf, 1, 2)));
