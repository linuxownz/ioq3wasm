...
#include "../qcommon/net_ws_server.h"
...
static int numIP;
const char *NET_AdrTypeToString ( int type );
...
static qboolean Sys_StringToSockaddr(const char *s, struct sockaddr *sadr, int sadr_len, sa_family_t family)
{
    ...
    if(!retval)
    {
        ...
        if(search)
        {
            if(search->ai_addrlen > (size_t)sadr_len)
...
qboolean NET_CompareBaseAdrMask(netadr_t a, netadr_t b, int netmask)
{
    ...
    if(a.type == NA_IP || a.type == NA_WEBSOCKET )
    {
        addra = (byte *) &a.ip;
        addrb = (byte *) &b.ip;
    ...
    {
        Com_Printf ("NET_CompareBaseAdr: bad address type %s\n", NET_AdrTypeToString( a.type ) );
        return qfalse;
    }
...
const char *NET_AdrTypeToString ( int type ) {
    switch ( type ) {
        case NA_BAD:        return "NA_BAD";
        case NA_BOT:        return "NA_BOT";
        case NA_LOOPBACK:   return "NA_LOOPBACK";
        case NA_BROADCAST:  return "NA_BROADCAST";
        case NA_IP:         return "NA_IP";
        case NA_IP6:        return "NA_IP6";
        case NA_MULTICAST6: return "NA_MULTICAST6";
        case NA_WEBSOCKET:  return "NA_WEBSOCKET";
        case NA_UNSPEC:     return "NA_UNSPEC";
    }
...
{
    // 127.0.0.1 not the same as 0.0.0.0 but the 0.0.0.0 is because it wasn't set??? ....
    //Com_Printf("%s netadr_t a: %d.%d.%d.%d  netadr_t b: %d.%d.%d.%d\n", __func__, a.ip[0], a.ip[1], a.ip[2], a.ip[3], b.ip[0], b.ip[1], b.ip[2], b.ip[3] );
    //Com_Printf("%s netadr_t a port: %d  netadr_t b port: %d\n", __func__, a.port, b.port);
    //Com_Printf("%s netadr_t a: %s  netadr_t b:%s\n", __func__, NET_AdrToString(a), NET_AdrToString(b));
...
static qboolean NET_GetPacket(netadr_t *net_from, msg_t *net_message, fd_set *fdr)
{
    ...
    if ( net_ws_enabled->integer ) {
        //Com_Printf("NET_GetPacket ws\n");
        net_packet_t np;
        if ( NET_Websockets_RecvPacket( &np )  ) {

            memcpy(net_from->ip, &np.ip, 4);
            net_from->port = np.port;
            net_from->type = NA_WEBSOCKET;

            net_message->readcount = 0;
            net_message->cursize = np.len;
            net_message->oob     = qfalse; // TODO ???

            ret = MIN((int)np.len, net_message->maxsize);
            memcpy(net_message->data, np.data, ret );

            free(np.data); // TODO
            np.data = NULL;

            if( (int)np.len >= net_message->maxsize ) {
                Com_Printf( "Oversize packet np.len:%lu nm->maxsize:%d from %d \n", np.len, net_message->maxsize, np.ip);
                return qfalse;
            }
        ...
        Com_Error(ERR_FATAL,"net_ws_enabled not enabled");
    }
...
void Sys_SendPacket( int length, const void *data, netadr_t to ) {
    ...
    if( to.type != NA_BROADCAST && to.type != NA_IP && to.type != NA_IP6 && to.type != NA_MULTICAST6 && to.type != NA_WEBSOCKET )
    {
        Com_Error( ERR_FATAL, "Sys_SendPacket: bad address type " );
        return;
    }

    if ( to.type == NA_WEBSOCKET ) {
        //Com_Printf("%s netadr_t dump:\n", __func__ );
        //Com_Printf("ip %d.%d.%d.%d ip\n", to.ip[0], to.ip[1], to.ip[2], to.ip[3]);
        //Com_Printf("port: %d\n", to.port);

        net_packet_t packet = {
            .alloced = qfalse,
            .data    = (byte *)data,
            .ip      = to.ip[0] | to.ip[1] << 8 | to.ip[2] << 16 | to.ip[3] << 24,
            .port    = to.port,
            .len     = length
        };

        //int res =
        NET_Websockets_SendPacket( &packet );
        //Com_Printf("%s size:%d sent:%d\n", __func__, length, res);
        return;
    } else {
        //Com_Printf("to.type IS NOT NA_WEBSOCKET!!!!!\n");
    }
...
static void NET_OpenIP( void ) {
    ...
    if(net_ws_enabled->integer) {
        int port = net_port->integer;// it may have changed above
        Com_Printf("NET_OpenIP net_ws_enabled true starting on port: %d\n", port);
        if ( NET_Websockets_Start(port) ) {
            Com_Error ( ERR_FATAL, "Failed to init websockets" );
        }
    } else {
        Com_Error(ERR_FATAL, "net_ws_enabled not enabled");
    }
...
static qboolean NET_GetCvars( void ) {
    ...
    net_enabled = Cvar_Get( "net_enabled", "1", CVAR_LATCH | CVAR_ARCHIVE ); // the "1" is a bitmap "2" is IP6 ..
    modified = net_enabled->modified;
    net_enabled->modified = qfalse;

    // WebSockets Support
    net_ws_enabled = Cvar_Get( "net_ws_enabled", "1", CVAR_LATCH | CVAR_ARCHIVE );
    modified = net_ws_enabled->modified;
    net_ws_enabled->modified = qfalse;
    ...
    // WebSockets Support
    //net_ws_port = Cvar_Get( "net_ws_port", va( "%i", PORT_WS_SERVER ), CVAR_LATCH | CVAR_SERVERINFO );
    net_ws_port = Cvar_Get( "net_port", va( "%i", PORT_SERVER ), CVAR_LATCH | CVAR_SERVERINFO );
    modified += net_ws_port->modified;
    net_ws_port->modified = qfalse;
...
void NET_Config( qboolean enableNetworking ) {
    ...
    if( enableNetworking == (qboolean)networkingEnabled && !modified ) {
        return;
    }
...
static void NET_Event(fd_set *fdr)
{
    ...
    while(1)
    {
        MSG_Init(&netmsg, bufData, sizeof(bufData)); // bufData is the buffer for *data this call
...
void NET_Sleep(int msec)
{
    ...
    else if(retval > 0 || NET_Websockets_GetQueueLength())
        NET_Event(&fdr);
}
