...
#if ! defined(CLIENT)
# error qcommon/cmd_client.c trying to compile as SERVER
#endif
...
void Cbuf_Execute (void)
{
    ...
    memset ( line, 0, MAX_CMD_LINE ); // TODO
...
static void Cmd_Exec_f_callback ( void * userdata ) {
    emscripten_fetch_t *fetch = userdata;

    //Com_Printf("%s %s %llu\n", __func__, fetch->url, fetch->numBytes);

    char *data = calloc(fetch->numBytes + 1, 1);
    if ( ! data ) {
...
// this func just runs the 'exec' cmd which loads a file to execute
static void Cmd_Exec_f( void ) {
    ...
    FS_GetFileAsync( filename, &(emscripten_fetch_callback_t) {
        .additionalData   = 0,
        .error_callback   = NULL,
        .errFatal         = qfalse,
        .errMessage       = "failed to load config file",
        .success_callback = Cmd_Exec_f_callback,
        .userData         = NULL,
        .nocache          = qtrue,
    });
}
...
char    *Cmd_Args( void ) {
    ...
    // TODO 'SERVER' there is something wrong with this function and the management of static char cmd_args;
    // this is called in Com_Quit_f to get args at shutdown and it looks like previous args mangled
    // this cmd_args needs to be cleaned after each use?
    cmd_args[0] = 0;
        ...
        if ( !Q_stricmp( cmd_argv[0],cmd->name ) ) {
            ...
            *prev = cmd->next;
            ...
            if ( !cmd->function ) {
                ...
                //Com_Printf ( "Cmd_ExecuteString: no function :'%s' letting game handle it\n", cmd->name );
                break;
            } else {
                //printf("%s execing %s %s\n", __func__, cmd->name, Cmd_Args());
                cmd->function ();
            }
            return;
        }
