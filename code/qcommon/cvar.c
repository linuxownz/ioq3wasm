...
//  Com_DPrintf( "Cvar_Set2: %s %s\n", var_name, value );
    ...
    // Com_Printf("%s %s %s keycatcher: %d\n", __func__, var->name, var->string, Key_GetCatcher());
    // assuming if keycatcher is 1, player typed at console to change cvar
    if ( Key_GetCatcher() == 1 ) {
        if ( strcmp ( var->name, "com_unfocused" ) ) {
            void setting_change(const char *name,const char *value);
            setting_change(var->name, var->string);
        }
    }
#endif
...
void Cvar_SetSafe( const char *var_name, const char *value )
{
    ...
    if(((unsigned int)flags != CVAR_NONEXISTENT) && (flags & CVAR_PROTECTED))
    {
        if( value )
...
void    Cvar_Update( vmCvar_t *vmCvar ) {
    ...
    if ( ! vmCvar ) {
        Com_Error(ERR_FATAL,"%s vmCvar is null", __func__);
    }

    assert(vmCvar);

    if ( /*(unsigned)*/vmCvar->handle >= cvar_numIndexes ) { // TODO ...
