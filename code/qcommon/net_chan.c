...
#else
    NET_SendPacket(send.cursize, send.data);
#endif
...
qboolean Netchan_Process( netchan_t *chan, msg_t *msg ) {
    ...
    if ( fragmented ) {
        ...
        if ( fragmentLength < 0 || msg->readcount + fragmentLength > msg->cursize ||
                (unsigned)chan->fragmentLength + fragmentLength > sizeof( chan->fragmentBuffer ) ) {
            if ( showdrop->integer || showpackets->integer ) {
        ...
        *(int *)msg->data = LittleLong( sequence ); // TODO possible alignment fault
...
#else
static void NET_QueuePacket( int length, const void *data, int offset )
#endif
{
...
#if defined(SERVER)
// TODO no sv_packetdelay???
    if ( to.type == NA_LOOPBACK ) {
        // NET_SendLoopPacket (sock, length, data, to); // TODO remove ?
        return;
    }
#else
    if ( cl_packetdelay->integer > 0 ) {
        NET_QueuePacket( length, data, cl_packetdelay->integer );
    } else {
        Sys_SendPacket ( length, data );
    }
#endif
...
#else
    NET_SendPacket( strlen( string ), string );
#endif
}
...
#else
    byte *string = malloc ( MAX_MSGLEN * 2 ); // TODO create static buffer instead???
...
#else
    NET_SendPacket(mbuf.cursize, mbuf.data);
    free(string);
    string = NULL;
#endif
}
