...
#include <errno.h>
#include "q_shared_server.h"
#include "qcommon_server.h"
#include "unzip.h"
...
typedef struct {
    char        path[MAX_OSPATH];       // c:\quake3          // TODO update this comment with linux specifics
...
// never load anything from pk3 files that are not present at the server when pure // TODO use this?  always pure?
...
qboolean com_logfileOpened = qfalse;
...
static qboolean FS_PakIsPure( pack_t *pack ) {
    ...
    if ( fs_numServerPaks ) {
        for ( i = 0 ; i < fs_numServerPaks ; i++ ) {
            ...
            if ( pack->checksum == fs_serverPaks[i] ) { // TODO THIS !!! <<<<<
        ...
        const char *function )
{
    ...
    if(  COM_CompareExtension( filename, ".qvm" ) || COM_CompareExtension( filename, ".pk3" ) )
    {
...
void FS_Remove( const char *osPath ) {
    ...
    if ( -1 == remove( osPath ) ) {
        Com_Printf ( "failed to remove '%s' errno:'%d'", osPath, errno );
    }
}
...
/*
======================
FS_FCreateOpenSockFile

~/Projects/IOQuake3/ioq3wasm/build/ioq3dedws.x86_64 +set com_sockfile sockets/q3ctf1.socket
mkdir -p ~/.q3a/{baseq3,defrag,etcetera}/sockets

======================
*/
void FS_FCreateOpenSockFile( const char *instance_id ) {

    char *sockpathfmt = "/sockets/%s.socket";
    char buffer[MAX_OSPATH] = { 0 };

    Com_sprintf ( buffer, MAX_OSPATH - 1, sockpathfmt, instance_id );

    char *ospath = FS_BuildOSPath( fs_homepath->string, fs_gamedir, buffer );

    if ( fs_debug->integer ) {
        Com_Printf( "FS_FCreateOpenSockFile: %s\n", ospath );
    }

    listensock = Sys_MkAFSock( ospath );

    if ( ! listensock ) { // TODO needs more testing
        // TODO this error needs to be fatal
        Com_Printf( S_COLOR_YELLOW "ERROR: Could not create new listening socket at %s!!\n", ospath );
        Com_Error ( ERR_FATAL, "ERROR: Could not create new listening socket at %s!!\n", ospath  );
    }
}
...
static long FS_FOpenFileReadDir(const char *filename, searchpath_t *search, fileHandle_t *file, qboolean uniqueFILE, qboolean unpure)
{
    long            hash;
    pack_t      *pak;
...
char * socket_err_to_str ( int err ) {
    char *str = "unknown error";

    switch ( err ) {
        case EWOULDBLOCK: //case EAGAIN:
            str = "EWOULDBLOCK: The socket is marked nonblocking and the receive operation would block, "
        ...
        case EBADF:
            str = "EBADF: The argument sockfd is an invalid file descriptor.";
            break;

        case ECONNREFUSED:
            str = "ECONNREFUSED: A remote host refused to allow the network connection (typically because it is not running the requested service).";
            break;

        case EFAULT:
            str = "EFAUL: The receive buffer pointer(s) point outside the process's address space.";
            break;

        case EINTR:
            str = "EINTR: The receive was interrupted by delivery of a signal before any data was available; see signal(7).";
            break;

        case EINVAL:
            str = "EINVAL: Invalid argument passed.";
            break;

        case ENOMEM:
            str = "ENOMEM: Could not allocate memory for recvmsg().";
            break;

        case ENOTCONN :
            str = "ENOTCONN: The socket is associated with a connection-oriented protocol and has not been connected (see connect(2) and accept(2)).";
            break;

        case ENOTSOCK:
            str = "ENOTSOCK: The file descriptor sockfd does not refer to a socket.";
            break;
        ...
        case ENFILE:  str="File table overflow"; break;
        case EMFILE:  str="Too many open files"; break;
        case ENOTTY:  str="Not a typewriter"; break;
        case ETXTBSY: str="Text file busy"; break;
        ...
        case EDOM:    str="Math argument out of domain of func"; break;
        case ERANGE:  str="Math result not representable"; break;
        case EDEADLK: str="Resource deadlock would occur"; break;
        case ENAMETOOLONG: str="File name too long"; break;
        case ENOLCK:    str="No record locks available"; break;
        ...
        case ENOMSG:   str="No message of desired type"; break;
        case EIDRM:    str="Identifier removed"; break;
        ...
        case EL3RST:   str="Level 3 reset"; break;
        case ELNRNG:   str="Link number out of range"; break;
        case EUNATCH:  str="Protocol driver not attached"; break;
        case ENOCSI:   str="No CSI structure available"; break;
        ...
        case ENOANO:   str="No anode"; break;
        case EBADRQC:  str="Invalid request code"; break;
        case EBADSLT:  str="Invalid slot"; break;
        case EBFONT:   str="Bad font file format"; break;
        case ENOSTR:   str="Device not a stream"; break;
        case ENODATA:  str="No data available"; break;
        case ETIME:    str="Timer expired"; break;
        case ENOSR:    str="Out of streams resources"; break;
        ...
        case ENOLINK:  str="Link has been severed"; break;
        case EADV:     str="Advertise error"; break;
        case ESRMNT:   str="Srmount error"; break;
        case ECOMM:    str="Communication error on send"; break;
        case EPROTO:   str="Protocol error"; break;
        case EMULTIHOP: str="Multihop attempted"; break;
        case EDOTDOT:  str="RFS specific error"; break;
        ...
        case ELIBACC:  str="Can not access a needed shared library"; break;
        ...
        case EDESTADDRREQ: str="Destination address required"; break;
        ...
        case EOPNOTSUPP: str="Operation not supported on transport endpoint"; break;
        ...
        case EISCONN: str="Transport endpoint is already connected"; break;
        case ESHUTDOWN: str="Cannot send after transport endpoint shutdown"; break;
        case ETOOMANYREFS: str="Too many references: cannot splice"; break;
        ...
        case ENOTNAM:      str="Not a XENIX named type file"; break;
        case ENAVAIL:      str="No XENIX semaphores available"; break;
        case EISNAM:       str="Is a named type file"; break;
        case EREMOTEIO:    str="Remote I/O error"; break;
        case EDQUOT:       str="Quota exceeded"; break;
        case ENOMEDIUM:    str="No medium found"; break;
        case EMEDIUMTYPE:  str="Wrong medium type"; break;
        case ECANCELED:    str="Operation Canceled"; break;
        ...
        case EOWNERDEAD: str="Owner died"; break;
        case ENOTRECOVERABLE: str="State not recoverable"; break;
    }
...
int FS_ReadFromSock( int msec ) {

    if ( listensock == -1 ) {
        Com_Error ( ERR_FATAL, "FS_ReadFromSock listensock == -1\n");
    }
    ...
    int res = select(listensock + 1, &fdset, NULL, NULL, &timeout);
    ...
    if ( ! FD_ISSET ( listensock, &fdset ) ) {
        Com_Logf ( "FS_ISSET false\n");
        return 1;
    }

    clientsock = accept ( listensock, NULL, 0 );

    if ( -1 == clientsock  ) {
        Com_Printf ( "FS_ReadFromSock accept failed %s\n", strerror(errno) );
        return 1;
    }

    //Com_Logf ( "accepted connection\n" );
    ...
    while( ( read = recv ( clientsock, buf + accu, sizeof( buf ) - accu - 1, 0 ) ) > 0 ) {
        ...
        if( brk ) {
            ...
            *brk = '\0';
            ...
            Cbuf_ExecuteTextToSock( EXEC_NOW, buf );

            *brk = tmp;
            accu -= brk - buf;
            memmove( buf, brk, accu + 1 );

        } else if( accu >= (int)sizeof( buf ) - 1 ) {
            Cbuf_ExecuteTextToSock( EXEC_NOW, buf );
            accu = 0;
        }
    }
...
defined here
because the sock
is a UNIX socket file
=================
*/
int FS_WriteToSock( const char *buffer, int len ) {
    // DO NOT Com_Printf here or infinite loop // TODO this should not be true anymore ...

    if ( -1 == clientsock ) {
        return -1;
    }

    // TODO if other side of pipe is closed or buffer full could cause a signal
    // if proxy.pl process closes the pipe, get SIGPIPE

    // GETTING BROKEN PIPE????  WHY????? TODO
    ssize_t written = send (clientsock, buffer, len, 0 ); // MSG_NOSIGNAL

    if ( written == 0 ) {
        Com_Logf ( "sent 0 bytes\n" );
        return -2;
    }

    if ( written == -1 ) {
        Com_Logf ( "send error '%s' %s\n", buffer, strerror ( errno ) );
        return -3;
    }

    if ( written != len ) {
        Com_Error ( ERR_FATAL, "FS_WriteToSock len %d != sent len %d\n", len, (int)written );
        return -4;
    }
...
static pack_t *FS_LoadZipFile(const char *zipfile, const char *basename)
{
    ...
    for (i = 1; i <= (uLong)MAX_FILEHASH_SIZE; i <<= 1) {
    ...
    for(i = 0; i < (uLong)pack->hashSize; i++) {
...
===============
*/
static char **FS_ListFilteredFiles( const char *path, const char *extension, char *filter, int *numfiles, qboolean allowNonPureFilesOnDisk ) {
    int             nfiles;
    char            **listCopy;
...
int FS_GetModList( char *listbuf, int bufsize ) {
    int nMods, k, nTotal, nLen, nPaks, nDirs, nPakDirs, nDescLen;
    uLong nPotential, i, j;
    char **pFiles = NULL;
    char **pPaks = NULL;
    char **pDirs = NULL;
    char *name, *path;
    ...
    for ( i = 0 ; i < fs_numServerReferencedPaks ; i++ )
    {
        ...
        if ( !havepak && fs_serverReferencedPakNames[i] && *fs_serverReferencedPakNames[i] ) {
            ...
            if (dlstring)
            {
                ...
                if(strlen(origpos) + (origpos - neededpaks) >= (uLong) len - 1)
                {
                    *origpos = '\0';
                    break;
                }
...
static int FS_FOpenLogFile ( const char *homepath, const char *gameName ) {
    UNUSED(homepath); UNUSED(gameName);

    if ( ! com_logfileOpened ) {

        char logfilepath[MAX_OSPATH] = { 0 };
        char *logfilename = "qconsole";
        char *logfilefmt  = "log/%s.log";

        //char *logdir = FS_BuildOSPath ( homepath, gameName, "/log" );
        // qboolean clogdir = FS_CreatePath ( logdir );

        cvar_t *com_instance_id = Cvar_Get ("com_instance_id", "", CVAR_INIT );
        if ( NULL != com_instance_id ) {
            logfilename = com_instance_id->string;
        }

        Com_sprintf ( logfilepath, MAX_OSPATH - 1, logfilefmt, logfilename );
        ...
        logfile = FS_FOpenFileAppend( logfilepath );

        if(logfile) {
            FS_ForceFlush(logfile);
            printf( "logfile opened on %s\n", asctime( newtime ) ); // TODO how is this not recursive??? did I add this??
            com_logfileOpened = qtrue;
        } else {
            printf("Opening %s failed!\n", logfilename);
            Cvar_SetValue("logfile", 0);
            com_logfileOpened = qfalse;
        }
    }

    return com_logfileOpened;
}
...
================
*/
static void FS_Startup( const char *gameName ) // com_basegame->string
{
    const char *homePath;
    ...
    if (fs_homepath->string[0] && Q_stricmp(fs_homepath->string,fs_basepath->string)) {
        ...
        FS_FOpenLogFile ( fs_homepath->string, gameName );
    }
    ...
    Com_Printf (
            ...
            "fs_basepath:'%s'\n"
            "fs_basegame:'%s'\n"
            "fs_gamedirvar(fs_game):'%s'\n"
            "gamename(com_basegame):'%s'\n",
            fs_homepath->string,
            fs_basepath->string,
            fs_basegame->string,
...
static void FS_CheckPak0( void )
{
    ...
    for( path = fs_searchpaths; path; path = path->next )
    {
        ...
        {
            if(curpack->checksum == (int)DEMO_PAK0_CHECKSUM)
                founddemo = qtrue;
        }
        ...
        {
            if( curpack->checksum != (int)pak_checksums[pakBasename[3]-'0'] )
            {
                if(pakBasename[3] == '0')
                {
                    Com_Printf("\n\n"
        ...
        {
            if(curpack->checksum != (int)missionpak_checksums[pakBasename[3]-'0'])
            {
                Com_Printf("\n\n"
                        "**************************************************\n"
        ...
        else
        {
            ...
            for(index = 0; (size_t)index < ARRAY_LEN(pak_checksums); index++)
            {
                if(curpack->checksum == (int)pak_checksums[index])
                {
                    Com_Printf("\n\n"
                            "**************************************************\n"
            ...
            for(index = 0; (size_t)index < ARRAY_LEN(missionpak_checksums); index++)
            {
                if(curpack->checksum == (int)missionpak_checksums[index])
                {
                    Com_Printf("\n\n"
                            "**************************************************\n"
                            "WARNING: %s is renamed pak file %s%cpak%d.pk3\n"
...
void FS_InitFilesystem( void ) {
    ...
    char buffer[MAX_OSPATH] = { 0 };
    cvar_t *com_instance_id = Cvar_Get ("com_instance_id", "", CVAR_INIT );
    if ( com_instance_id->string[0] ) {
        Com_sprintf ( buffer, MAX_OSPATH - 1, "%s.cfg", com_instance_id->string );
    } else {
        Com_Error ( ERR_FATAL, "com_instance_id not set %s", __func__ );
    }

    // if we can't find <uuid>.cfg, assume that the paths are
    // busted and error out now, rather than getting an unreadable
    // graphics screen when the font fails to load
    long size = FS_ReadFile( buffer, NULL );
    if ( size <= 0 ) {
        Com_Error( ERR_FATAL, "Couldn't load, or empty file '%s' size:%lu\n", buffer, size );
    }
