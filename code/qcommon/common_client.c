...
int console_log(const char *a, int b); // client/cl_emscripten.c
void Com_Init3();

void load_progress(const char *message, int progress);

void Com_ConsoleLog ( const char *str ) {
    console_log ( str, strlen(str) );
}
...
void Com_Printf( const char *fmt, ... ) {
    ...
    printf("%s", msg); // this goes to 'console' may need Com_ConsoleLog in future TODO
...
void Com_Error( int code, const char *fmt, ... ) {
    ...
    if (code == ERR_DISCONNECT || code == ERR_SERVERDISCONNECT) {
        Com_Printf("%s ERR_%sDISCONNECT\n", __func__, code == ERR_SERVERDISCONNECT ? "SERVER" : "" );
        ...
        // Com_Error(ERR_FATAL,"longjmp (abortframe, -1) %d;", __LINE__); // TODO
        longjmp (abortframe, -1);

    } else if (code == ERR_DROP) {
        Com_Printf ("********************\nERROR: %s\n********************\n", com_errorMessage);
        Com_Printf ("Server crashed: %s",  com_errorMessage);

        Sys_Error("server crashed, exiting for now...");
        ...
        CL_FlushMemory( );
        com_errorEntered = qfalse;
        //Com_Error(ERR_FATAL,"longjmp (abortframe, -1)%d;", __LINE__);
        longjmp (abortframe, -1);
...
static void Z_ClearZone( memzone_t *zone, int size ) {
    ...
    zone->blocklist.next = zone->blocklist.prev = block = (memblock_t *)( (byte *)zone + sizeof(memzone_t) ); // TODO possible alignment fault
...
void Z_Free( void *ptr ) {
    ...
    block = (memblock_t *) ( (byte *)ptr - sizeof(memblock_t)); // TODO possible alignment fault
    if (block->id != ZONEID) {
    ...
    if ( *(int *)((byte *)block + block->size - 4 ) != ZONEID ) { // TODO possible alignment fault
...
void *Z_TagMalloc( int size, int tag ) {
    ...
    if (extra > MINFRAGMENT) {
        ...
        new = (memblock_t *) ((byte *)base + size ); // TODO possible alignment fault
        new->size = extra;
    ...
    *(int *)((byte *)base + base->size - 4) = ZONEID; // TODO possible alignment fault
...
====================
*/
int Hunk_MemoryRemaining( void ) { // TODO
    int     low, high;
...
const char *Com_EventTypeToString( const sysEventType_t type ) {
    switch (  type ) {
        case SE_NONE:          return "SE_NONE";          // evTime is still valid
        case SE_KEY:           return "SE_KEY";           // evValue is a key code, evValue2 is the down flag
        case SE_CHAR:          return "SE_CHAR";          // evValue is an ascii char
        case SE_MOUSE:         return "SE_MOUSE";         // evValue and evValue2 are relative signed x / y moves
        case SE_JOYSTICK_AXIS: return "SE_JOYSTICK_AXIS"; // evValue is an axis number and evValue2 is the current state (-127 to 127)
        case SE_CONSOLE:       return "SE_CONSOLE";       // evPtr is a char*
    }
    return "??";
}
...
void Com_QueueEvent( int time, sysEventType_t type, int value, int value2, int ptrLength, void *ptr )
{
    ...
    if ( eventHead - eventTail >= MAX_QUEUED_EVENTS )
    {
        ...
        Com_Error(ERR_FATAL,"Com_QueueEvent: overflow\n");
    ...
    static int lastnumqueuedevents = -1;
    int numqueuedevents = abs( eventHead - eventTail );
    if ( numqueuedevents != lastnumqueuedevents ) {
        //Com_Printf("Num queued events:%d\n", numqueuedevents);
        lastnumqueuedevents = numqueuedevents;
    }
}
...
static sysEvent_t Com_GetSystemEvent( void )
{
...
#if 0
    // CONSOLE input is input from the terminal ....
    // check for console commands // TODO
    char        *s;
    s = Sys_ConsoleInput(); // this returns NULL
    if ( s )
    {
        char  *b;
        int   len;
    ...
    // so this is always SE_NONE
    if ( ev.evType != SE_NONE ) {
        Com_Error(ERR_FATAL, "%s", Com_EventTypeToString(ev.evType));
    }
...
int Com_EventLoop( void ) {
    ...
    Com_Error(ERR_FATAL, "NEVER REACHED???");
...
{
     //Cbuf_ExecuteText(EXEC_INSERT, "exec default.cfg\n");
     //Cbuf_Execute(); // Always execute after exec to prevent text buffer overflowing

     if(!Com_SafeMode())
     {
         // skip the q3config.cfg and autoexec.cfg if "safe" is on the command line
         Cbuf_ExecuteText(EXEC_INSERT, "exec " Q3CONFIG_CFG "\n");
         //Cbuf_Execute();
         //Cbuf_ExecuteText(EXEC_INSERT, "exec autoexec.cfg\n");
         //Cbuf_Execute();
     }
}
...
void Com_GameRestart(int checksumFeed, qboolean disconnect)
{
    ...
    if(!com_gameRestarting && com_fullyInitialized)
    {
        ...
        // Clean out any user created cvars
        Cvar_Restart(qtrue);
        assert(0);
        Com_ExecuteCfg();
...
void Com_Init( char *cmdline ) {
...
#if 1
    if ( setjmp (abortframe) ) { // TODO remove?  // used in emscripten exception handling...
        Sys_Error ("Error during initialization");
    }
#endif

    load_progress("Initializing", 5);
    // Clear queues
    ...
    ////              Com_ParseCommandLine( cmdline ); ///////////////////////// TODO
    ...
    com_developer = Cvar_Get("developer", "0", CVAR_TEMP); // TODO

    // done early so bind command exists
    CL_InitKeyCommands();

    load_progress("Initializing filesystem", 15);

    Com_Printf("calling FS_InitFilesystem\n");
    filesysinitstart = Sys_Milliseconds();
    FS_InitFilesystem(); // -> Com_Init2
}

void Com_Init2(void) {
    if ( ! FS_Initialized() ) {
        Com_Printf("FS_Initialized false\n");
        return;
    }

    load_progress("Loading config", 85);
    emscripten_cancel_main_loop();

    printf("Filesystem initialization complete: %dms\n", Sys_Milliseconds() - filesysinitstart);
    ...
    emscripten_set_main_loop(Com_Init3, 1, 0);
}

int Cbuf_GetCursize(void);
void Com_Init3() {
    if ( Cbuf_GetCursize() ) {
        printf("executing command buffer\n" );
        Cbuf_Execute();
        return;
    }
    ...
    //Cbuf_ExecuteText(EXEC_NOW, "cmdlist"); printf("Com_Init3\n"); assert(0);
    ...
    char *s = va("%s %s %s", Q3_VERSION, PLATFORM_STRING, PRODUCT_DATE );
    ...
    load_progress("Initializing Client", 90);

    CL_Init(); // should have set cl_running to 1
    ...
    if ( ! Com_AddStartupCommands() ) {
        ...
        char *map_change = emscripten_run_script_string("window.sessionStorage.getItem('mapname')");

        if ( map_change ) {
            Com_Printf("MAP CHANGE %s  command line%s\n", map_change, commandLine);
            assert(0);
        }
        ...
        if ( port ) {
            ...
            Com_sprintf(buffer, MAX_QPATH - 1, "connect localhost:%s\n", port + 5);
            Com_Printf("buffer %s\n", buffer);
            Cbuf_AddText (buffer);
            extern char cl_reconnectArgs[MAX_OSPATH];  // TODO
            strcpy(cl_reconnectArgs, buffer);
        } else {
            assert(0);
        }
    }
    ...
    load_progress("Initializing Graphics", 95);
    void CL_InitRenderer( void );
    CL_InitRenderer(); // -> RE_BeginRegistration -> R_Init -> up to R_InitShaders which calls LoadShaderListFile

    void Com_Init4(void);
    emscripten_set_main_loop(Com_Init4, 1, 0);
}

void Com_Init4(void) {
ONCE;
    void CL_InitRenderer2( void );
    CL_InitRenderer2(); // -> R_InitShaders2 -> Com_Init5
}
...
ONCE;
    ...
    load_progress("Initializing Sound ", 100);
    ...
    emscripten_set_main_loop(Com_Frame, -1, 0 );
...
#if 0
#define FPS_FRAMES  4
static float Com_CalcFPS(void) {
    static int previousTimes[FPS_FRAMES] = { 0 };
...
void Com_Frame( void ) {
...
#if 1
    if ( setjmp (abortframe) ) { // TODO do I need this?
    ...
    NET_Event(); // if CL_DownloadsComplete/CL_InitCGame is called, call CG_Init from here
    ...
    if ( CL_CgameStarted() ) {
        static int once = 0;
        if ( once ) {
            Com_Printf("RE-INITIALIZING\n");
            //assert(0);
        }
        Com_Printf("Calling CL_InitCGame\n");
        // originally called from CL_DownloadsComplete and Vid_Restart_f
        //CL_InitCGame(); // if clc.state == CA_LOADING && cls.cgameStarted == qtrue
        once++;
        show_canvas();
        return;
    }
    ...
    IN_Frame(); // NO FS_* reachability

    lastTime      = com_frameTime;
    com_frameTime = Com_EventLoop();
    int msec      = com_frameTime - lastTime;

    // console and config commands ( seta, etc ) ONLY???
    Cbuf_Execute (); // vid_restart indirectly calls FS_*
...
void Com_Shutdown (void) {
    Com_Printf("Com_Shutdown canceling main loop\n");
    emscripten_cancel_main_loop();
}
...
===============
*/
void Field_CompleteFilename( const char *dir, const char *ext, qboolean stripExt, qboolean allowNonPureFilesOnDisk ) // TODO REMOVE ME ?
{
    UNUSED(dir); UNUSED(ext); UNUSED(stripExt); UNUSED(allowNonPureFilesOnDisk);
    return; // TODO
            //
    matchCount = 0;
    shortestMatch[ 0 ] = 0;
...
int Com_strCompare( const void *a, const void *b )
{
...
EMSCRIPTEN_KEEPALIVE
void Com_FromJSCommand(int cmd) {
    char *com = "";
    switch(cmd) {
        case 1: com = "team red"; break;
        case 2: com = "team blue"; break;
        case 3: com = "team free"; break;
        case 4: com = "team spectator"; break;
        default: com = "say hello";
    }

    Cbuf_ExecuteText(EXEC_NOW, com);
}
