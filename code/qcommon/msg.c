...
void MSG_Init( msg_t *buf, byte *data, int length ) {
	if (!msgInit) {
		...
		MSG_initHuffman();
#else
        Com_Error(ERR_FATAL, "huffman not inited\n");
#endif
	}
...
void MSG_WriteShort( msg_t *sb, int c ) {
    ...
    if (c < ((short)0x8000) || c > (short)0x7fff) {
        Com_Printf ("MSG_WriteShort: range error '%x'\n", c);
        assert(0);
    }
#endif
...
void MSG_WriteDeltaEntity( msg_t *msg, struct entityState_s *from, struct entityState_s *to,
    ...
    if ( ! (numFields + 1 == sizeof( *from )/4 ) ) {
        Com_Error(ERR_FATAL,"%s numFields + 1 == sizeof( *from )/4 \n", __func__);
    }
#else
	...
	for ( i = 0, field = entityStateFields ; i < numFields ; i++, field++ ) {
        fromF = (int *)( (byte *)from + field->offset ); // TODO possible alignment fault
        toF = (int *)( (byte *)to + field->offset ); // TODO possible alignment fault
		if ( *fromF != *toF ) {
			lc = i+1;
		}
	}
...
void MSG_initHuffman( void ) {
    if ( ! msgInit ) {
        ...
        Com_Printf("MSG_initHuffman called more than once...\n");
    }
}
