	...
	struct ws_connection *client, const void *buf, size_t len, int flags)
{
	...
	pthread_mutex_lock(&client->mtx_snd);
		while (len)
		{
			r = client->ssl ? SSL_write(client->ssl, p, len) : send(client->client_sock, p, len, flags); //r = send(client->client_sock, p, len, flags);
			if (r == -1)
			{
	...
	uint64_t size, int type, uint16_t port)
{
	...
	if (client && port == 0)
	{
		output = client->ssl ? SSL_write(client->ssl, response, idx_response) : SEND(client, response, idx_response); //output = SEND(client, response, idx_response);
		goto skip_broadcast;
	}
